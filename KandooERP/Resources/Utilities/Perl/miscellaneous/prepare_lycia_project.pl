...
#  "@(#)$Id: prepare_lycia_project.pl 366 2016-07-24 14:49:46Z  $:"
...
## options for files selection
	...
	"formdir=s"=>\$FormSourceDir, 		# Directory where .per source files are located
	"fgldir=s"=>\$FourGLSourceDir, 		# Directory where .4gl source files are located
	"cdir=s"=>\$CSourceDir, 				# Directory where .c source files are located
...
if (!defined($CheckOnly)) {
	printf "Project %s almost ready for import\n",$ProjectName;
	printf "\nThe prepared project structure is located in %s\n",$ProjectDir ;
	...
	READMAKE: while (<MAKEFILE>) {
		...
		if ( /$ProgramTargetComponentsExpr/ || /$LibraryTargetComponentsExpr/ || $ContinueList == 1 ) {
			...
			while ( defined($ModulesListArray[$mdl]) ) {
				#if ( $MakefileDir ne "." ) {
				#if ( $MakefileDir ne "." && $ModulesListArray[$mdl] !~ /^\.\// ) {
				if ( $MakefileDir ne "." && $ModulesListArray[$mdl] !~ /^\// ) {
					#$RelativePathName[$mdl] = sprintf "%s/%s",$MakefileDir,$ModulesListArray[$mdl] ;
					#$ModulesListArray[$mdl] = $RelativePathName[$mdl] ;
					$RelativePathName[$mdl] = $ModulesListArray[$mdl] ;
					$ModulesListArray[$mdl] = sprintf "%s/%s",$MakefileDir,$ModulesListArray[$mdl] ;
				} elsif ( $ModulesListArray[$mdl] !~ /^\.\// ) {
					$RelativePathName[$mdl] = sprintf "%s",$ModulesListArray[$mdl] ;
					$ModulesListArray[$mdl] = $RelativePathName[$mdl] ;
				} else {
					$RelativePathName[$mdl] = sprintf "%s",$ModulesListArray[$mdl] ;
					$ModulesListArray[$mdl] = $RelativePathName[$mdl] ;
				}
				# case of 4gl/../C/Ea_Newparser.4gl
				# cut the dir/../ part
				if ( $RelativePathName[$mdl] =~ /^\w+\/\.\.\// ) {
					$RelativePathName[$mdl] = $' ;
				}
				if (defined($ProgName) ) {
					# this normally a .4gl file, try .4gl first, if not, try .c, else file not found
					$ProgramList{$ProgName}->{Type} = "fgl-program" ;
					
					# need full path name here....
					if ( ( -f $ModulesListArray[$mdl] )) {
						$ModuleTypeArray[$mdl] = "fgl" ;
						$FullName = abs_path ($ModulesListArray[$mdl]) ;
						$ModulesListArray[$mdl] = $FullName ;
						$FglCount++;
					} else {
						# probably a .c module
					...
					if ( ( -f $ModulesListArray[$mdl] )) {
						...
						if ( ( -f $ModulesListArray[$mdl] )) {
							$FullName = abs_path ($ModulesListArray[$mdl]) ;
							$ModulesListArray[$mdl] = $FullName ;
							$RelativePathName[$mdl] =~ s/\.c/.4gl/ ;
							# fix relative path from c to 4gl
							$ModuleTypeArray[$mdl] = "fgl" ;
							$FglCount++;
						}	else {
...
sub print_forms_dependencies {
	...
	if (defined($CheckOnly)) {
		if ( keys %ProgramForms > 0 ) {
			...
			foreach $form ( keys %ProgramForms ) {
				if (defined($FormSourceDir)) {
					$f=sprintf "%s/%s.per",$FormSourceDir,$form ;
				} else {
					$f=sprintf "./%s.per",$form ;
				}
				if ( -f $f ) {
					printf "%s ",$form;
	...
	foreach $form ( keys %ProgramForms ) {
		...
		printf LOGFILE "form %s referenced in fgltarget file %s\n",$SourceFormFile,$FglTargetFile;
		if (defined($FormSourceDir)) {
			$SourceFormFile=sprintf "%s/%s.per",$FormSourceDir,$form ;
		} else {
			$SourceFormFile=sprintf "%s.per",$form ;
		}
		# check directory as with Relative path
...
while (defined($$ProgramList->{ModuleList}[$mdl])) {
	if ( $$ProgramList->{ModuleList}[$mdl] =~ /\.c$/ && $$ProgramList->{Type} eq "static-c-library" ) {
		...
		$TargetDir=sprintf "%s/%s",$ProjectSourceDir,dirname($$ProgramList->{RelativePathName}[$mdl]);
		if (! -d $TargetDir) {
			make_path($TargetDir) ;
		}
		printf FGLTARGETFILE   "    <file location=\"%s\"/>\n",$$ProgramList->{RelativePathName}[$mdl];
...
while (defined($$ProgramList->{ModuleList}[$mdl])) {
	if ( $$ProgramList->{ModuleList}[$mdl] =~ /\.4gl$/ && $$ProgramList->{Type} eq "fgl-library" ) {
		...
		### => check here how to keep directory tree
		### => if (! -d $TargetDir ) { mkpath ($TargetDir) }
...
while (defined($$ProgramList->{ModuleList}[$mdl])) {
	if ( $$ProgramList->{ModuleList}[$mdl] =~ /\.c$/ && $$ProgramList->{Type} eq "fgl-program" ) {
		...
		if (!defined($Libraries{$$ProgramList->{ModuleList}[$mdl]})) {
			...
			$NewLibraryList{$CLibraryName}->{RelativePathName}[$cfiles] = $$ProgramList->{RelativePathName}[$mdl] ;
			#$TargetDir=sprintf "%s/C",$ProjectSourceDir;
			$TargetDir=sprintf "%s/%s",$ProjectSourceDir,$CLibraryName;
			if (! -d $TargetDir) {
				make_path($TargetDir) ;
			}
			copy ( $$ProgramList->{ModuleList}[$mdl],$TargetDir ) ;
...
if ( defined($ThisProgramLibraries)) {
	...
	$LibNumber = keys %ThisProgramLibraries ;
	if ( $LibNumber > 0 ) {
		printf    FGLTARGETFILE "  </libraries>\n\n" ;
	}

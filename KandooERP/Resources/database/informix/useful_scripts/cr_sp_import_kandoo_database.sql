-- this stored procedure imports a database exported by export_to_external procedure 
-- it is not compatible with dbexport
CREATE FUNCTION import_from_external(source_dbname varchar(250),load_path LVARCHAR(512),target_dbname varchar(250),set_dbschema_tables CHAR(1)) 
...
-- intermediate_dbname is the name of the temporary database in which data is loaded, 
-- it will be renamed to target_dbname if everything runs successfully
...
LET create_tables_cmd = "dbaccess " || trim(intermediate_dbname) || " " || create_tables_file ;

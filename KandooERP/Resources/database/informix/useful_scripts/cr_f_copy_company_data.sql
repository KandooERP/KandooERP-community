DROP FUNCTION IF EXISTS copy_company_data_for_table ;
CREATE FUNCTION copy_company_data_for_table(table_name VARCHAR(128),source_cmpy CHAR(2),target_cmpy CHAR(2) ) 
       RETURNING INTEGER
...
FOREACH SELECT c.colname,c.colno 
	...
	IF table_name = "company" AND l_colname = "name_text" THEN
...
-- we put cmpy_code at the end to make it easy for the last ,
LET sql_statement = trim(sql_statement)||"'"||trim(target_cmpy)||"' FROM "||trim(table_name)|| " WHERE cmpy_code = '" || trim(source_cmpy) || "'"  ;

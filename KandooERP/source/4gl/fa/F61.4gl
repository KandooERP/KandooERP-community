...
GLOBALS "../fa/F61_GLOBALS.4gl"

# Purpose	:	Asset register REPORT
...
MAIN 
	...
	cur_depr_cost_amt money(14,2), 
	depr_amt money(14,2), 
	net_book_val_amt money(14,2), 
	salvage_amt money(14,2), 
	priv_use_per DECIMAL(5,2), 
	...
	OPEN WINDOW w181 with FORM "F181" -- alch kd-757 
	CALL  windecoration_f("F181") -- alch kd-757 

	MENU " Asset Register" 
		BEFORE MENU 
			CALL publish_toolbar("kandoo","F61","menu-asset_reg-1") -- alch kd-504 
...
FUNCTION report_f61() 
	...
	famast.location_code 
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","F61","const-fastatus-3") -- alch kd-504 
		ON ACTION "WEB-HELP" 
			CALL onlinehelp(getmoduleid(),null) -- alch kd-371 
	END CONSTRUCT 
	...
	INPUT pr_year_num, pr_period_num WITHOUT DEFAULTS FROM year_num,period_num 
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","F61","inp-pr_year_num-1") -- alch kd-504 
	...
	#------------------------------------------------------------
	LET l_rpt_idx = rpt_start(getmoduleid(),"F61_rpt_list",where_part, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT F61_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH fastatus_curs INTO pr_fastatus.*,pr_famast.* 
		...
		CASE sort_code 
			WHEN "1" 
				...
				LET fastatus_ext.sort2 = pr_famast.facat_code 
				SELECT facat_text 
				INTO fastatus_ext.desc_text2 
				FROM facat 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
				AND facat_code = pr_famast.facat_code 
				IF status THEN 
					LET fastatus_ext.desc_text2 = "No Description on file" 
				END IF 

				LET fastatus_ext.sort3 = pr_famast.faresp_code 
				SELECT faresp_text 
				INTO fastatus_ext.desc_text3 
				FROM faresp 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
				AND faresp_code = pr_famast.faresp_code 
				IF status THEN 
					LET fastatus_ext.desc_text3 = "No Description on file" 
				END IF 
			...
			WHEN "2" 
				...
				LET fastatus_ext.sort2 = pr_famast.location_code 
				SELECT location_text 
				INTO fastatus_ext.desc_text2 
				FROM falocation 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
			...
			WHEN "3" 
				...
				LET fastatus_ext.sort3 = pr_famast.facat_code 
				SELECT facat_text 
				INTO fastatus_ext.desc_text3 
				FROM facat 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
			...
			WHEN "4" 
				...
				LET fastatus_ext.sort3 = pr_famast.location_code 
				SELECT location_text 
				INTO fastatus_ext.desc_text3 
				FROM falocation 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
			...
			WHEN "5" 
				...
				LET fastatus_ext.sort2 = pr_famast.orig_auth_code 
				SELECT auth_text 
				INTO fastatus_ext.desc_text2 
				FROM faauth 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
			...
			WHEN "7" 
				LET fastatus_ext.sort1 = NULL 
				LET fastatus_ext.desc_text1 = NULL 
	...
	FOREACH report_curs INTO pr_fastatus.*, fastatus_ext.* 
		...
		DISPLAY "Printing Asset : ",pr_fastatus.asset_code at 1,1 
		...
		IF sort_code = "7" THEN 
			LET fastatus_ext.facat_code = "*" 
			LET fastatus_ext.location_code = "*" 
			LET fastatus_ext.faresp_code = "*" 
		END IF 
		IF sort_code matches "[56]" THEN 
			LET fastatus_ext.location_code = "*" 
			LET fastatus_ext.faresp_code = "*" 
								...
								IF tmp_depr IS NULL THEN LET tmp_depr = 0 END IF 
									...
									IF tmp_asset IS NULL THEN LET tmp_asset = 0 END IF 
										IF tmp_depr IS NULL THEN LET tmp_depr = 0 END IF 
											...
											LET linerec.opening_cost = linerec.opening_cost + tmp_asset 
											# subtract adjustments depreciation NOT add.
											LET linerec.opening_depr = linerec.opening_depr - tmp_depr 
	...
	#------------------------------------------------------------
	FINISH REPORT F61_rpt_list
	CALL rpt_finish("F61_rpt_list")
	#------------------------------------------------------------
...
###########################################################################
# REPORT F61_rpt_list(p_rpt_idx,rr_fastatus,rr_fastatus_ext,rr_famast,rr_linerec)   
#
#
###########################################################################
REPORT F61_rpt_list(p_rpt_idx,rr_fastatus,rr_fastatus_ext,rr_famast,rr_linerec) 
	...
	FORMAT 
		...
		BEFORE GROUP OF rr_fastatus_ext.sort1 
			IF type1 IS NOT NULL THEN 
				PRINT COLUMN 1,type1 clipped," : ",rr_fastatus_ext.sort1 clipped, 
		...
		BEFORE GROUP OF rr_fastatus_ext.sort2 
			IF type2 IS NOT NULL THEN 
				PRINT COLUMN 1,type2 clipped," : ",rr_fastatus_ext.sort2 clipped, 
		...
		BEFORE GROUP OF rr_fastatus_ext.sort3 
			IF type3 IS NOT NULL THEN 
				PRINT COLUMN 1,type3 clipped," : ",rr_fastatus_ext.sort3 clipped, 
		...
		ON EVERY ROW 
			...
			# determine closing cost AND depreciation AND nbv
			LET closing_cost = rr_linerec.opening_cost + 
			rr_linerec.adds + 
			rr_linerec.adjustments + 
			rr_linerec.transfers + 
			rr_linerec.disposals 

			LET closing_depr = rr_linerec.opening_depr + 
			rr_linerec.adds_depr + 
			rr_linerec.adjustments_depr + 
			rr_linerec.depreciation + 
			rr_linerec.transfers_depr + 
			rr_linerec.disposals_depr 

			LET nbv = closing_cost - closing_depr 

			IF rr_linerec.adds = 0 THEN LET rr_linerec.adds = NULL END IF 
				IF rr_linerec.adds_depr = 0 THEN LET rr_linerec.adds_depr = NULL END IF 
					IF rr_linerec.adjustments = 0 THEN 
						LET rr_linerec.adjustments = NULL 
					END IF 
					IF rr_linerec.adjustments_depr = 0 THEN 
						LET rr_linerec.adjustments_depr = NULL 
					END IF 
					IF rr_linerec.transfers = 0 THEN LET rr_linerec.transfers = NULL END IF 
						IF rr_linerec.transfers_depr = 0 THEN 
							LET rr_linerec.transfers_depr = NULL 
						END IF 
						IF rr_linerec.depreciation = 0 THEN 
							LET rr_linerec.depreciation = NULL 
						END IF 
						IF rr_linerec.disposals = 0 THEN LET rr_linerec.disposals = NULL END IF 
							IF rr_linerec.disposals_depr = 0 THEN 
								LET rr_linerec.disposals_depr = NULL 
							END IF 

							PRINT COLUMN 1,rr_fastatus.asset_code, 
							COLUMN 12,rr_fastatus.add_on_code, 
							COLUMN 24,rr_famast.desc_text 
							PRINT COLUMN 1,rr_linerec.opening_cost USING "--,---,---,-$&.&&", 
							COLUMN 19,rr_linerec.adds USING "--,---,---,-$&.&&", 
							COLUMN 37,rr_linerec.transfers USING "--,---,---,-$&.&&", 
							COLUMN 55,rr_linerec.adjustments USING "--,---,---,-$&.&&", 
							COLUMN 91,rr_linerec.disposals USING "--,---,---,-$&.&&", 
							COLUMN 109,closing_cost USING "--,---,---,-$&.&&" 
							PRINT COLUMN 1,rr_linerec.opening_depr USING "--,---,---,-$&.&&", 
							COLUMN 19,rr_linerec.adds_depr USING "--,---,---,-$&.&&", 
							COLUMN 37,rr_linerec.transfers_depr USING "--,---,---,-$&.&&", 
							COLUMN 55,rr_linerec.adjustments_depr USING "--,---,---,-$&.&&", 
							COLUMN 73,rr_linerec.depreciation USING "--,---,---,-$&.&&", 
							COLUMN 91,rr_linerec.disposals_depr USING "--,---,---,-$&.&&", 
							COLUMN 109,closing_depr USING "--,---,---,-$&.&&" 
		...
		AFTER GROUP OF rr_fastatus_ext.sort1 
			IF type1 IS NOT NULL THEN 
				...
				LET rr_print = rr_fastatus_ext.sort1 clipped," - ", 
				rr_fastatus_ext.desc_text1 

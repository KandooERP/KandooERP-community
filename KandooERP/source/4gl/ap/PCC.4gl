...
# RETURN VOID
#
# PCC - Debits by Period Report
############################################################
FUNCTION PCC_main()

	CALL setModuleId("PCC") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query 
		WHEN RPT_OP_MENU #UI/MENU Mode
			...
			MENU " Debits by Period" 
		
				BEFORE MENU 
					CALL publish_toolbar("kandoo","PCC","menu-debits_by_period-1") 
					CALL PCC_rpt_process(PCC_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL PCC_rpt_process(NULL) # (NULL query-where-part will read report_code from URL
		...
		WHEN RPT_OP_CONSTRUCT 
			...
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL PCC_rpt_query()
			CALL set_url_sel_text(PCC_rpt_query()) #save where clause in env 
			CLOSE WINDOW P112

		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL PCC_rpt_process(get_url_sel_text())
	END CASE 

END FUNCTION 
############################################################
# END FUNCTION PCC_main()
############################################################


############################################################
# FUNCTION PCC_rpt_query()
...
FUNCTION PCC_rpt_query() 
	...
	debithead.com2_text 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","PCC","construct-debithead-1") 
...
END FUNCTION 
############################################################
# END FUNCTION PCC_rpt_query() 
############################################################

############################################################
# FUNCTION PCC_rpt_process()
# RETURN rpt_finish("PCC_rpt_list")
# 
# The report driver
############################################################
FUNCTION PCC_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"PCC_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT PCC_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	WITH PAGE LENGTH = glob_arr_rec_rpt_rmsreps[l_rpt_idx].page_length_num , TOP MARGIN = 0, BOTTOM MARGIN = 0, LEFT MARGIN = 0, RIGHT MARGIN = 0
	LET p_where_text = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("PCC_rpt_list")].sel_text
	#------------------------------------------------------------
	...
	FOREACH c_debit INTO l_rec_debithead.* 
		...
		#------------------------------------------------------------
		OUTPUT TO REPORT PCC_rpt_list(rpt_rmsreps_idx_get_idx("PCC_rpt_list"), l_rec_debithead.*) 
		IF NOT rpt_int_flag_handler2("Debit: ",l_rec_debithead.debit_num, "" ,rpt_rmsreps_idx_get_idx("PCC_rpt_list")) THEN
			EXIT FOREACH 
		END IF 
		#------------------------------------------------------------
	...
	#------------------------------------------------------------
	FINISH REPORT PCC_rpt_list
	RETURN rpt_finish("PCC_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION PCC_rpt_process()
############################################################

############################################################
# REPORT PCC_rpt_list(p_rpt_idx,p_rec_debithead)
#
...
############################################################
# REPORT PCC_list(p_rec_debithead)
#
#
############################################################
REPORT PCC_rpt_list(p_rpt_idx, p_rec_debithead) 
...
--	DEFINE l_offset1, l_offset2 SMALLINT
	...
	OUTPUT 
		ORDER BY p_rec_debithead.year_num, 
			p_rec_debithead.period_num, 
	...
	FORMAT 
		PAGE HEADER 
			...
			PRINT COLUMN 01, "All Year, Period, Totals in base currency" 
		...
		BEFORE GROUP OF p_rec_debithead.period_num 
			SKIP TO top OF PAGE 
			PRINT COLUMN 5, "Year: ",p_rec_debithead.year_num USING "####", 
			COLUMN 20,"Period:", p_rec_debithead.period_num USING "###" 
		...
		AFTER GROUP OF p_rec_debithead.period_num 
			SKIP 1 line 
			PRINT COLUMN 1, "Period: ", p_rec_debithead.period_num 
			PRINT COLUMN 1, "Debits:", GROUP count(*) USING "####", 
			COLUMN 90, GROUP sum(p_rec_debithead.base_total_amt) 
			USING "---,---,--&.&&", 
			COLUMN 107, GROUP sum(p_rec_debithead.base_apply_amt) 
			USING "---,---,--&.&&" 

		AFTER GROUP OF p_rec_debithead.year_num 
			SKIP 1 line 
			PRINT COLUMN 1, "Year: ", p_rec_debithead.year_num 
			PRINT COLUMN 1, "Debits:", GROUP count(*) USING "####", 
			COLUMN 90, GROUP sum(p_rec_debithead.base_total_amt) 
			USING "---,---,--&.&&", 
			COLUMN 107, GROUP sum(p_rec_debithead.base_apply_amt) 

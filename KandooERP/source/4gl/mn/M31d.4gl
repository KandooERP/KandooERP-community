...
#
###########################################################################
# Purpose - Shop Order Add - Cost details entry SCREEN &
	...
	WITHOUT DEFAULTS 
		...
		AFTER FIELD work_centre_code 
			...
			IF status = notfound THEN 
				...
				# ERROR "Work Centre does NOT exist in the database -Try Window"
				NEXT FIELD work_centre_code 
			END IF 
			...
			OR fv_work_centre_code IS NULL THEN 
				...
				CALL display_wcrates(fr_workcentre.*) 
				RETURNING fv_variable_cost, fv_fixed_cost,fv_cost_markup_per 
			END IF 

...
# Purpose - Shop Order Maintenance - Item type entry SCREEN
############################################################
...
FUNCTION input_shoporddetl(fv_desc_text, fv_old_qty, fv_adjust) 
	...
	fv_error_num SMALLINT, 
	fv_adjust_qty FLOAT, 
	fv_adjust CHAR(1), 
	fv_add_qty LIKE prodstatus.onord_qty, 
	fv_old_qty LIKE prodstatus.onord_qty, 
	...
	LET fv_adjust_qty = pr_shopordhead.order_qty / fv_old_qty 
	...
	FOREACH c_so_detl INTO fr_shoporddetl.* 
		...
		LET pa_scrn_sodetl[pv_cnt].type_ind = fr_shoporddetl.type_ind 
		LET pa_scrn_sodetl[pv_cnt].sequence_num = fr_shoporddetl.sequence_num 
		LET pa_scrn_sodetl[pv_cnt].desc_text = fr_shoporddetl.desc_text 
		...
		CASE fr_shoporddetl.type_ind 
			...
			WHEN "S" 
				...
				ELSE 
					...
					* pr_shopordhead.order_qty 
					LET fr_shoporddetl.act_wgted_cost_amt = 
					fr_shoporddetl.std_price_amt * pr_shopordhead.order_qty 
				END IF 

				LET pv_ext_cost_amt = pv_ext_cost_amt + 
				fr_shoporddetl.act_est_cost_amt 
				LET pa_scrn_sodetl[pv_cnt].unit_cost_amt = 
				fr_shoporddetl.std_est_cost_amt 
			...
			WHEN "W" 
				...
				SELECT sum(rate_amt) 
				INTO fr_shoporddetl.std_act_cost_amt 
				FROM workctrrate 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
				AND work_centre_code = fr_shoporddetl.work_centre_code 
				AND rate_ind = "V" 
				...
				IF fr_workcentre.processing_ind = "Q" THEN 
					LET fr_shoporddetl.std_act_cost_amt = fv_fixed_rate + 
					((fr_shoporddetl.std_act_cost_amt / 
					fr_workcentre.time_qty) * fr_shoporddetl.required_qty) 
				ELSE 
					LET fr_shoporddetl.std_act_cost_amt = fv_fixed_rate + 
					(fr_shoporddetl.std_act_cost_amt * 
				...
				fr_shoporddetl.std_act_cost_amt * (1 + 
			...
			WHEN "U" 
				...
				IF fr_shoporddetl.cost_type_ind = "Q" THEN 
					...
					fr_shoporddetl.var_amt 

					IF (pr_shopordhead.order_qty / fr_shoporddetl.var_amt) > 
					fv_setup_qty THEN 
						LET fv_setup_qty = fv_setup_qty + 1 
					END IF 

					LET fr_shoporddetl.act_est_cost_amt = fv_setup_qty * 
					fr_shoporddetl.std_est_cost_amt 
					LET fr_shoporddetl.act_wgted_cost_amt = fv_setup_qty * 
					fr_shoporddetl.std_price_amt 
				ELSE 
					LET fr_shoporddetl.act_est_cost_amt = 
					fr_shoporddetl.std_est_cost_amt 
			...
			OTHERWISE 
				LET pa_scrn_sodetl[pv_cnt].part_code = fr_shoporddetl.part_code 
				...
				INTO pa_scrn_sodetl[pv_cnt].component_type_ind 
				FROM prodmfg 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
				AND part_code = fr_shoporddetl.part_code 

				IF pa_scrn_sodetl[pv_cnt].component_type_ind = "P" THEN 
					EXIT CASE 
				END IF 
				...
				IF fr_shoporddetl.type_ind = "C" THEN 
					CALL update_costs(fr_shoporddetl.*, fr_product.*, 
					fr_prodmfg.*) 
		...
		LET pa_scrn_sodetl[pv_cnt].required_qty = fr_shoporddetl.required_qty 
		LET pa_shoporddetl[pv_cnt].* = fr_shoporddetl.* 
	...
	LET pv_orig_arr_size = pv_cnt 

	OPEN WINDOW w3_m132 with FORM "M132" 
	...
	WHILE true 
		IF pr_shopordhead.status_ind = "H" THEN 
			LET msgresp = kandoomsg("M", 1533, "") 
			#MESSAGE"F1 Insert,F2 Delete,RETURN TO Add/Edit,F9 Image,ESC Exit-D"
		ELSE 
			LET msgresp = kandoomsg("M", 1535, "") 
			#MESSAGE"F1 Insert,F2 Delete,RETURN TO Add/Edit,ESC Accept-DEL Exit"
		END IF 
		...
		FROM sr_shoporddetl.* 
			...
			BEFORE DELETE 
				...
				IF pa_scrn_sodetl[fv_idx].sequence_num IS NOT NULL THEN 
					...
					AND pa_shoporddetl[fv_seq_num].cost_type_ind IS NULL THEN 
						...
						EXIT INPUT 
					ELSE 
						IF pa_scrn_sodetl[fv_idx].type_ind NOT matches "[BI]" 
						AND pr_shopordhead.status_ind = "R" 
						AND fv_seq_num <= pv_orig_arr_size THEN 

							CALL delete_check(fv_idx, fv_seq_num) 
							RETURNING fv_error_num 

							IF fv_error_num > 0 THEN 
								LET msgresp = kandoomsg("M", fv_error_num, "") 
								# ERROR "Cannot delete - ..."
								# 9756 "Actual cost IS greater than 0"
								# 9757 "Work in progress has been receipted"
								# 9758 "Issued quantity IS greater than 0"
								LET fv_delete = true 
								EXIT INPUT 
							END IF 
						END IF 
				...
				IF fv_type_ind matches "[CISWUB]" THEN 
					...
					ELSE 
						...
						IF pa_scrn_sodetl[fv_idx].sequence_num IS NULL THEN 
							...
							CASE pr_shoporddetl.type_ind 
								...
								WHEN "S" 
									LET pa_scrn_sodetl[fv_idx].part_code 
									= kandooword("COST", "M15") 
									LET pr_shoporddetl.act_act_cost_amt = 0 
									LET pr_shoporddetl.act_price_amt = 0 
								...
								WHEN "W" 
									...
									LET pr_shoporddetl.act_act_cost_amt = 0 
									LET pr_shoporddetl.act_price_amt = 0 
									LET pr_shoporddetl.receipted_qty = 0 
									LET pr_shoporddetl.rejected_qty = 0 
								...
								OTHERWISE 
									IF pa_scrn_sodetl[fv_idx].component_type_ind 
									= "P" THEN 
										EXIT CASE 
									END IF 

									LET pr_shoporddetl.act_est_cost_amt = 0 
									LET pr_shoporddetl.act_wgted_cost_amt = 0 
									LET pr_shoporddetl.act_act_cost_amt = 0 
									LET pr_shoporddetl.act_price_amt = 0 
						...
						IF pr_shoporddetl.type_ind != "I" THEN 
							CALL update_total(fv_idx, fv_seq_num) 
						END IF 
			...
			AFTER INPUT 
				...
				IF pv_cnt1 = 0 
				AND NOT fv_image THEN 
					LET msgresp = kandoomsg("M", 9784, "") 
					# ERROR "There must be AT least one detail line on the so"
					NEXT FIELD type_ind 
				END IF 

				IF pv_cnt1 = pv_phant_cnt 
				AND NOT fv_image THEN 
					LET msgresp = kandoomsg("M", 9788, "") 
					#ERROR "Cannot have just phantom components on the shop ord"
					NEXT FIELD type_ind 
				END IF 
	...
	CLOSE WINDOW w3_m132 

	IF int_flag OR quit_flag THEN 
		RETURN 
	END IF 

	LET msgresp = kandoomsg("M", 1537, "") 
	# MESSAGE "Please wait WHILE maintaining the shop ORDER..."
	...
	BEGIN WORK 
		...
		###
		### Update shopordhead AND on ORDER qty on prodstatus
		###

		IF pr_shopordhead.order_qty > fv_old_qty 
		AND pr_shopordhead.receipted_qty + pr_shopordhead.rejected_qty = fv_old_qty 
		THEN 
			SELECT * 
			INTO fr_prodmfg.* 
			FROM prodmfg 
		...
		LET pr_shopordhead.last_program_text = "M34" 

		LET err_message = "M34b - Update of shopordhead failed" 

		UPDATE shopordhead 
		SET shopordhead.* = pr_shopordhead.* 
		WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
		AND shop_order_num = pr_shopordhead.shop_order_num 
		AND suffix_num = pr_shopordhead.suffix_num 

		IF fv_old_qty != pr_shopordhead.order_qty THEN 
			LET err_message = "M34b - SELECT FROM prodmfg failed" 
			...
			LET err_message = "M34b - SELECT FROM product failed" 
			...
			LET fv_old_qty = fv_old_qty - pr_shopordhead.receipted_qty - 
			pr_shopordhead.rejected_qty 

			IF fv_old_qty < 0 THEN 
				LET fv_old_qty = 0 
			END IF 

			CALL uom_convert(fv_old_qty, pr_shopordhead.uom_code, fr_prodmfg.*, 
			fr_product.*) 
			RETURNING fv_old_qty 

			LET fv_add_qty = pr_shopordhead.order_qty - pr_shopordhead.receipted_qty 
			- pr_shopordhead.rejected_qty 

			IF fv_add_qty < 0 THEN 
				LET fv_add_qty = 0 
			END IF 

			CALL uom_convert(fv_add_qty, pr_shopordhead.uom_code, fr_prodmfg.*, 
			fr_product.*) 
			RETURNING fv_add_qty 

			LET err_message = "M34b - Update of prodstatus failed" 

			UPDATE prodstatus 
			SET onord_qty = onord_qty - fv_old_qty + fv_add_qty 
			WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
			AND part_code = pr_shopordhead.part_code 
		...
		###
		### Reverse out reserved AND on ORDER qtys FROM prodstatus
		### FOR the old shoporddetl lines
		###
		...
		FOREACH c_shoporddetl INTO fr_shoporddetl.* 
			...
			IF fr_shoporddetl.type_ind = "C" THEN 
				...
				IF fr_shoporddetl.required_qty < 0 THEN 
					LET fr_shoporddetl.required_qty = 0 
				END IF 

				CALL uom_convert(fr_shoporddetl.required_qty, 
				...
				UPDATE prodstatus 
				SET reserved_qty = reserved_qty - fv_old_qty 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
				AND part_code = fr_shoporddetl.part_code 
			...
			ELSE 
				...
				UPDATE prodstatus 
				SET onord_qty = onord_qty + fv_old_qty 
				WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
				AND part_code = fr_shoporddetl.part_code 
				AND ware_code = fr_shoporddetl.issue_ware_code 
			END IF 
		...
		###
		### Delete shoporddetl lines AND INSERT all lines in the array.
		### Update reserved AND on_order qtys on prodstatus.
		###

		LET err_message = "M34b - DELETE FROM shoporddetl failed" 

		DELETE FROM shoporddetl 
		WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
		AND shop_order_num = pr_shopordhead.shop_order_num 
		AND suffix_num = pr_shopordhead.suffix_num 
		...
		FOR fv_cnt = 1 TO pv_cnt1 
			...
			AND pa_sodetl_final[fv_cnt].type_ind = "W" 
			AND pa_sodetl_final[fv_cnt].sequence_num <= pv_orig_arr_size 
			AND pa_sodetl_final[fv_cnt].sequence_num != fv_cnt THEN 
				CALL update_wip(fv_cnt) 
			END IF 

			LET pa_sodetl_final[fv_cnt].cmpy_code = glob_rec_kandoouser.cmpy_code 
			LET pa_sodetl_final[fv_cnt].shop_order_num = 
			pr_shopordhead.shop_order_num 
			LET pa_sodetl_final[fv_cnt].suffix_num = pr_shopordhead.suffix_num 
			...
			LET pa_sodetl_final[fv_cnt].last_program_text = "M34" 
			LET err_message = "M34b - Insert INTO shoporddetl failed" 
			...
			IF pa_sodetl_final[fv_cnt].type_ind matches "[CB]" THEN 
				...
				IF pa_sodetl_final[fv_cnt].type_ind = "C" THEN 
					...
					LET pa_sodetl_final[fv_cnt].required_qty 
					= pa_sodetl_final[fv_cnt].required_qty 
					- pa_sodetl_final[fv_cnt].issued_qty 

					IF pa_sodetl_final[fv_cnt].required_qty < 0 THEN 
						LET pa_sodetl_final[fv_cnt].required_qty = 0 
					END IF 
				...
				ELSE 
					LET pa_sodetl_final[fv_cnt].required_qty 
					= pa_sodetl_final[fv_cnt].required_qty 
					- pa_sodetl_final[fv_cnt].receipted_qty 
					- pa_sodetl_final[fv_cnt].rejected_qty 
...
FUNCTION update_wip(fv_cnt) 
	...
	FOR fv_cnt1 = 1 TO pv_temp_cnt 
		IF pa_wip_seq[fv_cnt1].work_centre_code IS NULL THEN 
			CONTINUE FOR 
		END IF 

		IF pa_sodetl_final[fv_cnt].work_centre_code = 
		pa_wip_seq[fv_cnt1].work_centre_code 
		AND pa_sodetl_final[fv_cnt].sequence_num = 
		pa_wip_seq[fv_cnt1].sequence_num THEN 

			IF pa_wip_seq[fv_cnt1].exist_flag = "N" THEN 
				INITIALIZE pa_wip_seq[fv_cnt1].* TO NULL 
				RETURN 
			END IF 
	...
	IF fv_temp THEN 
		LET err_message = "M34b - SELECT FROM wipreceipt failed" 

		SELECT unique sequence_num 
		FROM wipreceipt 
		WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
		AND work_centre_code = pa_sodetl_final[fv_cnt].work_centre_code 
		AND shop_order_num = pr_shopordhead.shop_order_num 
		AND suffix_num = pr_shopordhead.suffix_num 
		AND sequence_num = fv_cnt 
		AND part_code = pr_shopordhead.part_code 

		IF status = notfound THEN 
			LET err_message = "M34b - Update of wipreceipt failed" 

			UPDATE wipreceipt 
			SET sequence_num = fv_cnt, 
			last_change_date = today, 
			last_user_text = glob_rec_kandoouser.sign_on_code, 
			last_program_text = "M34" 
			WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
			AND work_centre_code = pa_sodetl_final[fv_cnt].work_centre_code 
			AND shop_order_num = pr_shopordhead.shop_order_num 
			AND suffix_num = pr_shopordhead.suffix_num 
			AND sequence_num = pa_wip_seq[fv_cnt1].temp_seq_num 
			AND part_code = pr_shopordhead.part_code 

			INITIALIZE pa_wip_seq[fv_cnt1].* TO NULL 
			LET pv_temp_cnt = pv_temp_cnt + 1 
			LET pa_wip_seq[pv_temp_cnt].work_centre_code = 
			pa_sodetl_final[fv_cnt].work_centre_code 
			LET pa_wip_seq[pv_temp_cnt].sequence_num = fv_cnt 
			LET pa_wip_seq[pv_temp_cnt].exist_flag = "N" 
		ELSE 
			LET pv_temp_cnt = pv_temp_cnt + 1 
			LET pa_wip_seq[pv_temp_cnt].work_centre_code = 
			pa_sodetl_final[fv_cnt].work_centre_code 
			LET pa_wip_seq[pv_temp_cnt].sequence_num = fv_cnt 
			LET pa_wip_seq[pv_temp_cnt].exist_flag = "Y" 
			LET pa_wip_seq[pv_temp_cnt].temp_seq_num = 5000 + pv_temp_cnt 
			LET err_message = "M34b - Update of wipreceipt failed" 

			UPDATE wipreceipt 
			SET sequence_num = pa_wip_seq[pv_temp_cnt].temp_seq_num, 
			last_change_date = today, 
			last_user_text = glob_rec_kandoouser.sign_on_code, 
			last_program_text = "M34" 
			WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
	...
	ELSE 
		...
		AND shop_order_num = pr_shopordhead.shop_order_num 
		AND suffix_num = pr_shopordhead.suffix_num 
		AND sequence_num = pa_sodetl_final[fv_cnt].sequence_num 
		AND part_code = pr_shopordhead.part_code 
...
FUNCTION delete_check(fv_idx, fv_seq_num) 
	...
	LET fv_error_num = false 

	CASE 
		WHEN pa_scrn_sodetl[fv_idx].type_ind matches "[SU]" 
			IF pa_shoporddetl[fv_seq_num].act_act_cost_amt > 0 THEN 
				LET fv_error_num = 9756 
			END IF 

		WHEN pa_scrn_sodetl[fv_idx].type_ind = "W" 
			SELECT unique sequence_num 
			FROM wipreceipt 
			WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
			AND work_centre_code =pa_shoporddetl[fv_seq_num].work_centre_code 
			AND shop_order_num = pr_shopordhead.shop_order_num 
			AND suffix_num = pr_shopordhead.suffix_num 
			AND sequence_num = pa_shoporddetl[fv_seq_num].sequence_num 
			AND part_code = pr_shopordhead.part_code 

			IF status != notfound THEN 
				LET fv_error_num = 9757 
			END IF 

		WHEN pa_scrn_sodetl[fv_idx].type_ind = "C" 
			IF pa_shoporddetl[fv_seq_num].issued_qty > 0 THEN 
				LET fv_error_num = 9758 
			END IF 
	END CASE 
...
FUNCTION update_total(fv_idx, fv_seq_num) 
...
FUNCTION expand(fv_part_code, fv_type_ind, fv_req_qty, fv_parent_code) 
	...
	fv_type_ind LIKE shoporddetl.type_ind, 
	...
	IF fv_type_ind = "P" THEN 
		...
		END IF 
	ELSE 
		CALL input_configs(fv_part_code, fv_req_qty) RETURNING fv_arr_size 
...
FUNCTION input_configs(fv_part_code, fv_bor_req_qty) 
	...
	IF NOT pv_config THEN 
		OPEN WINDOW w4_m133 with FORM "M133" 
			...
			CASE 
				WHEN pr_inparms.cost_ind = "S" 
					...
					* pa_shoporddetl[pv_cnt].std_est_cost_amt) 
					LET fr_bor.cost_amt =pa_shoporddetl[pv_cnt].std_est_cost_amt 
...
FUNCTION get_so_record(fr_bor, fv_type_ind) 
	...
	CASE 
		...
		WHEN fr_bor.type_ind = "W" 
			...
			ELSE 
				LET fv_wc_cost = (fr_bor.cost_amt * fr_bor.oper_factor_amt * 
				pr_shopordhead.order_qty) + fr_bor.price_amt 
			END IF 
		...
		WHEN fr_bor.type_ind matches "[CB]" 
			...
			IF fr_shoporddetl.type_ind = "C" THEN 
				LET fr_shoporddetl.issued_qty = 0 
			ELSE 
				LET fr_shoporddetl.receipted_qty = 0 
				LET fr_shoporddetl.rejected_qty = 0 
			END IF 
...
FUNCTION delete_kids(fv_idx) 
	...
	FOR fv_cnt = fv_idx + 1 TO pv_arr_size 
		...
		pa_scrn_sodetl[fv_idx].part_code THEN 
			...
			ELSE 
				IF pa_scrn_sodetl[fv_cnt].type_ind NOT matches "[BI]" 
				AND pr_shopordhead.status_ind = "R" 
				AND fv_seq_num <= pv_orig_arr_size THEN 

					CALL delete_check(fv_cnt, fv_seq_num) 
					RETURNING fv_error_num 

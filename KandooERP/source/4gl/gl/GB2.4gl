...
# FUNCTION GB2_main()
#
# GB2  Batches By Number
############################################################
FUNCTION GB2_main()
	DEFER QUIT 
	DEFER INTERRUPT 
	
	CALL setModuleId("GB2")
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 
			...
			MENU " Batches by Number" 
				BEFORE MENU 
					CALL publish_toolbar("kandoo","GB2","menu-batch-by-num") 
					CALL GB2_rpt_process(GB2_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL GB2_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW G109 with FORM "G109" 
			CALL windecoration_g("G109") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(GB2_rpt_query()) #save where clause in env 
			CLOSE WINDOW G109 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL GB2_rpt_process(get_url_sel_text())
	END CASE 				
END FUNCTION 


############################################################
# FUNCTION GB2_rpt_query() 
#
#
############################################################
FUNCTION GB2_rpt_query() 
	...
	batchhead.jour_date 
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","GB2","construct-batchhead") 
...
#####################################################################
# FUNCTION GB2_rpt_process(p_where_text) 
#
#
#####################################################################
FUNCTION GB2_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"GB2_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT GB2_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	WITH PAGE LENGTH = glob_arr_rec_rpt_rmsreps[l_rpt_idx].page_length_num , TOP MARGIN = 0, BOTTOM MARGIN = 0, LEFT MARGIN = 0, RIGHT MARGIN = 0
	LET p_where_text = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("GB2_rpt_list")].sel_text
	#------------------------------------------------------------

	
	LET l_query_text = "SELECT * FROM batchhead ", 
	"WHERE batchhead.cmpy_code = '",glob_rec_kandoouser.cmpy_code,"' ", 
	"AND ", glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("GB2_rpt_list")].sel_text clipped," ", 
	"ORDER BY batchhead.jour_num " 
	...
	FOREACH selcurs INTO l_rec_batchhead.*
		#---------------------------------------------------------
		OUTPUT TO REPORT GB2_rpt_list(l_rpt_idx,l_rec_batchhead.*)  
	...
	END FOREACH 
	#------------------------------------------------------------
	FINISH REPORT GB2_rpt_list
	CALL rpt_finish("GB2_rpt_list")
	#------------------------------------------------------------
...
############################################################
# REPORT GB2_rpt_list(p_rec_batchhead)
#
#
############################################################
REPORT GB2_rpt_list(p_rpt_idx,p_rec_batchhead) 
	...
	OUTPUT 
	left margin 0 
	ORDER external BY p_rec_batchhead.jour_num 
	FORMAT 
		...
		ON EVERY ROW 
			...
			END IF 
			PRINT COLUMN 1, p_rec_batchhead.jour_num USING "#######", 
			COLUMN 10, p_rec_batchhead.jour_code, 
			COLUMN 18, p_rec_batchhead.jour_date USING "dd/mm/yy", 
			COLUMN 29, p_rec_batchhead.year_num USING "####", 
			COLUMN 34, p_rec_batchhead.period_num USING "###", 
			COLUMN 38, p_rec_batchhead.debit_amt USING "--,---,---,---,--&.&&", 
			COLUMN 63, p_rec_batchhead.for_debit_amt USING "--,---,---,---,--&.&&", 
			COLUMN 87, p_rec_batchhead.currency_code, 
			COLUMN 97, p_rec_batchhead.post_flag, 
			COLUMN 101, p_rec_batchhead.post_run_num USING "#######", 
			COLUMN 111, l_error_message 
		
		ON LAST ROW 
			SKIP 1 line 
			PRINT COLUMN 1, "Report Total:", 
			COLUMN 38, sum(p_rec_batchhead.debit_amt) USING "--,---,---,---,--&.&&" 
			SKIP 2 line 

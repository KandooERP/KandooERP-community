...
############################################################
############################################################
# FUNCTION GRG_rpt_process_a1()
#
# Summary Trial Balance - standard REPORT  with Year/Period TO Date & Actual/Preclose Options
############################################################
FUNCTION GRG_rpt_process_a1(p_rpt_idx,p_query_text,p_conv_qty,p_rec_period)
	...
	FOREACH ca1_account INTO l_rec_account.* 
		...
		AND l_rec_report.bal_amt = 0 THEN 
			CONTINUE FOREACH 
		ELSE 
			LET l_rec_report.open_amt = l_rec_report.open_amt * p_conv_qty 
			LET l_rec_report.debit_amt = l_rec_report.debit_amt * p_conv_qty 
			LET l_rec_report.credit_amt = l_rec_report.credit_amt * p_conv_qty 
			LET l_rec_report.bal_amt = l_rec_report.bal_amt * p_conv_qty 

			--DISPLAY l_rec_account.acct_code TO lbLabel1b  -- 1,29
			MESSAGE "Account: ", trim(l_rec_account.acct_code) 

			#---------------------------------------------------------
			OUTPUT TO REPORT GRG_rpt_list_A(p_rpt_idx,l_rec_report.*,p_rec_period.*) 
			IF NOT rpt_int_flag_handler2("GL-Account:",l_rec_account.acct_code, NULL,p_rpt_idx) THEN
				EXIT FOREACH 
			END IF 
			#---------------------------------------------------------
		END IF 
...
############################################################
# FUNCTION GRG_rpt_process_a2()
#
#
############################################################
FUNCTION GRG_rpt_process_a2(p_rpt_idx,p_query_text,p_conv_qty,p_rec_period)
	DEFINE p_rpt_idx SMALLINT #array index for glob_arr_rmsreps[p_rpt_idx]
	DEFINE p_query_text STRING
	DEFINE p_conv_qty LIKE rate_exchange.conv_sell_qty 
	DEFINE p_rec_period RECORD LIKE period.* 
	...
	FOREACH ca2_account INTO l_rec_account.* 
		...
		ELSE 
			...
			#---------------------------------------------------------
			OUTPUT TO REPORT GRG_rpt_list_A(p_rpt_idx,l_rec_report.*,p_rec_period.*) 
			IF NOT rpt_int_flag_handler2("GL-Acccount:",l_rec_report.acct_code, NULL,p_rpt_idx) THEN
				EXIT FOREACH 
			END IF 
			#---------------------------------------------------------
		END IF 
...
############################################################
# FUNCTION GRG_rpt_process_a3()
#
#
############################################################
FUNCTION GRG_rpt_process_a3(p_rpt_idx,p_query_text,p_conv_qty,p_rec_period)
	DEFINE p_rpt_idx SMALLINT #array index for glob_arr_rmsreps[p_rpt_idx]
	DEFINE p_query_text STRING
	DEFINE p_conv_qty LIKE rate_exchange.conv_sell_qty 
	DEFINE p_rec_period RECORD LIKE period.* 
...
############################################################
# FUNCTION GRG_rpt_process_a4()
#
#
############################################################
FUNCTION GRG_rpt_process_a4(p_rpt_idx,p_query_text,p_conv_qty,p_rec_period)
	DEFINE p_rpt_idx SMALLINT #array index for glob_arr_rmsreps[p_rpt_idx]
	DEFINE p_query_text STRING
	DEFINE p_conv_qty LIKE rate_exchange.conv_sell_qty 
	DEFINE p_rec_period RECORD LIKE period.*
...
############################################################
# REPORT GRG_rpt_list_A(p_rec_report)
#
#
############################################################
REPORT GRG_rpt_list_A(p_rpt_idx,p_rec_report,p_rec_period) 
	DEFINE p_rpt_idx SMALLINT #array index for glob_arr_rmsreps[p_rpt_idx]
	...
	DEFINE l_rec_period_head CHAR(6) 
	...
	FORMAT 
		PAGE HEADER
			...
			PRINT COLUMN 01, "Values in Currency: ", glob_arr_rec_rpt_rmsreps[p_rpt_idx].ref1_code #l_rept_curr_code
...
{					 
			...
			IF glob_totals_ind = "Y" THEN 
				...
				LET l_rec_period_head = NULL 
			ELSE 
				LET l_todate_head = "Period TO Date" 
				LET l_rec_period_head = "Period" 
			END IF 
			...
			COLUMN l_offset3, "Values in Currency: ", glob_rept_curr_code 
			PRINT COLUMN l_offset3, " Exchange Rate: ", glob_conv_qty USING "<<<.<<<" 
			PRINT COLUMN 01,"--------------------------------------------", 
			"--------------------------------------------", 
			"--------------------------------------------" 
}	

...
FUNCTION GAJ_main() 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 
			...
			MENU " Account History by Journal" 
				BEFORE MENU 
					CALL publish_toolbar("kandoo","GAJ","menu-account-history-by-journ") 
					CALL GAJ_rpt_process(GAJ_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL GAJ_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW G103 with FORM "G103" 
			CALL windecoration_g("G103") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(GAJ_rpt_query()) #save where clause in env 
			CLOSE WINDOW G103 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL GAJ_rpt_process(get_url_sel_text())
	END CASE
...
FUNCTION GAJ_rpt_query() 
	...
	CONSTRUCT l_where_text ON accounthist.acct_code, 
	coa.desc_text, 
	...
	accountledger.stats_qty 
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","GAJ","construct-accountledger") 
	...
	RETURNING modu_q1_text 

	IF modu_q1_text IS NULL THEN 
		RETURN NULL 
	END IF 
...
############################################################
# FUNCTION GAJ_rpt_process ()  
#
#
############################################################
FUNCTION GAJ_rpt_process(p_where_text) 
	DEFINE p_where_text STRING 
	DEFINE l_query_text STRING
	DEFINE l_rpt_idx SMALLINT
	DEFINE l_rec_prev_acct_code LIKE accountledger.acct_code 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"GAJ_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT GAJ_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	WITH PAGE LENGTH = glob_arr_rec_rpt_rmsreps[l_rpt_idx].page_length_num , TOP MARGIN = 0, BOTTOM MARGIN = 0, LEFT MARGIN = 0, RIGHT MARGIN = 0
	LET p_where_text = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("GAJ_rpt_list")].sel_text
	#------------------------------------------------------------


	LET l_query_text = 
	"SELECT accounthist.acct_code, ", 
	"accounthist.year_num, ", 
	"accounthist.period_num, ", 
	"accounthist.open_amt, ", 
	"accounthist.close_amt, ", 
	"accountledger.* ", 
	"FROM accounthist, coa, outer accountledger ", 
	...
	"AND accounthist.acct_code = accountledger.acct_code ", 
	"AND accounthist.year_num = accountledger.year_num ", 
	"AND accounthist.period_num = accountledger.period_num ", 
	"AND ",	glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("GAJ_rpt_list")].sel_text clipped, " ", 
	" ORDER BY accounthist.acct_code, accounthist.year_num,", 
	" accounthist.period_num, accountledger.jour_code,", 
	" accountledger.seq_num " 
	...
	LET l_rec_prev_acct_code = " " 
	FOREACH c_accountledger INTO modu_rec_hist_rec.*, 
		modu_rec_accountledger.* 
		...
		END IF 
		#
		# IF accountledger details are NULL, there are no ledger entries
		# FOR this accounthist.  Do NOT PRINT accounts with a zero opening
		# balance AND no account ledger details.
		#
		IF modu_rec_accountledger.jour_code IS NOT NULL OR 
		modu_rec_hist_rec.open_amt <> 0 THEN 
			#---------------------------------------------------------
			OUTPUT TO REPORT GAJ_rpt_list(l_rpt_idx,modu_rec_hist_rec.*, modu_rec_accountledger.*) 
			IF NOT rpt_int_flag_handler2("Account:",modu_rec_hist_rec.acct_code, NULL,l_rpt_idx) THEN
				EXIT FOREACH 
			END IF 
			#---------------------------------------------------------		
		END IF 

		IF l_rec_prev_acct_code != modu_rec_hist_rec.acct_code THEN 
			MESSAGE modu_rec_hist_rec.acct_code 
			LET l_rec_prev_acct_code = modu_rec_hist_rec.acct_code 
		END IF 
	...
	#------------------------------------------------------------
	FINISH REPORT GAJ_rpt_list
	CALL rpt_finish("GAJ_rpt_list")
	#------------------------------------------------------------
...
############################################################
# REPORT GAJ_rpt_list(p_rec_hist_rec, p_rec_accountledger)
#
#
############################################################
REPORT GAJ_rpt_list(p_rpt_idx,p_rec_hist_rec, p_rec_accountledger) 
	DEFINE p_rpt_idx SMALLINT #array index for glob_arr_rmsreps[p_rpt_idx]
	DEFINE p_rec_hist_rec RECORD 
		acct_code LIKE accounthist.acct_code, 
		year_num LIKE accounthist.year_num, 
		period_num LIKE accounthist.period_num, 
		open_amt LIKE accounthist.open_amt, 
	...
	OUTPUT 
	--left margin 0 
	ORDER external BY p_rec_hist_rec.acct_code, 
	p_rec_hist_rec.year_num, 
	p_rec_hist_rec.period_num, 
	...
	FORMAT 
		PAGE HEADER 
			...
			IF modu_first_time = 0 THEN 
				PRINT COLUMN 1, " .... Account: ", p_rec_hist_rec.acct_code, 
				2 spaces, l_rec_coa.desc_text 
			ELSE 
				PRINT COLUMN 1, " " 
			END IF 
		BEFORE GROUP OF p_rec_hist_rec.acct_code 
			SELECT * INTO l_rec_coa.* FROM coa 
			WHERE acct_code = p_rec_hist_rec.acct_code 
			AND cmpy_code = glob_rec_kandoouser.cmpy_code 
			IF status = NOTFOUND THEN 
				LET l_rec_coa.desc_text = "** Not found **" 
			END IF 
			SKIP 2 LINES 
			PRINT COLUMN 5, " Account: ", p_rec_hist_rec.acct_code, 
			2 spaces, l_rec_coa.desc_text 
			LET modu_first_time = 1 
			SKIP 1 LINES 
			
		AFTER GROUP OF p_rec_hist_rec.period_num 
			PRINT COLUMN 1, "Period: ", p_rec_hist_rec.year_num USING "###&","/", 
			p_rec_hist_rec.period_num USING "<<<", 
			COLUMN 44, GROUP sum(p_rec_accountledger.debit_amt) 
			USING "--,---,---,--&.&&", 
			COLUMN 62, GROUP sum(p_rec_accountledger.credit_amt) 
			USING "--,---,---,--&.&&" 
			LET modu_close_amt = ac_form(glob_rec_kandoouser.cmpy_code, p_rec_hist_rec.close_amt, 
			l_rec_coa.type_ind, 
		...
		ON EVERY ROW 
			IF modu_first_time = 1 THEN 
				LET modu_open_amt = ac_form(glob_rec_kandoouser.cmpy_code, p_rec_hist_rec.open_amt, 
				l_rec_coa.type_ind, 
				glob_rec_glparms.style_ind) 
			...
			END IF 
			#
			# Print AND keep track of DB / CR FOR jour_code's

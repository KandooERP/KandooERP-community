...
# FUNCTION GB4_main()
#
#  Batch Detail listing
############################################################
FUNCTION GB4_main()
	DEFER QUIT 
	DEFER INTERRUPT 
	
	CALL setModuleId("GB4")
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 
			...
			MENU " Batch Detail" 
				BEFORE MENU 
					CALL publish_toolbar("kandoo","GB4","menu-batch-detail") 
					CALL GB4_rpt_process(GB4_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL GB4_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW G157 with FORM "G157" 
			CALL windecoration_g("G157") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(GB4_rpt_query()) #save where clause in env 
			CLOSE WINDOW G157 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL GB4_rpt_process(get_url_sel_text())
	END CASE 
END FUNCTION 


############################################################
# FUNCTION GB4_rpt_query()
#
#
############################################################
FUNCTION GB4_rpt_query() 
	...
	batchhead.currency_code 
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","GB4","construct-batchhead") 
	...
	ELSE
		...
		IF glob_rec_glparms.use_currency_flag = "Y" THEN 
			...
			#3511 Print Base OR Foreign amounts (b/f):? "
		...
		LET glob_rec_rpt_selector.ref2_ind = modu_currency
		RETURN l_where_text
	END IF 
END FUNCTION 
...
#####################################################################
# FUNCTION GB4_rpt_process(p_where_text) 
#
#
#####################################################################
FUNCTION GB4_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"GB4_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT GB4_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	WITH PAGE LENGTH = glob_arr_rec_rpt_rmsreps[l_rpt_idx].page_length_num , TOP MARGIN = 0, BOTTOM MARGIN = 0, LEFT MARGIN = 0, RIGHT MARGIN = 0
	LET p_where_text = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("GB4_rpt_list")].sel_text
	#------------------------------------------------------------

	#data selector from rmsreps
	LET modu_newpage = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("GB4_rpt_list")].ref1_ind 
	LET modu_currency = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("GB4_rpt_list")].ref2_ind 
		
	LET l_query_text = 
	"SELECT batchdetl.* FROM batchdetl, batchhead ", 
	...
	"AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("GB4_rpt_list")].sel_text clipped," ", 
	"ORDER BY batchdetl.cmpy_code,batchdetl.jour_num,batchdetl.seq_num" 
	...
	FOREACH selcurs INTO l_rec_batchdetl.* 
		...
		END IF 
		#---------------------------------------------------------
		OUTPUT TO REPORT GB4_rpt_list(l_rpt_idx,l_rec_batchdetl.*)
		IF NOT rpt_int_flag_handler2("Batch:",l_rec_batchdetl.currency_code, NULL,l_rpt_idx) THEN
			EXIT FOREACH 
		END IF 
		#---------------------------------------------------------	
	END FOREACH 
	...
	#------------------------------------------------------------
	FINISH REPORT GB4_rpt_list
	CALL rpt_finish("GB4_rpt_list")
	#------------------------------------------------------------
...
############################################################
# REPORT GB4_rpt_list(p_rpt_idx,p_rec_batchdetl)
#
#
############################################################
REPORT GB4_rpt_list(p_rpt_idx,p_rec_batchdetl) 
...
--	left margin 0 
	...
	FORMAT 
		...
		BEFORE GROUP OF p_rec_batchdetl.jour_num 
			...
			COLUMN 25, "Posting Year: ", l_rec_batchhead.year_num, 
			COLUMN 55, "Period: ", l_rec_batchhead.period_num, 
			COLUMN 75, "Entered By: ", l_rec_batchhead.entry_code, 
			COLUMN 115, "(", l_rec_batchhead.currency_code, ")" 
			PRINT COLUMN 5, "Cleared : ", l_rec_batchhead.cleared_flag, 
			COLUMN 25, "Posting Run: ", l_rec_batchhead.post_run_num 
			USING "#######", 
			COLUMN 88, "Control Total: ", l_rec_batchhead.control_amt 
			USING "----,---,---,--&.&&" 

			IF l_rec_batchhead.com1_text IS NOT NULL THEN 
				PRINT COLUMN 5, "Comments:", 
				COLUMN 16, l_rec_batchhead.com1_text, 
				COLUMN 93, "Quantity: ", l_rec_batchhead.control_qty 
				USING "----,---,---,--&.&" 
			...
			ELSE 
				IF l_rec_batchhead.com2_text IS NOT NULL THEN 
					...
					USING "----,---,---,--&.&" 
				ELSE 
					PRINT COLUMN 93, "Quantity: ", l_rec_batchhead.control_qty 
					USING "----,---,---,--&.&" 
				END IF 
			END IF 
		...
		ON EVERY ROW 
			...
			IF l_rec_coa.uom_code IS NULL THEN 
				LET p_rec_batchdetl.stats_qty = NULL 
			END IF 
			PRINT COLUMN 1, p_rec_batchdetl.seq_num USING "####", 
			COLUMN 6, p_rec_batchdetl.tran_type_ind, 
			COLUMN 10, p_rec_batchdetl.acct_code, 
			COLUMN 29, p_rec_batchdetl.desc_text, 
			COLUMN 60, p_rec_batchdetl.stats_qty USING "#,###,###,##&.&", 
			COLUMN 75, l_rec_coa.uom_code, 
			COLUMN 79, p_rec_batchdetl.analysis_text; 
			IF modu_currency = "F" THEN 
				PRINT COLUMN 96, p_rec_batchdetl.for_debit_amt USING "###,###,###,##&.&&", 
				COLUMN 115,p_rec_batchdetl.for_credit_amt USING "###,###,###,##&.&&" 
			ELSE 
				PRINT COLUMN 96, p_rec_batchdetl.debit_amt USING "###,###,###,##&.&&", 
				COLUMN 115, p_rec_batchdetl.credit_amt USING "###,###,###,##&.&&" 
			END IF 
		...
		ON LAST ROW 
			SKIP 2 LINES 
			IF modu_currency = "F" THEN 
				IF NOT modu_multiple_curr_flag THEN 
					PRINT COLUMN 1, "Report Total:", 
					COLUMN 96, sum(p_rec_batchdetl.for_debit_amt) 
					USING "---,---,---,--&.&&" 
					PRINT COLUMN 115, sum(p_rec_batchdetl.for_credit_amt) 
					USING "---,---,---,--&.&&" 
				END IF 
			ELSE 
				PRINT COLUMN 1, "Report Total:", 
				COLUMN 96, sum(p_rec_batchdetl.debit_amt) USING "---,---,---,--&.&&" 
				PRINT COLUMN 115, sum(p_rec_batchdetl.credit_amt) 
				USING "---,---,---,--&.&&" 
			END IF 

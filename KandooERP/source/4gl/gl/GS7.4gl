...
# \file
# \brief module GS7 Allows mass changes of organisational codes
...
############################################################
	...
	DEFINE modu_start_year SMALLINT
	DEFINE modu_start_period SMALLINT
...
--	DISPLAY " Account Matches : ", modu_bacct at 2,1 
	...
	IF length(modu_bacct) = 18 THEN 
	ELSE 
		LET modu_bacct = modu_bacct [1,modu_lengther] , "*" 
	END IF
	 
	WHILE true 
		LET modu_start_year = fgl_winprompt(5,5, "WHERE Start Year =", "", 25, 0) 

		IF modu_start_year IS NULL THEN 
			LET modu_start_year = 0 
			SELECT count(*) 
			INTO modu_counter 
			FROM coa 
		...
		ELSE 
			LET modu_start_period = fgl_winprompt(5,5, "WHERE Start Period ", "", 25, 0) 

			IF modu_start_period IS NULL THEN 
				LET modu_start_period = 0 
				SELECT count(*) 
				INTO modu_counter 
				FROM coa 
				WHERE acct_code matches modu_bacct 
				AND start_year_num = modu_start_year 
				AND cmpy_code = glob_rec_kandoouser.cmpy_code 
			ELSE 
				SELECT count(*) 
				INTO modu_counter 
				...
				WHERE acct_code matches modu_bacct 
				AND start_year_num = modu_start_year 
				AND start_period_num = modu_start_period 
				AND cmpy_code = glob_rec_kandoouser.cmpy_code 
			END IF 
		END IF
	...
	LABEL nextgo: 
	LET modu_to_year = fgl_winprompt(5,5, " TO Start Year", "", 25, 0) 

	IF modu_to_year < 1988 THEN 
		ERROR " Start year must be > 1988 " 
		GOTO nextgo 
	END IF 

	LET modu_to_period = fgl_winprompt(5,5, "TO Start Period", "", 25, 0) 
	...
	IF modu_start_year = 0 THEN 
		UPDATE coa 
		SET start_year_num = modu_to_year, 
		start_period_num = modu_to_period 
		WHERE acct_code matches modu_bacct 
		AND cmpy_code = glob_rec_kandoouser.cmpy_code 
	ELSE 
		IF modu_start_period = 0 THEN 
			UPDATE coa 
			SET start_year_num = modu_to_year, 
			start_period_num = modu_to_period 
			WHERE acct_code matches modu_bacct 

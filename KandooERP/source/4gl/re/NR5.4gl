...
# MAIN
#
# \brief module NR5 - Requisition Back Order Detail List Report
############################################################
MAIN 
	DEFER quit 
	DEFER interrupt 
	
	CALL setModuleId("NR5") 
...
FUNCTION NR5_rpt_query() 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"NR5_rpt_list",where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT NR5_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
...
{
	#Keep this here.. I could not understand why this was done... may be later we will understand
	IF rpt_note IS NULL THEN 
		LET rpt_note = "RE Requisition Back Order Detail List II" 
	END IF 
	CALL init_report(glob_rec_kandoouser.cmpy_code, 
	glob_rec_kandoouser.sign_on_code, 
	rpt_note) 
	RETURNING pr_output 
	IF rpt_note IS NULL THEN 
		LET rpt_note = "RE Requisition Back Order Detail List" 
	END IF 
	START REPORT NR5_rpt_list TO pr_output 
	...
	FOREACH c_reqhead INTO pr_reqhead.*, pr_reqdetl.*
		...
		#---------------------------------------------------------
		OUTPUT TO REPORT NR5_rpt_list(l_rpt_idx,	 
	...
	#------------------------------------------------------------
	FINISH REPORT NR5_rpt_list
	CALL rpt_finish("NR5_rpt_list")
	#------------------------------------------------------------
...
REPORT NR5_rpt_list(p_rpt_idx,pr_reqhead, pr_reqdetl) 
	...
	FORMAT 
		...
		ON EVERY ROW 
			...
			COLUMN 64, pr_reqdetl.po_qty - pr_reqdetl.po_rec_qty 
			USING "#######&.&&", 
			COLUMN 76, pr_reqdetl.desc_text[1,25], 
		...
		AFTER GROUP OF pr_reqdetl.part_code 
			...
			COLUMN 64, GROUP sum(pr_reqdetl.po_qty) USING "#######&.&&", 

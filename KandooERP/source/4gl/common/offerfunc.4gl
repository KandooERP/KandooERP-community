...
#  calculated.  The table t_orderdetl IS used TO interface with lines
#  in the sale.
#
#  program note  1. The temp table COLUMN t_orderdetl.job_code IS used
...
FUNCTION validate_offer(p_cmpy,p_type_ind,p_type_code) 
	...
	IF p_type_ind = "1" THEN ### condition validation 
		...
		IF l_rec_condsale.prodline_disc_flag = "N" THEN 
			IF l_rec_condsale.tier_disc_flag = "Y" THEN 
				UPDATE t_orderdetl 
				SET 
					disc_per = l_rec_conddisc.disc_per, 
		...
		ELSE 
			...
			###- check t_orderdetl records that do NOT match a          -###
			...
			DECLARE c0_orderdetl CURSOR FOR 
			SELECT 
				part_code, 
				prodgrp_code, 
				maingrp_code, 
			...
			FOREACH c0_orderdetl INTO 
				l_rec_orderdetl.part_code, 
				l_rec_orderdetl.prodgrp_code, 
				l_rec_orderdetl.maingrp_code, 
				l_rec_orderdetl.ext_price_amt 

				UPDATE t_proddisc 
				SET 
					allocated_amt = allocated_amt+l_rec_orderdetl.ext_price_amt, 
					poss_disc_amt = poss_disc_amt	+ ((disc_per/100)	* l_rec_orderdetl.ext_price_amt), 
					disc_taken_ind = false 
				WHERE type_ind = "1" 
				AND key_num = p_type_code 
				AND maingrp_code = l_rec_orderdetl.maingrp_code 
				AND (prodgrp_code IS NULL 
				OR prodgrp_code = l_rec_orderdetl.prodgrp_code) 
				AND (part_code IS NULL OR part_code = l_rec_orderdetl.part_code) 
			END FOREACH 
			...
			WHILE true 
				...
				IF sqlca.sqlcode = 0 THEN 
					...
					FOREACH c1_orderdetl INTO 
						...
						OR part_code = l_rec_orderdetl.part_code) 
					END FOREACH
		...
		DECLARE c1_t_orderdetl CURSOR FOR 
		...
		FOREACH c1_t_orderdetl INTO l_rec_orderdetl.* 
			LET l_calc_disc = "Y" 
			IF l_rec_orderdetl.inv_qty != 0 THEN 
				LET l_calc_disc = "N" 
			END IF 
			
			IF l_rec_orderdetl.trade_in_flag = "Y" THEN 
				LET l_calc_disc = "N" 
			END IF 
			
			IF l_rec_orderdetl.list_price_amt = 0 THEN 
				LET l_calc_disc = "N" 
			END IF 
			
			IF l_rec_orderdetl.list_price_amt < l_rec_orderdetl.unit_price_amt THEN 
				LET l_calc_disc = "N" 
			END IF 
			
			IF NOT l_rec_orderdetl.job_code THEN 
				## use whatever price entered in line item entry routine
				LET l_calc_disc = "N" 
			END IF 
			
			LET l_rec_orderdetl.bonus_disc_amt = 0 
			IF l_rec_orderdetl.disc_allow_flag = "Y" THEN 
				IF l_disc_base_amt != 0 THEN 
					LET l_rec_orderdetl.bonus_disc_amt = 
						l_rec_orderdetl.list_price_amt	* l_rec_orderdetl.order_qty	* (l_total_bonus_amt/l_disc_base_amt) 
				END IF 
			END IF 
			
			IF l_calc_disc = "Y" THEN 
				## calc price based on disc
				LET l_rec_orderdetl.disc_amt = l_rec_orderdetl.sold_qty * (l_rec_orderdetl.list_price_amt-l_rec_orderdetl.unit_price_amt) 
				LET l_rec_orderdetl.unit_price_amt = l_rec_orderdetl.list_price_amt -(l_rec_orderdetl.list_price_amt*(l_rec_orderdetl.disc_per/100)) 
				
				UPDATE t_orderdetl SET
					unit_price_amt = l_rec_orderdetl.unit_price_amt, 
					disc_amt = l_rec_orderdetl.disc_amt 
				WHERE line_num = l_rec_orderdetl.line_num 
			END IF
			 
			LET l_total_grs_amt = l_total_grs_amt + (l_rec_orderdetl.order_qty*l_rec_orderdetl.list_price_amt) 
			LET l_total_net_amt = l_total_net_amt + (l_rec_orderdetl.sold_qty*l_rec_orderdetl.unit_price_amt) 
		END FOREACH 
		...
		IF l_disc_base_amt != 0 THEN 
			...
			IF (l_actual_disc_per < 0) OR	(l_actual_disc_per IS null) 
			THEN 
				LET l_actual_disc_per = 0 
			END IF 
		ELSE 
		...
		ELSE
			...
			ELSE 
				LET l_bonus_disc_per = (l_total_bonus_amt*100) / (l_total_sold_amt + l_total_bonus_amt) 
			END IF 
			
			IF l_bonus_disc_per > l_rec_conddisc.bonus_check_per THEN 
				IF l_error_code = 0 THEN 
					LET l_error_code = 7021 
	...
	ELSE 
		...
		CASE l_rec_offersale.disc_rule_ind 
			WHEN "1" 
				...
				IF l_rec_offersale.bonus_check_per = 0 THEN 
					...
					LET l_rec_offersale.bonus_check_amt = l_rec_offersale.bonus_check_amt		* l_rec_orderoffer.offer_qty 
					IF (l_total_sold_amt+l_total_bonus_amt) = 0 THEN 
						LET l_rec_offersale.bonus_check_per = 0 
					ELSE 
						LET l_rec_offersale.bonus_check_per = 
						(100*l_rec_offersale.bonus_check_amt) / (l_total_sold_amt+l_total_bonus_amt) 
					END IF 
				END IF 
		...
		LET l_rec_offersale.min_sold_amt = l_rec_offersale.min_sold_amt * l_rec_orderoffer.offer_qty 
		LET l_rec_offersale.min_order_amt = l_rec_offersale.min_order_amt * l_rec_orderoffer.offer_qty 
		...
		IF l_rec_offersale.checkrule_ind > "1" THEN 
			...
			IF l_total_sold_amt < l_rec_offersale.min_sold_amt THEN 
				IF l_error_code = 0 THEN 
					...
					LET l_error_code = 7022 
				END IF 
			END IF 

			IF (l_total_sold_amt + l_total_bonus_amt)	< l_rec_offersale.min_order_amt THEN 
				IF l_error_code = 0 THEN 
					#7023" Offer Amount < minimum required "
					LET l_error_code = 7023 
				END IF 
			END IF 
			...
			IF l_rec_offersale.checkrule_ind > "2" THEN 
				...
				IF l_rec_offersale.checktype_ind = "1" THEN 
					...
					SELECT 
						...
						prodgrp_code, 
						part_code, 
						(reqd_qty*l_rec_orderoffer.offer_qty),
						0 
					FROM offerprod 
					WHERE cmpy_code = p_cmpy 
					AND offer_code = l_rec_orderoffer.offer_code 
				ELSE 
					...
					SELECT 
						...
						prodgrp_code, 
						part_code, 
						(reqd_amt*l_rec_orderoffer.offer_qty), 
						0 
					FROM offerprod 
					WHERE cmpy_code = p_cmpy 
					AND offer_code = l_rec_orderoffer.offer_code 
				...
				IF l_rec_offersale.checkrule_ind = "3" OR l_rec_offersale.checkrule_ind = "5" THEN 
					...
					IF sqlca.sqlcode = 0 THEN 
						...
						AND status_ind in ("0","1","2") 
						AND sold_qty != 0 
						AND (autoinsert_flag = "N" OR autoinsert_flag IS null) 
						GROUP BY 
							part_code, 
							prodgrp_code, 
							maingrp_code 

						FOREACH c2_orderdetl INTO l_rec_orderdetl.part_code, 
							l_rec_orderdetl.prodgrp_code, 
							l_rec_orderdetl.maingrp_code, 
							l_rec_orderdetl.sold_qty, 
							l_rec_orderdetl.ext_price_amt 

							IF l_rec_offersale.checktype_ind = "1" THEN 
								LET l_avail_qty = l_rec_orderdetl.sold_qty 
							ELSE 
								LET l_avail_qty = l_rec_orderdetl.ext_price_amt 
							END IF 

							FOR i = 1 TO 3 
								LET l_query_text = 
									"SELECT rowid,* FROM t_offerprod ", 
									"WHERE type_ind = \"1\" ", 
								...
								CASE i 
									WHEN 1 
										...
										"AND prodgrp_code=\"",l_rec_orderdetl.prodgrp_code,"\" ", 
										"AND maingrp_code=\"",l_rec_orderdetl.maingrp_code,"\"" 
									WHEN 2 
										LET l_query_text = l_query_text CLIPPED," ", 
										"AND part_code IS NULL ", 
								...
								IF sqlca.sqlcode = 0 THEN 
									IF (l_rec_offerprod.allocated_qty + l_avail_qty) > l_rec_offerprod.reqd_qty THEN 
										UPDATE t_offerprod 
										SET allocated_qty = reqd_qty 
										WHERE rowid = l_rowid 
										
										LET l_avail_qty = l_avail_qty - l_rec_offerprod.reqd_qty + l_rec_offerprod.allocated_qty 
									ELSE 
									
										UPDATE t_offerprod 
										SET allocated_qty = allocated_qty	+ l_avail_qty 
										WHERE rowid = l_rowid 
										LET l_avail_qty = 0 
									END IF 
								END IF 
						...
						IF sqlca.sqlcode = 0 THEN 
							IF l_error_code = 0 THEN 
								...
								IF l_rec_offerprod.part_code IS NOT NULL THEN 
									...
									LET l_error_code = 7039 
									LET l_temp_text = l_rec_offerprod.part_code 
								ELSE 
									#7024 Insufficent amount of product combo
									LET l_error_code = 7024 
									LET l_temp_text = l_rec_offerprod.maingrp_code,":",	l_rec_offerprod.prodgrp_code 
								END IF 
							END IF 
						END IF 
					END IF 
				...
				IF l_rec_offersale.checkrule_ind = "4" OR l_rec_offersale.checkrule_ind = "5" THEN 
					...
					AND (autoinsert_flag = "N" OR autoinsert_flag IS null) 
					IF sqlca.sqlcode = 0 THEN 
						DECLARE c3_orderdetl CURSOR FOR 
						SELECT 
							part_code, 
							prodgrp_code, 
							maingrp_code, 
						...
						FOREACH c3_orderdetl INTO l_rec_orderdetl.part_code, 
							l_rec_orderdetl.prodgrp_code, 
							l_rec_orderdetl.maingrp_code, 
							l_rec_orderdetl.bonus_qty, 
							l_rec_orderdetl.ext_bonus_amt 

							IF l_rec_offersale.checktype_ind = "1" THEN 
								LET l_avail_qty = l_rec_orderdetl.bonus_qty 
							ELSE 
								LET l_avail_qty = l_rec_orderdetl.ext_bonus_amt 
							END IF 
		...
		IF NOT l_error_code THEN 
			...
			IF l_rec_offersale.prodline_disc_flag = "N" THEN 
				...
				UPDATE t_orderdetl 
				SET 
					disc_per = l_rec_offersale.disc_per, 
					job_code = true ##job_code =>disc_taken_ind(see note) 
				WHERE offer_code = l_rec_orderoffer.offer_code 
				AND disc_allow_flag = "Y" 
				AND job_code = false ##job_code =>disc_taken_ind(see note) 
			...
			ELSE 
				...
				DECLARE c4_orderdetl CURSOR FOR 
				SELECT 
					part_code, 
					prodgrp_code, 
					maingrp_code, 
				...
				FOREACH c4_orderdetl INTO l_rec_orderdetl.part_code, 
					l_rec_orderdetl.prodgrp_code, 
					l_rec_orderdetl.maingrp_code, 
					l_rec_orderdetl.sold_qty, 
					l_rec_orderdetl.ext_price_amt 
					...
					UPDATE t_proddisc 
					SET 
						allocated_qty = allocated_qty + l_rec_orderdetl.sold_qty, 
						allocated_amt = allocated_amt + l_rec_orderdetl.ext_price_amt, 
						poss_disc_amt = poss_disc_amt + ((disc_per/100)	* l_rec_orderdetl.ext_price_amt), 
						disc_taken_ind = false 
					WHERE type_ind = "2" 
					AND key_num = l_rec_orderoffer.offer_code 
					AND maingrp_code = l_rec_orderdetl.maingrp_code 
					AND (prodgrp_code IS NULL OR prodgrp_code = l_rec_orderdetl.prodgrp_code) 
					AND (part_code IS NULL OR part_code = l_rec_orderdetl.part_code) 
				END FOREACH 
				...
				WHILE true 
					...
					IF sqlca.sqlcode = 0 THEN 
						...
						IF l_rec_proddisc.prodgrp_code IS NOT NULL THEN 
							LET l_query_text = 
								l_query_text CLIPPED," ",	"AND prodgrp_code='",l_rec_proddisc.prodgrp_code,"'" 
						END IF 

						IF l_rec_proddisc.part_code IS NOT NULL THEN 
							LET l_query_text = l_query_text CLIPPED," ","AND part_code='",l_rec_proddisc.part_code,"'" 
						END IF 
						
						PREPARE s5_orderdetl FROM l_query_text 
						DECLARE c5_orderdetl CURSOR FOR s5_orderdetl
						 
						FOREACH c5_orderdetl INTO l_rowid, 
							l_rec_orderdetl.part_code, 
							l_rec_orderdetl.prodgrp_code, 
							l_rec_orderdetl.maingrp_code, 
							l_rec_orderdetl.sold_qty, 
		...
		PREPARE s_t_orderdetl FROM l_query_text 
		DECLARE c_t_orderdetl CURSOR FOR s_t_orderdetl 
		OPEN c_t_orderdetl USING l_rec_offersale.offer_code 
		...
		FOREACH c_t_orderdetl INTO l_rec_orderdetl.* 
			...
			IF l_rec_orderdetl.disc_allow_flag = "Y" THEN 
				IF l_disc_base_amt != 0 THEN 
					LET l_rec_orderdetl.bonus_disc_amt = 
						l_rec_orderdetl.list_price_amt 
		...
		SET 
			...
			msg_num = l_error_code, 
			msg_text = l_temp_text, 
			gross_amt = l_total_grs_amt, 
			net_amt = l_total_net_amt 
		WHERE offer_code = l_rec_offersale.offer_code 
	END IF 
	RETURN l_error_code,l_temp_text 
END FUNCTION 
###########################################################################
# END FUNCTION validate_offer(p_cmpy,p_type_ind,p_type_code) 
###########################################################################
...
FUNCTION cr_offer_tables() 
	...
	CREATE INDEX t_proddisc_1 ON t_proddisc(
		type_ind, 
		key_num) 
	CREATE INDEX t_orderdetl_1 ON t_orderdetl(
		offer_code, 
		status_ind) 
	CREATE INDEX t_orderdetl_2 ON t_orderdetl(
		part_code, 
		prodgrp_code, 
		maingrp_code) 
END FUNCTION 

...
# Purpose - Master Job Activity Edit Program
...
MAIN 
	...
	IF status = notfound THEN 
		...
		#J1401error " Must SET up JM Parameters first in JZP"
		SLEEP 5 
		EXIT program 
	END IF 
...
FUNCTION select_job() 
	...
	pr_activity.var_code 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J55","input-pr_activity-1") -- alch kd-506 
		...
		ON KEY (control-b) 
			CASE 
				WHEN infield (job_code) 
					LET pr_activity.job_code = showmjobs(glob_rec_kandoouser.cmpy_code,pr_user_scan_code) 
					SELECT title_text, cust_code 
				...
				WHEN infield (var_code) 
					LET pr_activity.var_code = show_jobvars(glob_rec_kandoouser.cmpy_code,pr_activity.job_code) 
		...
		AFTER FIELD job_code 
			...
			ELSE 
				...
				IF status = notfound THEN 
					...
					NEXT FIELD job_code 
				ELSE 
					SELECT name_text INTO pr_customer.name_text 
					FROM customer 
					WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
...
FUNCTION select_activity() 
	...
	WHILE true 
		...
		sort_text 
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","J55","const-activity_code-3") -- alch kd-506 
			ON ACTION "WEB-HELP" -- albo kd-373 
				CALL onlinehelp(getmoduleid(),null) 
		END CONSTRUCT 
		...
		END IF 
	END WHILE 
	# MESSAGE " RETURN TO Edit - F2 Toggle Delete"
	...
	INPUT ARRAY pa_activity WITHOUT DEFAULTS FROM sr_activity.* 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J55","input_arr-pa_activity-1") -- alch kd-506 
...
FUNCTION edit_activity() 
	...
	MENU "Activity Edit" 
		BEFORE MENU 
			CALL publish_toolbar("kandoo","J55","menu-act_edit-2") -- alch kd-506 
		COMMAND KEY ("D",f20) "Detail" " Edit non-financial activity details" 
			#Detail, Edit non-financial activity details
			CALL read_details() 
			IF int_flag OR quit_flag THEN 
				CALL read_data() RETURNING pr_rowid 
				CALL display_details() 
		...
		COMMAND "Financials" " Edit activity financials" 
			#Financials, Edit activity financials
		...
		COMMAND "Page" " Edit full page activity description" 
			#Page Description, Edit Full Page Activity Description
...
FUNCTION update_activity(pr_rowid) 
	...
	BEGIN WORK 
		LET err_message = " J55 - Updating Activity" 
		WHENEVER ERROR GOTO recovery 
		UPDATE activity 
		SET * = pr_activity.* 
		WHERE rowid = pr_rowid 
		LET err_message = " J55 - Deleting Activity Description" 
		...
		OPEN ins_j_curs 
		LET err_message = " J55 - Inserting activity description" 
		...
		END FOR 
		CLOSE ins_j_curs 
		LET err_message = " J55 - Updating Job" 
		WHENEVER ERROR GOTO recovery 
		UPDATE job SET ( est_start_date , 
		est_end_date) 
		= 
...
FUNCTION read_data() 
	...
	FOREACH c_act_desc INTO pr_act_desc.* 
		...
		IF cnt > 100 OR cnt < 1 THEN 
			LET err_message = "Program J55, act_desc count > 100 " 

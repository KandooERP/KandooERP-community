...
FUNCTION summup() 
	...
	IF tmp_tax_total IS NULL THEN 
		LET tmp_tax_total = 0 
	END IF 
	IF pr_invoicehead.freight_tax_amt IS NULL THEN 
		LET pr_invoicehead.freight_tax_amt = 0 
	END IF 
	IF pr_invoicehead.hand_tax_amt IS NULL THEN 
		LET pr_invoicehead.hand_tax_amt = 0 
	END IF 
	# The invoicehead.tax_amt IS the Tax on the goods_amt NOT
	# the sum of all tax's (goods, freight AND hand)
	...
	LET msgresp = kandoomsg("J",1017,"") 
	#1017 ' F5 TO view foreign currency details' attribute (yellow)
	...
	pr_invoicehead.com2_text WITHOUT DEFAULTS 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J31e","input-pr_invoicehead-1") -- alch kd-506 
		...
		ON KEY (f5) 
			...
			pr_invoicehead.freight_amt + 
			pr_invoicehead.freight_tax_amt 
			CALL view_currency2(pr_customer.cust_code, pr_invoicehead.conv_qty) 
		...
		AFTER FIELD hand_amt 
			...
			LET pr_invoicehead.total_amt = pr_invoicehead.total_amt + 
			pr_invoicehead.hand_amt 
			CALL find_tax(pr_invoicehead.tax_code, " ", " ", 0, 0, 
			pr_invoicehead.hand_amt , 1, "H", "", "") 
			RETURNING dummy, 
			pr_invoicehead.hand_tax_amt, 
			dummy, 
			dummy, 
		...
		AFTER FIELD freight_amt 
			...
			pr_invoicehead.freight_amt 
			CALL find_tax(pr_invoicehead.tax_code, " ", " ", 0, 0, 
			pr_invoicehead.freight_amt , 1, "F", "", "") 
			RETURNING dummy, 
			pr_invoicehead.freight_tax_amt, 
			dummy, 
			dummy, 
...
FUNCTION view_currency2(fv_cust, fv_xchange) 
	...
	DISPLAY fv_goods_amt, 
	fv_tax_amt, 
	fv_hand_amt, 
	fv_freight_amt, 
	fv_total_amt, 
	fv_disc_amt TO goods_amt, 
	tax_amt, 
	hand_amt, 
	freight_amt, 
	total_amt, 
	...
	LET msgresp = kandoomsg("J",7017,fv_currency) 
	#7017 Values shown in currency
	LET int_flag = false 
	LET quit_flag = false 
	DISPLAY BY NAME pr_invoicehead.goods_amt, 
	pr_invoicehead.tax_amt, 
	pr_invoicehead.hand_amt, 
	pr_invoicehead.freight_amt, 
	pr_invoicehead.total_amt, 

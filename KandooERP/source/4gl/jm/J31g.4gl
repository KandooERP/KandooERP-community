...
FUNCTION alloc(inv_idx) 
	...
	" AND jobledger.var_code = ", pa_inv_line[inv_idx].var_code, 
	" AND jobledger.activity_code = ", pa_inv_line[inv_idx].activity_code, 
	...
	PREPARE ewww FROM str 

	DECLARE jl_c CURSOR FOR ewww 
	FOREACH jl_c INTO pr_jobledger.*, 
		...
		IF (pr_resbill.apply_qty != pr_jobledger.trans_qty) 
		OR (pr_jobledger.trans_type_ind = "CT" 
		AND pr_resbill.apply_cos_amt != pr_jobledger.trans_amt) 
		OR (pr_job.bill_way_ind = "R" 
		AND (pr_resbill.apply_amt != pr_jobledger.charge_amt 
		OR pr_resbill.apply_cos_amt != pr_jobledger.trans_amt)) THEN 
			...
			IF pr_jobledger.allocation_ind != "R" THEN 
				LET pr_tempbill.apply_cos_amt = pr_jobledger.trans_amt - 
				pr_resbill.apply_cos_amt 
			ELSE 
			...
			IF pr_jobledger.allocation_ind != "C" THEN 
				...
				ELSE 
					LET pr_tempbill.apply_amt = pr_jobledger.charge_amt - 
					pr_resbill.apply_amt 
				END IF 
			ELSE 
			...
			ELSE 
				LET pr_tempbill.prev_apply_qty = pr_resbill.apply_qty 
			END IF 
			LET pr_tempbill.prev_apply_amt = pr_resbill.apply_amt 
			LET pr_tempbill.prev_apply_cos_amt = pr_resbill.apply_cos_amt 
	...
	CASE 
		...
		WHEN pa_inv_line[inv_idx].bill_way_ind = "F" 
			# Cost allocation flag processing
			CASE (pa_inv_line[inv_idx].cost_alloc_flag) 
				WHEN "1" 
					LET fv_cost = ((pa_inv_line[inv_idx].est_comp_per * pa_inv_line[ 
					inv_idx ].est_cost_amt / 100) - pa_inv_line[inv_idx] 
					.post_cost_amt ) 
				WHEN "2" 
					LET fv_cost = ((pa_inv_line[inv_idx].est_comp_per * pa_inv_line[ 
					inv_idx ].est_cost_amt / 100) - pa_inv_line[inv_idx] 
...
FUNCTION edit_alloc(inv_idx) 
	...
	cnt, 
	inv_idx SMALLINT, 
	pr_tran_num LIKE poaudit.tran_num 
	...
	IF pr_kandoooption_sn = "Y" THEN 
		OPEN WINDOW j154 with FORM "J154a" -- alch kd-747 
		CALL winDecoration_j("J154a") -- alch kd-747 
	ELSE 
	...
	this_bill_amt, 
	this_cos_amt 
	LET msgresp = kandoomsg("J",1430,0) 
	# MESSAGE " RETURN TO Edit Billing - F7 Toggle Transaction FOR Invoice"
	WHENEVER ERROR CONTINUE 
	OPTIONS INSERT KEY f36, 
	DELETE KEY f36 
	WHENEVER ERROR stop CALL set_count(bill_idx) 
	INPUT ARRAY pa_resbill WITHOUT DEFAULTS 
	FROM sr_tempbill.* 

		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J31g","input_arr-pa_resbill-1") -- alch kd-506 
		...
		BEFORE INSERT 
			LET msgresp = kandoomsg("J",9472,0) 
	...
	FOR idx = 1 TO bill_idx 
		...
		(pa_resbill[ idx ].trans_invoice_flag, pa_resbill[idx].apply_qty, 
		pa_resbill[idx].apply_amt , pa_resbill[idx].apply_cos_amt) 
		WHERE var_code = pa_inv_line[inv_idx].var_code 
		AND activity_code = pa_inv_line[inv_idx].activity_code 
...
FUNCTION evaluate_totals(idx) 
	DEFINE 
	pr_this_bill_amt LIKE resbill.apply_amt, 
	pr_this_cos_amt LIKE resbill.apply_cos_amt, 
	...
	pr_this_bill_amt, 
	pr_this_cos_amt TO this_bill_qty, 
	this_bill_amt, 
	this_cos_amt 
...
FUNCTION get_detail(pr_seq_num, inv_idx) 
	...
	pr_tempbill.apply_cos_amt WITHOUT DEFAULTS 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J31g","input-pr_tempbill-1") -- alch kd-506 
		...
		BEFORE FIELD desc_text 
			...
			IF pr_invoicehead.bill_issue_ind = "2" OR "4" THEN 
				LET msgresp = kandoomsg("J",1021,0) 
				# MESSAGE " Edit the Transaction Description FOR Invoice Printing"
			ELSE 
				LET msgresp = kandoomsg("J",1022,0) 
		...
		AFTER INPUT 
			...
			IF pr_tempbill.apply_qty = 0 
			AND (pr_tempbill.apply_amt != 0 
			OR pr_tempbill.apply_cos_amt != 0) THEN 
			...
			AND pr_tempbill.serial_flag = "Y" THEN 
				CALL serial_line_init(pr_tempbill.part_code,"") 
				SELECT count(*) 
				INTO pr_qty_cnt 
				FROM t_serialinfo 
				WHERE part_code = pr_tempbill.part_code 
				DISPLAY "" at 2,1 
				DISPLAY "count ", pr_qty_cnt at 2,1 
				SLEEP 3 
				LET pr_qty_cnt = serial_input(pr_tempbill.part_code, "","") 

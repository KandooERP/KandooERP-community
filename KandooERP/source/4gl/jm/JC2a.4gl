...
# \file
# \brief module - JC2a.4gl - FUNCTION header()
...
FUNCTION JC2_header() 
	...
	pr_credithead.tax_code WITHOUT DEFAULTS 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","JC2a","input-pr_creditdetl-1") -- alch kd-506 
		...
		AFTER FIELD cred_date 
			...
			pr_credithead.period_num 
			IF conv_flag AND save_conv = pr_credithead.conv_qty THEN 
				CALL get_conv_rate(
	...
	WHILE true 
		...
		END FOREACH 
		IF enter_seg = "Y" THEN 
			CALL acct_fill(glob_rec_kandoouser.cmpy_code, glob_rec_kandoouser.sign_on_code, "JC2", mask_code, 
		...
		AND pr_arparms.show_seg_flag = "Y" THEN 
			CALL acct_fill(glob_rec_kandoouser.cmpy_code, glob_rec_kandoouser.sign_on_code, "JC2", pr_credithead.acct_override_code, 
...
FUNCTION get_jm_info() 
	...
	LET save_activity = " " 
	LET save_var = -1 
	FOREACH line_curs INTO pr_creditdetl.* 
...
FUNCTION setup_creditdetl() 
	...
	LET ps_creditdetl[load_idx].* = pr_creditdetl.* 
END FUNCTION 

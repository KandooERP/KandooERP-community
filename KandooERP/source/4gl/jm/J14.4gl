...
# Purpose - Job Management Job Edit & Deletion



MAIN 
	#Initial UI Init
	CALL setModuleId("J14") -- albo 
	CALL ui_init(0) 
...
FUNCTION select_job() 
	...
	CONSTRUCT BY NAME where_text ON 
			...
			sale_code 
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","J14","const-job_code-1") -- alch kd-506 
	...
	FOREACH c_job INTO pr_job.* 
		...
		IF idx = 240 THEN 
			...
			# " First 240 Jobs Selected Only"
			EXIT FOREACH 
		END IF 
	END FOREACH 
	LET job_array_size = idx 
	IF job_array_size = 0 THEN 
		LET msgresp = kandoomsg("U",9131,idx) 
		# " No Jobs Selected - Try Again"
		RETURN true 
	END IF 
	LET msgresp = kandoomsg("U",1518,"") 
	#" RETURN TO Edit - F2 TO Toggle Delete - ESC Re-SELECT"
	...
	INPUT ARRAY pa_job WITHOUT DEFAULTS FROM sr_job.* 
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J14","input_arr-pr_job-1") -- alch kd-506 
		ON KEY (F2) 
			IF pa_job[idx].delete_flag IS NULL THEN 
				...
				IF cnt > 0 THEN 
					...
					## " Job Code has Activities - Cannot be Deleted"
				ELSE 
		...
		BEFORE ROW 
			...
			ELSE 
				...
				LET pr_job.sale_code = pa_job[idx].sale_code 
				DISPLAY pa_job[idx].* 
				TO sr_job[scrn].* 
	...
	ELSE 
		IF job_delete_cnt > 0 THEN 
			LET msgresp = kandoomsg("J",8025,job_delete_cnt) 
			### " About TO Delete ",job_delete_cnt," Job/s ?(y/n)"
			IF msgresp = "Y" THEN 
				GOTO bypass 
				LABEL recovery: 
				...
				BEGIN WORK 
					LET err_message = " J14 - Deleting Jobs" 
...
FUNCTION edit_job_details() 
	...
	MENU "Job Edit" 
		BEFORE MENU 
			CALL publish_toolbar("kandoo","J14","menu-job_edit-1") -- alch kd-506 
		ON ACTION "WEB-HELP" -- albo kd-373 
			CALL onlinehelp(getmoduleid(),null) 
		COMMAND "Details" "Amend Job Details" 
			WHILE true 
				LET msgresp = kandoomsg("U",1020,"") 
				...
				job.cust_code, 
				job.sale_code 
				# Add customer lookup IF customer maintenance IS allowed

					BEFORE INPUT 
						CALL publish_toolbar("kandoo","J14","input-pr_job-1") -- alch kd-506 
					...
					BEFORE FIELD title_text 
						...
						### " Amend Job Title - F10 FOR Page Description"
					...
					AFTER FIELD cust_code 
						...
						IF status = notfound THEN 
							...
							#### " No such Client Code - try help window "
							NEXT FIELD job.cust_code 
						ELSE 
							IF pr_customer.currency_code != pr_base_currency THEN 
								LET msgresp = kandoomsg("J",9598,"") 
								NEXT FIELD job.cust_code 
							END IF 
...
FUNCTION update_job(pr_rowid) 
	...
	BEGIN WORK 
		...
		LET err_message = " J14 - Inserting job description" 
...
FUNCTION report_code() 
	...
	pr_jobtype.prompt8_ind != 5 THEN 
		...
		pr_job.report8_text WITHOUT DEFAULTS 
			...
			BEFORE INPUT 
				CALL publish_toolbar("kandoo","J14","input-pr_job-2") -- alch kd-506 

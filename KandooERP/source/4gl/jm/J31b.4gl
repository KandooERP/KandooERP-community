...
DEFINE 
pa_activity ARRAY [500] OF RECORD 
	invoice_flag CHAR(1), 
...
FUNCTION select_lines() 
	...
	WHILE true 
		...
		#1001 " Enter Activity Selection Criteria - ESC TO continue"
		CONSTRUCT BY NAME where_text ON 
		activity.var_code, 
		activity.activity_code 
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","J31b","const-activity_var_code-1") -- alch kd-506 
		...
		LET arr_size = 0 
		LET idx = 1 
		FOREACH c_1 INTO pa_inv_line[idx].activity_code, 
			...
			END IF 
			LET pa_pcs[1].act_cost_amt = pa_pcs[1].act_cost_amt + pa_inv_line[idx] 
			.act_cost_amt 
			LET pa_pcs[1].act_bill_amt = pa_pcs[1].act_bill_amt + pa_inv_line[idx] 
			.act_bill_amt 
			LET pa_pcs[1].est_bill_amt = pa_pcs[1].est_bill_amt + pa_inv_line[idx] 
			.est_bill_amt 
			LET pa_pcs[1].post_cost_amt = pa_pcs[1].post_cost_amt + 
			pa_inv_line[idx] .post_cost_amt 
			IF pa_inv_line[idx].bill_way_ind = "C" 
			AND get_markup THEN 
				...
				INPUT BY NAME pr_job.markup_per WITHOUT DEFAULTS 
					BEFORE INPUT 
						CALL publish_toolbar("kandoo","J31b","input-pr_job-1") -- alch kd-506 
					ON ACTION "WEB-HELP" -- albo kd-373 
						CALL onlinehelp(getmoduleid(),null) 
				END INPUT 
				CLOSE WINDOW j130 
			...
			IF pr_kandoooption_sn = "Y" THEN 
				...
				AND var_code = pa_inv_line[idx].var_code 
				AND activity_code = pa_inv_line[idx].activity_code 
				AND trans_type_ind = "PU" 

				IF pr_pu_count > 0 THEN 
					LET pa_inv_line[idx].title_text[28,29] = "PU" 
				ELSE 
					LET pa_inv_line[idx].title_text[28,29] = " " 
				END IF 
			END IF 
			...
			IF idx = 500 THEN 
				...
				#9100 " Only first 500 activities selected"
				EXIT FOREACH 
			ELSE 
				LET idx = idx + 1 
			END IF 
		END FOREACH 
		IF arr_size = 0 THEN 
			LET msgresp = kandoomsg("U", 9101, idx) 
			#9101" No Activities Selected - DEL TO Re-SELECT"
		END IF 
		IF pa_pcs[1].est_bill_amt = 0 THEN 
			LET pa_pcs[1].bill_pc = 0 
		ELSE 
			LET pa_pcs[1].bill_pc = pa_pcs[1].act_bill_amt / pa_pcs[1].est_bill_amt 
			* 100 
		END IF 
		IF pa_pcs[1].act_bill_amt = 0 THEN 
			LET pa_pcs[1].act_p_l_pc = 0 
		ELSE 
			LET pa_pcs[1].act_p_l_amt = pa_pcs[1].act_bill_amt - pa_pcs[1] 
			.post_cost_amt 
			LET pa_pcs[1].act_p_l_pc = pa_pcs[1].act_p_l_amt / pa_pcs[1] 
			.act_bill_amt * 100 
		END IF 
		IF pa_pcs[1].act_cost_amt = 0 THEN 
			LET pa_pcs[1].cost_pc = 0 
		ELSE 
			LET pa_pcs[1].cost_pc = pa_pcs[1].act_bill_amt / pa_pcs[1].act_cost_amt 
			* 100 
		END IF 
...
FUNCTION disp_lineitems() 
	...
	LET msgresp = kandoomsg("J", 1015, "") 
	#1015" ESC Invoice - RETURN Edit Line -",
	#" F7 Invoice Line Toggle - F10 Information"
	# F5 Foreign Currency' - CTRL-N Notes
	OPTIONS DELETE KEY f36 
	OPTIONS INSERT KEY f36 
	CALL set_count(arr_size) 
	INPUT ARRAY pa_activity WITHOUT DEFAULTS 
	FROM sr_activity.* 

		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J31b","input_arr-pa_activity-1") -- alch kd-506 
		...
		BEFORE ROW 
			...
			IF pa_inv_line[idx].activity_code IS NULL THEN 
				LET msgresp = kandoomsg("J", 9627, "") 
				#9627 " No further Invoice lines Available "
		...
		ON ACTION "NOTES"  --	ON KEY (control-n) 
			IF pa_inv_line[idx].activity_code IS NULL THEN 
				LET msgresp = kandoomsg("J", 9624, "") 
				#9624 "No notes available FOR an empty line!"
			ELSE 
				CALL scan_notes(idx) 
			END IF 
		BEFORE FIELD title_text 
			IF edit_alloc(idx) THEN 
				...
				LET pa_activity[idx].this_cos_amt = pa_inv_line[idx].this_cos_amt 
			ELSE 
				LET pa_inv_line[idx].this_bill_qty = pa_activity[idx].this_bill_qty 
				LET pa_inv_line[idx].this_bill_amt = pa_activity[idx].this_bill_amt 
				LET pa_inv_line[idx].this_cos_amt = pa_activity[idx].this_cos_amt 
			END IF 
			DISPLAY pa_activity[idx].* TO sr_activity[scrn].* 
			...
			NEXT FIELD invoice_flag 
		AFTER ROW 
			LET pa_inv_line[idx].invoice_flag = pa_activity[idx].invoice_flag 
			DISPLAY pa_activity[idx].* TO sr_activity[scrn].* 
		...
		AFTER INPUT 
			...
			OR quit_flag) THEN 
				...
				IF pv_corp_cust THEN 
					...
					bal_amt, 
					onorder_amt 
					INTO pr_corp_cust.cred_limit_amt, 
					pr_corp_cust.bal_amt, 
					pr_corp_cust.onorder_amt 
					FROM customer 
					WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
					AND cust_code = pr_corp_cust.cust_code 
				...
				ELSE 
					...
					bal_amt, 
					onorder_amt 
					INTO pr_customer.cred_limit_amt, 
					pr_customer.bal_amt, 
					pr_customer.onorder_amt 
					FROM customer 
					WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
					AND cust_code = pr_customer.cust_code 
					IF pr_customer.onorder_amt IS NULL THEN 
						LET pr_customer.onorder_amt = 0 
					END IF 
				...
				FOR x = 1 TO arr_size 
					...
					IF NOT pv_corp_cust THEN 
						IF chk_bal_amt > pr_customer.cred_limit_amt THEN 
							LET msgresp = kandoomsg("J", 9625, "") 
							#9625 "Customer will exceed Credit with above invoicing"
							NEXT FIELD invoice_flag 
						END IF 
					...
					ELSE 
						IF chk_bal_amt > pr_corp_cust.cred_limit_amt THEN 
							LET msgresp = kandoomsg("J", 9625, "") 
							#9625 "Corporate customer will exceed Credit Limit with",
							##     " above billings"
							NEXT FIELD invoice_flag 
						END IF 
	...
	FOR idx = 1 TO arr_size 
		IF pa_inv_line[idx].invoice_flag IS NOT NULL THEN 
			...
			IF pa_inv_line[idx].this_bill_qty IS NOT NULL THEN 
				LET pa_invoicedetl[idx].ship_qty = pa_inv_line[idx].this_bill_qty 
			ELSE 
			...
			LET pa_invoicedetl[idx].ext_cost_amt = pa_inv_line[idx].this_cos_amt 
			LET pa_invoicedetl[idx].disc_amt = 0 
			LET pa_invoicedetl[idx].unit_sale_amt = NULL 
			LET pa_invoicedetl[idx].ext_sale_amt = pa_inv_line[idx].this_bill_amt 
			LET pa_invoicedetl[idx].unit_tax_amt = 0 
			LET pa_invoicedetl[idx].ext_tax_amt = 0 
			LET pa_invoicedetl[idx].line_total_amt = pa_inv_line[idx].this_bill_amt 
			...
			LET pa_invoicedetl[idx].comp_per = pa_inv_line[idx].est_comp_per 
			LET pr_invoicehead.goods_amt = pr_invoicehead.goods_amt + pa_inv_line[idx] 
			.this_bill_amt 
			LET pr_invoicehead.cost_amt = pr_invoicehead.cost_amt + pa_inv_line[idx] 
			.this_cos_amt 
		END IF 
...
FUNCTION disp_line_detail(idx, scrn) 
	...
	LET pr_tot_bill_amt = pa_inv_line[idx].this_bill_amt + pa_inv_line[idx] 
	.act_bill_amt 
	LET pr_tot_cos_amt = pa_inv_line[idx].this_cos_amt + pa_inv_line[idx] 
	.post_cost_amt 
	IF pr_job.bill_way_ind = "R" THEN 
		LET pr_tot_bill_qty = pa_inv_line[idx].this_bill_qty 
	ELSE 
		LET pr_tot_bill_qty = pa_inv_line[idx].this_bill_qty + pa_inv_line[idx] 
		.act_bill_qty 
	END IF 
	DISPLAY BY NAME pa_inv_line[idx].activity_code, 
	pa_inv_line[idx].var_code, 
	pa_inv_line[idx].est_comp_per, 
	pa_inv_line[idx].est_cost_amt, 
	pa_inv_line[idx].act_cost_amt, 
	...
	pr_tot_cos_amt, 
	pr_tot_bill_qty TO tot_bill_amt, 
	tot_cos_amt, 
	tot_bill_qty 
...
FUNCTION display_info(inv_idx) 
	...
	MENU " Information" 
		BEFORE MENU 
			CALL publish_toolbar("kandoo","J31b","menu-info-1") -- alch kd-506 
		ON ACTION "WEB-HELP" -- albo kd-373 
			CALL onlinehelp(getmoduleid(),null) 
		...
		COMMAND "Activity Detail " " DISPLAY Details of this Activity" 
			IF pa_inv_line[inv_idx].activity_code IS NOT NULL THEN 
				LET runner = " job.job_code = '", 
				pr_job.job_code clipped, 
...
FUNCTION calc_pcs(inv_idx) 
	...
	FOR cnt = 1 TO arr_size 
		...
		ELSE 
			LET pa_pcs[2].act_bill_amt = pa_pcs[2].act_bill_amt + pa_inv_line[cnt] 
			.this_bill_amt 
		END IF 
		IF pa_inv_line[cnt].this_cos_amt IS NULL THEN 
			LET pa_inv_line[cnt].this_cos_amt = 0 
		ELSE 
			LET pa_pcs[2].post_cost_amt = pa_pcs[2].post_cost_amt + pa_inv_line[cnt 
			] .this_cos_amt 
		END IF 
		# recalculate new total billed including this invoice
		LET pa_pcs[3].act_bill_amt = pa_pcs[1].act_bill_amt + pa_pcs[2] 
		.act_bill_amt 
		# recalculate new total cost of sales including this invoice
		LET pa_pcs[3].post_cost_amt = pa_pcs[1].post_cost_amt + pa_pcs[2] 
		.post_cost_amt 
	END FOR 
	# recalculate percentages
	# Profit AND Profit %
	# profit = billed_amt - cos_amt,
	LET pa_pcs[3].act_p_l_amt = pa_pcs[3].act_bill_amt - pa_pcs[3].post_cost_amt 
	IF pa_pcs[3].est_bill_amt = 0 THEN 
		LET pa_pcs[3].bill_pc = 0 
	ELSE 
		LET pa_pcs[3].bill_pc = pa_pcs[3].act_bill_amt / pa_pcs[3].est_bill_amt * 
		100 
	END IF 
	...
	ELSE 
		...
		LET pa_pcs[3].act_p_l_pc = pa_pcs[3].act_p_l_amt / pa_pcs[3].act_bill_amt 
		* 100 
	END IF 
	IF pa_pcs[3].act_cost_amt = 0 THEN 
		LET pa_pcs[3].cost_pc = 0 
	ELSE 
		LET pa_pcs[3].cost_pc = pa_pcs[3].act_bill_amt / pa_pcs[3].act_cost_amt * 
		100 
	END IF 
	DISPLAY pa_pcs[1].act_bill_amt, 
	...
	pa_pcs[1].act_p_l_pc, 
	pa_pcs[3].act_p_l_pc TO td_act_bill_amt, 
	this_act_bill_amt, 
	fcst_act_bill_amt, 
	td_bdgt_bill_amt, 
	fcst_bdgt_bill_amt, 
	...
	# " Any key TO continue" FOR CHAR ans
	LET int_flag = false 
	LET quit_flag = false 
	CLOSE WINDOW j129 
END FUNCTION 
...
FUNCTION scan_notes(inv_idx) 
	...
	#1003 " RETURN TO Edit - F1 TO Add - ESC TO Continue"
	INPUT ARRAY pa_notes WITHOUT DEFAULTS 
	FROM sr_notes.* 

		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J31b","input_arr-pa_notes-1") -- alch kd-506 
		...
		BEFORE ROW 
			...
			AND idx != 1 THEN 
				...
				### " There are no more rows in this direction"
				LET last_note = true 
			ELSE 
				LET last_note = false 
		...
		BEFORE INSERT 
			...
			OR NOT last_note THEN 
				...
				IF pa_notes[idx].note_code IS NOT NULL THEN 
					LET pa_notes[idx].var_code = pa_inv_line[inv_idx].var_code 
					LET pa_notes[idx].activity_code = pa_inv_line[inv_idx].activity_code 
					LET pa_notes[idx].title_text = pa_inv_line[inv_idx].title_text 
					DISPLAY pa_notes[idx].* TO sr_notes[scrn].* 
...
FUNCTION taxing(fr_inv_line, ln_num) 
	...
	IF fr_inv_line.bill_way_ind != "F" THEN 
		...
		FOREACH iss_cur 
			...
			END IF 
			CALL find_tax(pr_invoicehead.tax_code, pr_tmpbill.desc_text[1, 15], 
			pr_tmpbill.desc_text [16, 18], tot_lines, tax_ln_num, 
			unit_price, 
		...
		FOREACH res_cur 
			...
			ELSE 
				LET unit_price = pr_tmpbill.apply_amt 
			END IF 
			CALL find_tax(pr_invoicehead.tax_code, pr_tmpbill.trans_source_text, 
			" " , # ware_code NOT required 
			tot_lines, tax_ln_num, 
			unit_price, 
			pr_tmpbill.apply_qty, 
			"S", 
			"", "") 
	...
	ELSE 
		...
		IF fr_inv_line.this_bill_qty != 0 THEN 
			LET unit_price = fr_inv_line.this_bill_amt / fr_inv_line.this_bill_qty 
		ELSE 
			LET unit_price = fr_inv_line.this_bill_amt 
			LET fr_inv_line.this_bill_qty = 1 
		END IF 
		IF pr_tax.calc_method_flag = "T" 
		OR "N" THEN 
			CALL find_tax(pr_invoicehead.tax_code, " ", " ", arr_size, ln_num, 
			unit_price , fr_inv_line.this_bill_qty, "S", "", "") 
			RETURNING tmp_ext_price_amt, 
			tmp_unit_tax_amt, 
			tmp_ext_tax_amt, 
			tmp_line_tot_amt, 
...
FUNCTION view_currency(fv_idx, fv_cust, fv_xchange) 
	...
	fa_fc_activity ARRAY [500] OF RECORD 
		invoice_flag CHAR(001), 
		title_text LIKE activity.title_text, 
		this_bill_amt DECIMAL(010, 002), 
	...
	# - First get customer FC code
	SELECT customer.currency_code INTO fv_currency 
	FROM customer 
	...
	FOR fv_loop = 001 TO 500 
		IF pa_activity[fv_loop].title_text IS NULL THEN 
			EXIT FOR 
		END IF 
		LET fa_fc_activity[fv_loop].* = pa_activity[fv_loop].* 
		LET fa_fc_activity[fv_loop].this_bill_amt = (pa_activity[fv_loop] 
		.this_bill_amt * fv_xchange) 
		LET fa_fc_activity[fv_loop].this_cos_amt = (pa_activity[fv_loop] 
		.this_cos_amt * fv_xchange) 
	END FOR 
	...
	LET fv_post_revenue_amt = (pa_inv_line[fv_idx].post_revenue_amt * fv_xchange) 
	LET fv_tot_bill_amt = (pa_inv_line[fv_idx].this_bill_amt + pa_inv_line[fv_idx 
	] .act_bill_amt) * fv_xchange 
	LET fv_tot_cos_amt = (pa_inv_line[fv_idx].this_cos_amt + pa_inv_line[fv_idx] 
	.post_cost_amt ) * fv_xchange 
	# - Now DISPLAY these FC VALUES TO the form.
	LET msgresp = kandoomsg("J", 1016, fv_currency) 
	#1016 'Foreign Currency View in ', fv_currency CLIPPED,
	#CANCEL TO EXIT.
	DISPLAY fv_est_cost_amt, 
	fv_act_cost_amt, 
	fv_est_bill_amt, 
	fv_act_bill_amt, 
	fv_post_cost_amt, 
	fv_post_revenue_amt, 
	fv_tot_bill_amt, 
	fv_tot_cos_amt TO est_cost_amt, 
	act_cost_amt, 
	est_bill_amt, 
	act_bill_amt, 
	post_cost_amt, 

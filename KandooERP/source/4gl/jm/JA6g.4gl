...
FUNCTION alloc(inv_idx) 
	...
	PREPARE reter FROM str 
	DECLARE jl_c CURSOR FOR reter 
	FOREACH jl_c INTO pr_jobledger.*, pr_resbill.apply_qty, 
		...
		(pr_jobledger.trans_type_ind = "CT" AND 
		pr_resbill.apply_cos_amt != pr_jobledger.trans_amt) OR 
		...
		pr_resbill.apply_cos_amt != pr_jobledger.trans_amt))then 
			...
			LET pr_tempbill.trans_source_num = 
			...
			LET pr_tempbill.trans_source_text = 
			...
			IF pr_jobledger.desc_text IS NULL OR 
			pr_jobledger.desc_text = " " THEN 
				SELECT desc_text 
			...
			LET pr_tempbill.prev_apply_cos_amt = 
			pr_resbill.apply_cos_amt 
...
FUNCTION edit_alloc(inv_idx) 
	...
	INPUT ARRAY pa_resbill WITHOUT DEFAULTS FROM sr_tempbill.* 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","JA6g","input_arr-pa_resbill-1") -- alch kd-506 
...
FUNCTION get_detail(pr_seq_num, inv_idx) 
	...
	WITHOUT DEFAULTS 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","JA6g","input-pr_tempbill-1") -- alch kd-506 
	...
	pr_tempbill.prev_apply_cos_amt + pr_tempbill.apply_cos_amt 

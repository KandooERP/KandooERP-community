...
GLOBALS "../jm/J75_GLOBALS.4gl"

# Purpose - Allocate resources TO activities
MAIN 
	DEFER quit 
	DEFER interrupt 
	
	CALL setModuleId("J75") 
	...
	IF status = notfound THEN 
		...
		EXIT program 
	END IF 
	OPEN WINDOW j134 with FORM "J134" -- alch kd-747 
	CALL winDecoration_j("J134") -- alch kd-747 
	WHILE get_resource() 
...
FUNCTION get_resource() 
	...
	period_num 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J75","input-pr_jmresource-1") -- alch kd-506 
...
FUNCTION alloc_details() 
	...
	INPUT ARRAY pa_res_alloc WITHOUT DEFAULTS FROM sr_res_alloc.* 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J75","input_arr-pa_res_alloc-1") -- alch kd-506 
		...
		AFTER INPUT 
			IF not(int_flag OR quit_flag) THEN 
				...
				ELSE 
					IF pa_res_alloc[1].job_code IS NOT NULL THEN 
						LET msgresp = kandoomsg("J",8023,"") 
						#8023 Do you wish TO save Resource Allocation details? (Y/N)
						IF msgresp = "N" THEN 
							NEXT FIELD scroll_flag 
						END IF 
					END IF 
			...
			ELSE 
				LET pr_first_time = false 
				IF infield(scroll_flag) THEN 
					LET msgresp = kandoomsg("J",8024,"") 
					#8024 Do you wish TO abandon resources allocated? (Y/N)
					IF msgresp = "N" THEN 
						LET int_flag = false 
						LET quit_flag = false 
						INITIALIZE pf_res_alloc.* TO NULL 
...
FUNCTION get_comment(pr_comment) 
	...
	INPUT BY NAME pr_jobledger.desc_text WITHOUT DEFAULTS 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","J75","input-pr_jobledger-1") -- alch kd-506 
...
FUNCTION upd_jobledg() 
	...
	BEGIN WORK 
		...
		FOR idx = 1 TO arr_count() 
			IF pa_res_alloc[idx].trans_amt IS NOT NULL THEN 
				LET err_message = "J75 SELECT FROM activity" 
				...
				OR pr_activity.unit_code = pa_unit_code[idx] THEN 
					...
					pa_res_alloc[idx].trans_qty 
				END IF 
				LET err_message = "J75 Updating Activity" 
				# Set start date
	...
	#OPEN WINDOW J126 AT 10,10 WITH FORM "J126"
	#ATTRIBUTE(border)
	#DISPLAY BY NAME pr_jmparms.ra_num
	#
	LET msgresp = kandoomsg("J",1554,pr_jmparms.ra_num) 
	#1554 Successful Addition. Enter another Resource Allocation?
	IF msgresp = "N" THEN 
		EXIT program 
	END IF 
	#CLOSE WINDOW J126

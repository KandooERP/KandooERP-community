...
# Purpose - JM credit entry - cull FROM A41d.4gl
...
FUNCTION get_acct() 
	...
	IF pr_credithead.tax_code IS NOT NULL THEN 
		...
		INPUT BY NAME pr_coa.acct_code WITHOUT DEFAULTS ATTRIBUTE(UNBUFFERED) 

			BEFORE INPUT 
				CALL publish_toolbar("kandoo","JC1d","input-pr_coa-1") -- alch kd-506 
	...
	ELSE 
		...
		pr_creditdetl.tax_code WITHOUT DEFAULTS 
			...
			BEFORE INPUT 
				CALL publish_toolbar("kandoo","JC1d","input-pr_coa-2") -- alch kd-506 

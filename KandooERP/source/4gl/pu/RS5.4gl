...
# MAIN
#
# also Jetform P.O. PRINT
# \brief module RS1 - Enter selection criteria
#             - Determines which REPORT TO CALL using the purchase type
#               on the purchase ORDER
#######################################################################
MAIN 

	CALL setModuleId("RS5") -- albo 
	...
	IF status = notfound THEN 
		LET msgresp = kandoomsg("A",5003,"") 
		#5003 Company has NOT been SET up; Refer System Administrator
		EXIT program 
	END IF 
	...
	MENU " PO Print" 
		...
		BEFORE MENU 
			CALL publish_toolbar("kandoo","RS5","menu-po_print-1") 
...
FUNCTION select_po() 
	...
	purchhead.printed_flag 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","RS5","construct-purchhead-1") 
...
FUNCTION po_allocate(where_text) 
	...
	--   OPEN WINDOW w1_RS5 AT 12,15 with 1 rows, 40 columns  -- albo  KD-756
	--      ATTRIBUTE(border)
	...
	IF err_cnt > 0 THEN 
		LET msgresp = kandoomsg('R',7001,'')	#7001 Unable TO allocate FORMAT FOR some Purchase Orders - refer to ", trim(get_settings_logFile())
	END IF 
END FUNCTION
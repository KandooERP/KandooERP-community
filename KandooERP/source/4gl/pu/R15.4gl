...
# Purpose - Purchase Order line item Edit
{
KandooERP runs on Querix Lycia www.querix.com
Adapted by eric@begooden.it,hoelzl@querix.com
}
# \file
# \brief module IS passed a PO number OR offers scan ARRAY FOR PO selection
...
MAIN 
	...
	IF num_args() = 1 THEN 
		IF po_mod(glob_rec_kandoouser.cmpy_code, glob_rec_kandoouser.sign_on_code, arg_val(1),"EDIT") THEN 
		END IF 
	ELSE 
...
FUNCTION scan_orders() 
	...
	pa_purchhead array[400] OF RECORD 
		scroll_flag CHAR(1), 
	...
	order_text 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","R15","construct-purchhead-1") 
	...
	PREPARE s_purchhd FROM query_text 
	DECLARE c_purchhd CURSOR FOR s_purchhd 
	LET idx = 0 
	FOREACH c_purchhd INTO pr_purchhead.* 
		LET idx = idx + 1 
	...
	INPUT ARRAY pa_purchhead WITHOUT DEFAULTS FROM sr_purchhead.* ATTRIBUTE(UNBUFFERED) 
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","R15","inp-arr-purchhead-1") 
		...
		BEFORE FIELD vend_code 
			IF pa_purchhead[idx].order_num IS NOT NULL THEN 
				...
				pr_voucher_tot, 
				pr_tax_tot 
				IF pr_received_tot > 0 THEN 
					UPDATE purchhead 
					SET status_ind = "P" 
					WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
					AND order_num = pa_purchhead[idx].order_num 
					LET pa_purchhead[idx].status_ind = "P" 
				END IF 
				OPTIONS INSERT KEY f36, 
				DELETE KEY f36 
				DISPLAY pa_purchhead[idx].* TO sr_purchhead[scrn].* 

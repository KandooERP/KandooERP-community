...
GLOBALS "../pu/RB5_GLOBALS.4gl" 

# \brief module RB5 Commitments Report by Number
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode 
			...
			MENU " Outstanding ORDER BY Number" 
				...
				BEFORE MENU 
					CALL publish_toolbar("kandoo","RB5","menu-outst_order-1") 
					CALL RB5_rpt_process(RB5_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL RB5_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW R124 with FORM "R124" 
			CALL  windecoration_r("R124") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(RB5_rpt_query()) #save where clause in env 
			CLOSE WINDOW R124 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL RB5_rpt_process(get_url_sel_text())
	END CASE 
END FUNCTION 


FUNCTION RB5_rpt_query() 
	...
	purchdetl.acct_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","RB5","construct-purchdetl-1") 
...
FUNCTION RB5_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"RB5_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT RB5_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("RB5_rpt_list")].sel_text clipped," ",
	...
	WHILE true 
		...
		#---------------------------------------------------------
		OUTPUT TO REPORT RB5_rpt_list(l_rpt_idx,
	...
	FINISH REPORT RB5_rpt_list 
	--   CLOSE WINDOW wfPU  -- albo  KD-756

	#------------------------------------------------------------
	FINISH REPORT RB5_rpt_list
	CALL rpt_finish("RB5_rpt_list")
	#------------------------------------------------------------
...
REPORT RB5_rpt_list(pr_purchdetl,pr_curr_code,pr_order_date, 	pr_conv_qty) 

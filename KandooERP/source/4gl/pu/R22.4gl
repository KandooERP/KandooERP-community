...
# \file
# \brief module R22 allows the user TO search FOR orders on selected info
...
MAIN 
	...
	IF num_args() = 1 THEN 
		...
		CALL pohdwind(glob_rec_kandoouser.cmpy_code, pr_order_num) 
	ELSE 
...
FUNCTION scan_purchase_orders() 
	...
	order_text 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","R22","construct-purchhead-1") 
	...
	#1504 F3/F4 TO Page Fwd/Bwd - RETURN on line TO Edit
	INPUT ARRAY pa_purchhead WITHOUT DEFAULTS FROM sr_purchhead.* ATTRIBUTE(UNBUFFERED) 

		BEFORE INPUT 
			CALL publish_toolbar("kandoo","R22","inp-arr-purchhead-1") 

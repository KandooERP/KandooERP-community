...
GLOBALS "../pu/RB4_GLOBALS.4gl" 

DEFINE mdou_start_flg INTEGER 

#RB4 Orders Report by Product
FUNCTION RB4_main() 
	DEFER quit 
	DEFER interrupt
	
	CALL setModuleId("RB4")
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode 
			...
			MENU " Outstanding ORDER BY Product" 
				...
				BEFORE MENU 
					CALL publish_toolbar("kandoo","RB4","menu-outst_order-1") 
					CALL RB4_rpt_process(RB4_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL RB4_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW R124 with FORM "R124" 
			CALL  windecoration_r("R124") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(RB4_rpt_query()) #save where clause in env 
			CLOSE WINDOW R124 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL RB4_rpt_process(get_url_sel_text())
	END CASE 
END FUNCTION 


FUNCTION RB4_rpt_query() 
	...
	purchdetl.acct_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","RB4","construct-purchdetl-1") 
...
FUNCTION RB4_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"RB4_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT RB4_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("RB4_rpt_list")].sel_text clipped," ",  
	...
	WHILE true 
		...
		END IF 
		#---------------------------------------------------------
		OUTPUT TO REPORT RB4_rpt_list(l_rpt_idx,
	...
	#------------------------------------------------------------
	FINISH REPORT RB4_rpt_list
	CALL rpt_finish("RB4_rpt_list")
	#------------------------------------------------------------
...
REPORT RB4_rpt_list(p_rpt_idx,pr_purchdetl,pr_curr_code,pr_order_date, pr_conv_qty) 

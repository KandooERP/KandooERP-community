...
# FUNCTION RA3_main()
#
# RA3 Purchase Order Listing By Due date
###########################################################################
FUNCTION RA3_main() 
	DEFER quit 
	DEFER interrupt 

	CALL setModuleId("RA3") -- albo 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode 
			...
			MENU " Purchase Orders (by Due date)" 
				BEFORE MENU 
					CALL publish_toolbar("kandoo","RA3","menu-purchase_orders-1") 
					CALL RA3_rpt_process(RA3_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL RA3_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW R107 with FORM "R107" 
			CALL  windecoration_r("R107") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(RA3_rpt_query()) #save where clause in env 
			CLOSE WINDOW R107 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL RA3_rpt_process(get_url_sel_text())
	END CASE
END FUNCTION 
###########################################################################
# END FUNCTION RA3_main()
###########################################################################


###########################################################################
# FUNCTION RA3_rpt_query()
#
#
###########################################################################
FUNCTION RA3_rpt_query() 
	...
	entry_date 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","RA3","construct-purchhead-1") 
...
END FUNCTION 
###########################################################################
# END FUNCTION RA3_rpt_query()
###########################################################################


###########################################################################
# FUNCTION RA3_rpt_process(p_where_text)
#
#
###########################################################################
FUNCTION RA3_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"RA3_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT RA3_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	"AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("RA3_rpt_list")].sel_text clipped," ", 
	"ORDER BY due_date, vend_code, order_num"
	...
	FOREACH c_purchhead INTO pr_purchhead.* 
		...
		#---------------------------------------------------------
		OUTPUT TO REPORT RA3_rpt_list(l_rpt_idx,pr_purchhead.*,pr_total_rec.*) 
		IF NOT rpt_int_flag_handler2("Due Date:",pr_purchhead.due_date,NULL,l_rpt_idx) THEN
			EXIT FOREACH 
		END IF 
		#---------------------------------------------------------
	...
	#------------------------------------------------------------
	FINISH REPORT RA3_rpt_list
	CALL rpt_finish("RA3_rpt_list")
	#------------------------------------------------------------
...
END FUNCTION 
###########################################################################
# END FUNCTION RA3_rpt_process(p_where_text)
###########################################################################


###########################################################################
# REPORT RA3_rpt_list(p_rpt_idx,pr_purchhead,pr_total_rec)
#
#
###########################################################################
REPORT RA3_rpt_list(p_rpt_idx,pr_purchhead,pr_total_rec) 
	...
	OUTPUT 
--	left margin 0 
	ORDER external BY pr_purchhead.due_date 
	...
	FORMAT 
		...
		AFTER GROUP OF pr_purchhead.due_date 
			SKIP 1 line 
...
END REPORT
###########################################################################
# END REPORT RA3_rpt_list(p_rpt_idx,pr_purchhead,pr_total_rec)
########################################################################### 
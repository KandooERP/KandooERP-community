...
# FUNCTION RA1_main() 
#
# RA1 Purchase Order Listing By Vendor
###########################################################################
FUNCTION RA1_main() 
	DEFINE l_run_arg STRING #launch argument for URS report print manager

	DEFER quit 
	DEFER interrupt
	
	CALL setModuleId("RA1") -- albo 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode 	
			...
			MENU " Purchase Orders (by Vendor)" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","RA1","menu-purchase_orders-1") 
					CALL RA1_rpt_process(RA1_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL RA1_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW R107 with FORM "R107" 
			CALL  windecoration_r("R107") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(RA1_rpt_query()) #save where clause in env 
			CLOSE WINDOW R107 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL RA1_rpt_process(get_url_sel_text())
	END CASE

END FUNCTION 
###########################################################################
# END FUNCTION RA1_main() 
###########################################################################


###########################################################################
# FUNCTION RA1_rpt_query() 
#
# 
###########################################################################
FUNCTION RA1_rpt_query() 
	...
	entry_date 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","RA1","construct-purchhead-1") 
...
END FUNCTION 
###########################################################################
# END FUNCTION RA1_rpt_query() 
###########################################################################


###########################################################################
# FUNCTION RA1_rpt_process(p_where_text) 
#
# 
###########################################################################
FUNCTION RA1_rpt_process(p_where_text) 
	...
	"AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("RA1_rpt_list")].sel_text clipped," ",
	"ORDER BY vend_code,order_num" 
	...
	FOREACH c_purchhead INTO pr_purchhead.* 
		...
		#---------------------------------------------------------
		OUTPUT TO REPORT RA1_rpt_list(l_rpt_idx,pr_purchhead.*,pr_total_rec.*) 
		IF NOT rpt_int_flag_handler2("Vendor:",pr_purchhead.vend_code,NULL,l_rpt_idx) THEN
			EXIT FOREACH 
		END IF 
		#---------------------------------------------------------		
...
END FUNCTION 
###########################################################################
# END FUNCTION RA1_rpt_process(p_where_text) 
###########################################################################


###########################################################################
# REPORT RA1_rpt_list(p_rpt_idx,pr_purchhead,pr_total_rec)
#
# 
###########################################################################
REPORT RA1_rpt_list(p_rpt_idx,pr_purchhead,pr_total_rec)
	...
	OUTPUT 
--	left margin 0 
	ORDER external BY pr_purchhead.vend_code, 
	...
	FORMAT 
		...
		ON EVERY ROW 
			...
			COLUMN 25, pr_purchhead.order_date USING "dd/mm/yy", 
		...
		AFTER GROUP OF pr_purchhead.vend_code 
			NEED 2 LINES 
			PRINT COLUMN 25, "Vendor Totals:", 
			COLUMN 115, "---------------" 
			PRINT COLUMN 40, "Orders:", GROUP count(*) USING "<<<<"; 
			PRINT COLUMN 60, "Average: ",group avg(pr_total_rec.order_amt) 
			USING "--,---,---.&&", 
			COLUMN 115, GROUP sum(pr_total_rec.order_amt) 
			USING "---,---,---.&&" 
			SKIP 2 LINES 
...
END REPORT
###########################################################################
# END REPORT RA1_rpt_list(p_rpt_idx,pr_purchhead,pr_total_rec)
###########################################################################
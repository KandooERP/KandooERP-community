...
# MAIN
#
# \brief module Q16 - Allows the user TO Scan Quotations by Valid Date
#######################################################################
MAIN 

	CALL setModuleId("Q16") -- albo 
	...
	AND parm_code = "1" 
	OPEN WINDOW q103 with FORM "Q103" -- alch kd-747 
	CALL windecoration_q("Q103") -- alch kd-747 
...
FUNCTION scan_quote() 
	...
	END RECORD, 
	idx,scrn SMALLINT, 
	pr_valid_date LIKE quotehead.quote_date, 
	query_text CHAR(800), 
	where_text CHAR(400) 
	...
	WHILE true 
		...
		LET pr_quotehead.valid_date = today 
		INPUT BY NAME pr_quotehead.valid_date WITHOUT DEFAULTS 
			BEFORE INPUT 
				CALL publish_toolbar("kandoo","Q16","inp-valid_date-1") -- alch kd-501 
		...
		INPUT ARRAY pa_quotehead WITHOUT DEFAULTS FROM sr_quotehead.* 
			BEFORE INPUT 
				CALL publish_toolbar("kandoo","Q16","inp_arr-pa_quotehead-2") -- alch kd-501 
			ON ACTION "WEB-HELP" -- albo kd-369 
				CALL onlinehelp(getmoduleid(),null) 
			BEFORE ROW 
				LET idx = arr_curr() 

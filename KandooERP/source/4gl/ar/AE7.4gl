...
# FUNCTION AE7_main() 
#
# AE7 Sales Analysis by Invoicehead Department Code
############################################################
FUNCTION AE7_main() 
	DEFER interrupt 
	DEFER quit 
	
	CALL setModuleId("AE7")
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 
			...
			MENU " Customer by Product Sales Analysis" 
				...
				BEFORE MENU 
					CALL publish_toolbar("kandoo","AE7","menu-customer-product-sales") 
					CALL AE7_rpt_process(AE7_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL AE7_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW A105 with FORM "A105" 
			CALL windecoration_a("A105") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(AE7_rpt_query()) #save where clause in env 
			CLOSE WINDOW A105 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL AE7_rpt_process(get_url_sel_text())
	END CASE 				
END FUNCTION 

############################################################
# FUNCTION AE7_rpt_query()   
#
#
############################################################
FUNCTION AE7_rpt_query() 
	...
	INPUT BY NAME l_rec_period.year_num, l_rec_period.period_num WITHOUT DEFAULTS 
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","AE7","inp-period") 
	...
	customer.email 	 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","AE7","construct-customer") 
...
#####################################################################
# FUNCTION AE7_rpt_process(p_where_text) 
#
#
#####################################################################
FUNCTION AE7_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"AE7_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT AE7_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH selcurs INTO l_rec_tempdoc.* 
		IF l_rec_tempdoc.tm_part_code IS NOT NULL THEN 
			#---------------------------------------------------------
			OUTPUT TO REPORT AE7_rpt_list(l_rpt_idx,l_rec_tempdoc.*,l_rec_period.period_num, l_rec_period.year_num) 
	...
	#------------------------------------------------------------
	FINISH REPORT AE7_rpt_list
	RETURN rpt_finish("AE7_rpt_list")
	#------------------------------------------------------------
 
END FUNCTION 

############################################################
# REPORT AE7_rpt_list(p_rec_tempdoc, p_curr_period, p_curr_year)  
#
#
############################################################
REPORT AE7_rpt_list(p_rpt_idx, p_curr_period, p_curr_year) 
	...
	ORDER external BY p_rec_tempdoc.tm_cat_code, 
	...
	FORMAT 
		PAGE HEADER 
			...
			IF pageno > 1 THEN 
				PRINT COLUMN 1, "Continuation of Sales Analysis FOR Product Category ", 
		...
		AFTER GROUP OF p_rec_tempdoc.tm_cat_code 
			IF GROUP sum(p_rec_tempdoc.tm_ship_qty) != 0 THEN 
				IF (group sum(p_rec_tempdoc.tm_ext_sale_amt) 
				WHERE p_rec_tempdoc.tm_period_num = p_curr_period) !=0 
				AND GROUP sum(p_rec_tempdoc.tm_ext_sale_amt) != 0 THEN 
					SKIP 1 line 
					NEED 4 LINES 
					PRINT COLUMN 11, "----------------------------------------", 
					"----------------------------------------", 
					"-----------------------------------------" 
					PRINT COLUMN 1, "Totals FOR ", p_rec_tempdoc.tm_cat_code 
					PRINT COLUMN 17, GROUP sum(p_rec_tempdoc.tm_ext_sale_amt) 
					WHERE p_rec_tempdoc.tm_period_num = p_curr_period 
					USING "--,---,--$.&&", 

...
# \brief module A3C allows the user TO edit sundry receipts without having TO
# unapply AND reapply them. This allows the corresponding invoice TO be
# changed as well.
...
# FUNCTION A3C_main()
#
# arg = get_url_cashreceipt_number()
######################################################################
FUNCTION A3C_main() 
	...
	CALL setModuleId("A3C") 

	IF get_url_cashreceipt_number() IS NOT NULL THEN 
		CALL cashreceipt_edit(
			glob_rec_kandoouser.cmpy_code, 
			get_url_cashreceipt_number(), 
...
END FUNCTION
######################################################################
# END FUNCTION A3C_main()
######################################################################

{
######################################################################
# FUNCTION db_receipt_get_datasource(p_flag)
#
#
######################################################################
FUNCTION db_receipt_get_datasource(p_flag)
	DEFINE p_flag BOOLEAN
	...
	INPUT BY NAME l_rec_t_cashreceipt.cust_code,l_rec_t_cashreceipt.cash_num #Search/Select Customer 

		BEFORE INPUT 
			CALL publish_toolbar("kandoo","A3C","inp-cashreceipt") 
...
--			WHERE cust_code = l_rec_t_cashreceipt.cust_code 
...
--			IF status = NOTFOUND THEN 
	...
	IF l_idx = 0 THEN 
		LET l_idx = 1 
		INITIALIZE l_arr_rec_cashreceipt[l_idx].* TO NULL 
	END IF 

	RETURN l_arr_rec_cashreceipt 
END FUNCTION
######################################################################
# END FUNCTION db_receipt_get_datasource(p_flag)
######################################################################
}

######################################################################
# FUNCTION A3C_enter_receipt()
#
#
######################################################################
FUNCTION A3C_enter_receipt() 
	...
	--CALL db_receipt_get_datasource(FALSE) RETURNING l_arr_rec_s_cashreceipt
	CALL db_cashreceipt_get_datasource(l_rec_cashreceipt.*) RETURNING l_arr_rec_s_cashreceipt, l_rec_cashreceipt.*
	...
	DISPLAY ARRAY l_arr_rec_s_cashreceipt TO sr_cashreceipt.* ATTRIBUTE(UNBUFFERED) 
		BEFORE DISPLAY 
			CALL publish_toolbar("kandoo","A3C","inp-arr-cashreceipt") 
		...
		ON ACTION ("ACCEPT","EDIT","DOUBLECLICK") 
			...
			IF l_arr_rec_s_cashreceipt[l_idx].cash_num IS NOT NULL 
			AND l_arr_rec_s_cashreceipt[l_idx].cash_num > 0 THEN 
...
END FUNCTION 
######################################################################
# END FUNCTION A3C_enter_receipt()
######################################################################
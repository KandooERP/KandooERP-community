...
########################################################################
# FUNCTION A20_invoice_result_menu(p_mode)
...
FUNCTION A20_invoice_result_menu(p_mode) 
	...
	IF get_debug() THEN
		DISPLAY "###############################################"	
		LET l_msg = "BEGIN - A20_invoice_result_menu(p_mode=", trim(p_mode), ")" 					
		DISPLAY l_msg 
		DISPLAY "###############################################"
	END IF

	LET l_ret_nav = NAV_DONE --NAV_FORWARD
	...
	MENU "Invoice" 
		...
		ON ACTION "actToolbarManager" 
			CALL setuptoolbar() 
		#duplicate as a temp solution, remove this later 19.02.2020	
		...
		ON ACTION ("PRINT","PRINT2")  #duplicate as a temp solution, remove this later 19.02.2020
		...
		ON ACTION ("RECEIPT","RECEIPT2") #duplicate as a temp solution, remove this later 19.02.2020
			IF glob_rec_invoicehead.inv_num IS NOT NULL THEN 
				IF glob_rec_invoicehead.inv_num > 0 THEN 
					MESSAGE kandoomsg2("A",7071,glob_rec_invoicehead.inv_num)		#A7071Invoice added successfully"
...
--					EXIT MENU 
		#duplicate as a temp solution, remove this later 19.02.2020
		ON ACTION ("NAV_BACKWARD","NAV_BACKWARD2") -- " RETURN TO invoice entry" #COMMAND KEY(interrupt,"E")"Exit" " RETURN TO invoice entry"
...
--					LET int_flag = TRUE
		...
		ON ACTION ("ACCEPT") -- RETURN TO main menu
			LET l_ret_nav = NAV_DONE 
			EXIT MENU 
...
END FUNCTION 
########################################################################
# END FUNCTION A20_invoice_result_menu(p_mode)
########################################################################
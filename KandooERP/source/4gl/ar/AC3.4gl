...
FUNCTION AC3_main() 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 	
			...
			MENU " Cash Receipt By Period" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","AC3","menu-cash-receipt-period") 
					CALL AC3_rpt_process(AC3_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL AC3_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW A148 with FORM "A148" 
			CALL windecoration_a("A148") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(AC3_rpt_query()) #save where clause in env 
			CLOSE WINDOW A148 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL AC3_rpt_process(get_url_sel_text())
	END CASE 

END FUNCTION 
#####################################################################
# END FUNCTION AC3_main()
#####################################################################


#####################################################################
# FUNCTION AC3_rpt_query()
#
#
#####################################################################
FUNCTION AC3_rpt_query() 
	...
	cashreceipt.entry_date 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","AC3","construct-cashreceipt") 
...
END FUNCTION 
#####################################################################
# END FUNCTION AC3_rpt_query()
#####################################################################


#####################################################################
# FUNCTION AC3_rpt_process()
#
#
#####################################################################
FUNCTION AC3_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"AC3_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT AC3_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	" AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("AC3_rpt_list")].sel_text clipped clipped 
	...
	FOREACH c_cashreceipt INTO l_rec_cashreceipt.*
		#---------------------------------------------------------
		OUTPUT TO REPORT AC3_rpt_list(l_rpt_idx,l_rec_cashreceipt.*)  
	...
	#------------------------------------------------------------
	FINISH REPORT AC3_rpt_list
	RETURN rpt_finish("AC3_rpt_list")
	#------------------------------------------------------------
 
END FUNCTION 
#####################################################################
# END FUNCTION AC3_rpt_process()
#####################################################################


#####################################################################
# REPORT AC3_rpt_list(p_rpt_idx,p_rec_cashreceipt) 
#
#
#####################################################################
REPORT AC3_rpt_list(p_rpt_idx,p_rec_cashreceipt) 
	...
	OUTPUT 
--	left margin 0 
	ORDER BY p_rec_cashreceipt.year_num, 
	p_rec_cashreceipt.period_num, 
	...
	FORMAT 
		...
		BEFORE GROUP OF p_rec_cashreceipt.period_num 
			SKIP TO top OF PAGE 
			PRINT COLUMN 1, "Year: ", p_rec_cashreceipt.year_num USING "####", 
			COLUMN 12, "Period: ",p_rec_cashreceipt.period_num USING "###" 
...
--			AND cmpy_code = glob_rec_kandoouser.cmpy_code 
			...
			COLUMN 25, l_rec_customer.name_text, 
			" Currency ", p_rec_cashreceipt.currency_code 
		...
		ON EVERY ROW 
			...
			COLUMN 35, p_rec_cashreceipt.period_num USING "###", 
...
END REPORT
#####################################################################
# END REPORT AC3_rpt_list(p_rpt_idx,p_rec_cashreceipt) 
#####################################################################
...
GLOBALS "../ar/AA1_GLOBALS.4gl" 
#####################################################################
# FUNCTION AA1_main()
#
# AA1 Customer Address Listing
#####################################################################
FUNCTION AA1_main() 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 
			...
			MENU " Customer Address Report" 
				BEFORE MENU 
					CALL publish_toolbar("kandoo","AA1","menu-customer-address-rep")
					CALL rpt_rmsreps_reset(NULL)					 
					CALL AA1_rpt_process(AA1_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL AA1_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW A112 with FORM "A112" 
			CALL windecoration_a("A112") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(AA1_rpt_query()) #save where clause in env 
			CLOSE WINDOW A112 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL AA1_rpt_process(get_url_sel_text())
	END CASE 

END FUNCTION 
#####################################################################
# ENDFUNCTION AA1_main()
#####################################################################


#####################################################################
# FUNCTION AA1_rpt_query()
#
#
#####################################################################
FUNCTION AA1_rpt_query() 
	...
	delete_date 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","AA1","construct-customer") 
...
END FUNCTION 
#####################################################################
# END FUNCTION AA1_rpt_query()
#####################################################################


#####################################################################
# FUNCTION AA1_rpt_process(p_where_text)
#
#
#####################################################################
FUNCTION AA1_rpt_process(p_where_text)
	...
	"AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("AA1_rpt_list")].sel_text clipped," ", 
	"ORDER BY" 
	...
	FOREACH c_customer INTO l_rec_customer.* 
		...
		#---------------------------------------------------------
		OUTPUT TO REPORT AA1_rpt_list(l_rpt_idx,l_rec_customer.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT AA1_rpt_list
	CALL rpt_finish("AA1_rpt_list")
	#------------------------------------------------------------
...
END FUNCTION 
#####################################################################
# END FUNCTION AA1_rpt_process(p_where_text)
#####################################################################


#####################################################################
# REPORT AA1_rpt_list(p_rpt_idx,p_rec_customer)
#
#
#####################################################################
REPORT AA1_rpt_list(p_rpt_idx,p_rec_customer) 
...
END REPORT
#####################################################################
# END REPORT AA1_rpt_list(p_rpt_idx,p_rec_customer)
#####################################################################
...
###########################################################################
	...
	Source code beautified by beautify.pl on 2020-01-03 11:19:03	$Id: $
}
############################################################
...
FUNCTION AAT_main()
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		...
		MENU " Customer Summary Aging" 
			BEFORE MENU 
				CALL publish_toolbar("kandoo","AAT","menu-customer-summary-aging")
				CALL rpt_rmsreps_reset(NULL) 
				CALL AAT_rpt_process(AAT_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL AAT_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW A657 with FORM "A657" 
			CALL windecoration_a("A657") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(AAT_rpt_query()) #save where clause in env 
			CLOSE WINDOW A657 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL AAT_rpt_process(get_url_sel_text())
	END CASE 
...
END FUNCTION 
#####################################################################
# END FUNCTION AAT_main()
#####################################################################


#####################################################################
# FUNCTION AAT_rpt_query()
#
#
#####################################################################
FUNCTION AAT_rpt_query()
	...
	MESSAGE kandoomsg("A",1079,"") #1079 Enter Date TO age REPORT TO, F9 FOR last aging date
	LET modu_age_date = today 
	LET modu_report_level = "1" 

	INPUT modu_age_date,	modu_report_level WITHOUT DEFAULTS 
	FROM age_date,	report_level

		BEFORE INPUT 
			CALL publish_toolbar("kandoo","AAT","inp-rep-param") 
		...
		AFTER FIELD report_level 
			...
			(modu_report_level <> "1" AND 
			modu_report_level <> "2") THEN 
				LET modu_report_level = 1 
		...
		AFTER INPUT 
			...
			LET glob_rec_rpt_selector.ref1_ind = modu_report_level 
	...
	email
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","AAT","construct-customer") 
...
END FUNCTION 
#####################################################################
# END FUNCTION AAT_rpt_query()
#####################################################################


#####################################################################
# FUNCTION AAT_rpt_process(p_where_text)
#
#
#####################################################################
FUNCTION AAT_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"AAT_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT AAT_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	" AND ", glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("AAT_rpt_list")].sel_text clipped, 
...
#		END IF 
		...
		FOREACH c_invoicepay INTO modu_rec_invoicepay.* 
			...
			ELSE 
				...
				IF status = 0 THEN 
					...
					LET modu_rec_doc.d_type = TRAN_TYPE_INVOICE_IN 
					LET modu_rec_doc.d_age = get_age_bucket(TRAN_TYPE_INVOICE_IN,l_rec_invoicehead.due_date) 
					LET modu_rec_doc.d_bal = l_rec_invoicehead.total_amt - l_rec_invoicehead.paid_amt 
					+ modu_rec_invoicepay.pay_amt + modu_rec_invoicepay.disc_amt 
					INSERT INTO doctab VALUES (modu_rec_doc.*) 
				END IF 
			END IF 
		...
		FOREACH c_t_doctab INTO modu_rec_doc.* 
			#---------------------------------------------------------
			OUTPUT TO REPORT AAT_rpt_list(l_rpt_idx,modu_rec_doc.*) 
	...
	END FOREACH
	#------------------------------------------------------------
	FINISH REPORT AAT_rpt_list
	CALL rpt_finish("AAT_rpt_list")
	#------------------------------------------------------------
...
END FUNCTION 
#####################################################################
# END FUNCTION AAT_rpt_process(p_where_text)
#####################################################################


#####################################################################
# REPORT AAT_rpt_list(p_rec_doc)
#
#
#####################################################################
REPORT AAT_rpt_list(p_rpt_idx,p_rec_doc) 
...
--	DEFINE l_bal_amt, l_over60_90, l_over90,l_over30_60, l_over1_30, l_curr, l_bal money(12,2)
...
END REPORT
#####################################################################
# END REPORT AAT_rpt_list(p_rec_doc)
#####################################################################
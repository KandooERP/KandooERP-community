...
--DEFINE l_where2_text CHAR(500)
DEFINE modu_temp_text VARCHAR(200)	
#####################################################################
# FUNCTION AA6_main()
#
# AA6 - Customer address Listing (by sales person)
#####################################################################
FUNCTION AA6_main() 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 	
			...
			MENU "Customer Address Report" 
				BEFORE MENU 
					CALL publish_toolbar("kandoo","AA6","menu-customer-address-rep")
					CALL rpt_rmsreps_reset(NULL) 
					CALL AA6_rpt_process(AA6_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL AA6_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW A112 with FORM "A112" 
			CALL windecoration_a("A112") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(AA6_rpt_query()) #save where clause in env 
			CLOSE WINDOW p120 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL AA6_rpt_process(get_url_sel_text())
	END CASE 	 
	
END FUNCTION 
#####################################################################
# END FUNCTION AA6_main()
#####################################################################


#####################################################################
# FUNCTION AA6_rpt_query()
#
#
#####################################################################
FUNCTION AA6_rpt_query()
	...
	CONSTRUCT BY NAME l_where1_text ON cust_code, 
	name_text, 
	currency_code, 
	addr1_text, 
	addr2_text, 
	...
	delete_date 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","AA6","construct-customer") 
...
END FUNCTION
#####################################################################
# END FUNCTION AA6_rpt_query()
#####################################################################


#####################################################################
# FUNCTION AA6_rpt_process(p_where1_text,p_where2_text)
#
#
#####################################################################
FUNCTION AA6_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"AA6_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT AA6_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	"AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("AA6_rpt_list")].sel_text clipped," ", 
	"ORDER BY sale_code," 
	...
	FOREACH c_customer INTO l_rec_customer.*
		...
		OUTPUT TO REPORT AA6_rpt_list(l_rpt_idx,l_rec_customer.*) 
		IF NOT rpt_int_flag_handler2("Salesperson",l_rec_customer.sale_code, l_rec_customer.cust_code,l_rpt_idx) THEN
			EXIT FOREACH 
		END IF 
	...
	#------------------------------------------------------------
	FINISH REPORT AA6_rpt_list
	CALL rpt_finish("AA6_rpt_list")
	#------------------------------------------------------------
...
END FUNCTION 
#####################################################################
# END FUNCTION AA6_rpt_process(p_where1_text,p_where2_text)
#####################################################################


#####################################################################
# REPORT AA6_rpt_list(p_rec_customer)
#
#
#####################################################################
REPORT AA6_rpt_list(p_rpt_idx,p_rec_customer) 
	...
	BEFORE GROUP OF p_rec_customer.sale_code 
		#Sales Person
		CALL db_salesperson_get_rec(UI_OFF,p_rec_customer.sale_code) RETURNING l_rec_salesperson.*
		IF l_rec_salesperson.sale_code IS NULL THEN
			LET l_rec_salesperson.name_text = "**********"
		END IF
		PRINT COLUMN 01, "Salesperson: ",p_rec_customer.sale_code CLIPPED," ", l_rec_salesperson.name_text CLIPPED 
...
END REPORT
#####################################################################
# END REPORT AA6_rpt_list(p_rec_customer)
#####################################################################
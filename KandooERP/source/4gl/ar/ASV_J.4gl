...
FUNCTION ASV_J_main() 
	...
	MENU " Extract " 
		...
		BEFORE MENU 
			CALL publish_toolbar("kandoo","ASV_J","menu-extract") 
				CALL ASV_J_update_trans(ASV_J_select_trans()) 
		...
		ON ACTION "FILTER" #COMMAND "Query" " Enter Search Criteria" 
				CALL ASV_J_update_trans(ASV_J_select_trans()) 
...
FUNCTION ASV_J_select_trans() 
	...
	END IF 
	MESSAGE kandoomsg2("A",1074,"") 

	#1074 Enter start & END dates FOR extraction
	INPUT modu_start_date,	modu_end_date WITHOUT DEFAULTS FROM modu_start_date,	modu_end_date 


		BEFORE INPUT 
			CALL publish_toolbar("kandoo","ASV","inp-date") 
	...
	customer.type_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","ASV_J","construct-customer") 
...
##########################################################################
# FUNCTION ASV_J_update_trans(p_where_text)
#
#
##########################################################################
FUNCTION ASV_J_update_trans(p_where_text) 
	...
	DEFINE l_tmpmsg STRING --for MESSAGE strings 
	...
	LET l_err_message = "Imprest Extract (ASV) - Started" 
	...
	BEGIN WORK 
		...
		#DISPLAY "see ar/ASV_J.4gl"
		#EXIT PROGRAM (1)
		################################################


		DECLARE i1_impresttran CURSOR FOR 
		INSERT INTO jmj_impresttran VALUES (l_rec_invoicehead.*) 
		OPEN i1_impresttran 
		...
		#huho
		#OPEN WINDOW w1 AT 10,15
		#   with 2 rows, 50 columns
		#   attribute (border)
		# DISPLAY "Customer: " AT 2,3 --huho
		...
		FOREACH c_customer INTO l_cust_code 
			...
			LET l_tmpmsg = "Customer: ", trim(l_cust_code) 
			CALL displaystatus(l_tmpmsg) 
			...
			FOREACH c_invoicehead INTO l_rec_invoicehead.* 
				...
				LET l_rec_invoicehead.stat_date = modu_end_date 
				LET l_rec_invoicehead.printed_num = l_suffix_num 
				PUT i1_impresttran 
			END FOREACH 
		...
		IF int_flag OR quit_flag THEN 
			...
			CLOSE WINDOW w1 
			ROLLBACK WORK 
			LET l_err_message = "Imprest Extract (ASV) - Interrupted" 
			CALL errorlog(l_err_message) 
			RETURN 
		END IF 
	...
	LET l_err_message = "Imprest Extract (ASV) - Finished" 
	MESSAGE "Imprest Extract (ASV) - Finished" --huho 
	CALL errorlog(l_err_message) 

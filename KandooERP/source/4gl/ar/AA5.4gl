...
GLOBALS "../ar/AA5_GLOBALS.4gl" 
############################################################
# MODU Scope Variables
############################################################
--DEFINE modu_where1_text CHAR(800)
--DEFINE modu_where2_text CHAR(500)
DEFINE modu_temp_text VARCHAR(200)	
#####################################################################
# FUNCTION AA5_main() 
#
# AA5 - Customer address Listing
#####################################################################
FUNCTION AA5_main() 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 	
			...
			MENU " Customer Address (by Postcode)" 
				BEFORE MENU 
					CALL publish_toolbar("kandoo","AA5","menu-customer-address-po-code") 
					CALL rpt_rmsreps_reset(NULL)
					CALL AA5_rpt_process(AA5_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL AA5_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW A112 with FORM "A112" 
			CALL windecoration_a("A112") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(AA5_rpt_query()) #save where clause in env 
			CLOSE WINDOW A112 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL AA5_rpt_process(get_url_sel_text())
	END CASE 
...
#####################################################################
# FUNCTION AA5_rpt_query()
#
# 	RETURN l_query_text/NULL
#####################################################################
FUNCTION AA5_rpt_query() 
	...
	delete_date 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","AA5","construct-customer") 
...
END FUNCTION
#####################################################################
# END FUNCTION AA5_rpt_query()
#####################################################################


#####################################################################
# FUNCTION AA5_rpt_process()
#
#
#####################################################################
FUNCTION AA5_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"AA5_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT AA5_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	"AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("AA5_rpt_list")].sel_text clipped," ",
	"ORDER BY post_code," 
	...
	FOREACH c_customer INTO l_rec_customer.* 
		...
		#---------------------------------------------------------
		OUTPUT TO REPORT AA5_rpt_list(l_rpt_idx,l_rec_customer.*) 
		IF NOT rpt_int_flag_handler2("Postcode:",l_rec_customer.post_code, l_rec_customer.cust_code,l_rpt_idx) THEN
			EXIT FOREACH 
		END IF 
		#---------------------------------------------------------
	...
	#------------------------------------------------------------
	FINISH REPORT AA5_rpt_list
	CALL rpt_finish("AA5_rpt_list")
	#------------------------------------------------------------
	IF int_flag THEN
		RETURN FALSE
	ELSE
...
END FUNCTION 
#####################################################################
# END FUNCTION AA5_rpt_process()
#####################################################################


#####################################################################
# REPORT AA5_rpt_list(p_rec_customer)
#
#
#####################################################################
REPORT AA5_rpt_list(p_rpt_idx,p_rec_customer) 
	...
	BEFORE GROUP OF p_rec_customer.post_code 
		SKIP 1 line 
		PRINT COLUMN 01, "Postcode: ", p_rec_customer.post_code CLIPPED 
...
END REPORT
#####################################################################
# END REPORT AA5_rpt_list(p_rec_customer)
#####################################################################
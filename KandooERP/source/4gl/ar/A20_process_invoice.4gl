...
FUNCTION process_invoice_wizzard(p_inv_num) 
	...
	LET l_edit_req = TRUE #???? remove this ?
...
--				LET l_msg = " A20_invoice_result_menu()\nStep (", trim(l_step) , "/5)\nl_ret_nav = ", trim(l_ret_nav)
--				CALL fgl_winmessage(l_step,l_msg,"INFO")
...
{				IF glob_rec_invoicehead.inv_num > 0 THEN 
				...
				LET l_ret_nav = A20_invoice_result_menu(l_mode)
				CASE l_ret_nav
					WHEN NAV_BACKWARD
						LET l_step = l_step - 1
					WHEN NAV_FORWARD

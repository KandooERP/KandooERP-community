...
FUNCTION AE3_main() 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 	
			...
			MENU " Salesperson Sales Analysis" 
		
				BEFORE MENU 
					CALL publish_toolbar("kandoo","AE3","menu-sales-person-analysis") 
					CALL AE3_rpt_process(AE3_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL AE3_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW A177 with FORM "A177" 
			CALL windecoration_a("A177") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(AE3_rpt_query()) #save where clause in env 
			CLOSE WINDOW A177 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL AE3_rpt_process(get_url_sel_text())
	END CASE 				
END FUNCTION 

############################################################
# FUNCTION AE3_rpt_query()  
#
#
############################################################
FUNCTION AE3_rpt_query() 
	...
	customer.tax_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","AE3","construct-customer") 
...
#####################################################################
# FUNCTION AE3_rpt_process(p_where_text) 
#
#
#####################################################################
FUNCTION AE3_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"AE3_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT AE3_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH c_invoice INTO l_rec_invoicehead.* 
		...
		LET l_rec_tempdoc.tm_sales = l_rec_invoicehead.sale_code 
		IF glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("AE3_rpt_list")].ref1_ind = "N" THEN 
	...
	FOREACH credcurs INTO l_rec_credithead.* 
		...
		cust_code = l_rec_credithead.cust_code 
		...
		LET l_rec_tempdoc.tm_sales = l_rec_credithead.sale_code 
		IF glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("AE3_rpt_list")].ref1_ind = "N" THEN 
	...
	FOREACH selcurs INTO l_rec_tempdoc.* 
		#---------------------------------------------------------
		OUTPUT TO REPORT AE3_rpt_list(l_rpt_idx,l_rec_tempdoc.*, glob_rec_kandoouser.cmpy_code)  
	...
	#------------------------------------------------------------
	FINISH REPORT AE3_rpt_list
	RETURN rpt_finish("AE3_rpt_list")
	#------------------------------------------------------------
...
############################################################
# REPORT AE3_rpt_list(p_rpt_idx,p_rec_tempdoc, p_cmpy_code) 
#
#
############################################################
REPORT AE3_rpt_list(p_rpt_idx,p_rec_tempdoc, p_cmpy_code) 
	...
	DEFINE l_rec_net_profit RECORD LIKE salesperson.* 

	OUTPUT 
--	left margin 0 
	ORDER external BY p_rec_tempdoc.tm_sales, 
	...
	FORMAT 
		...
		ON EVERY ROW 
			...
			OR p_rec_tempdoc.tm_amount IS NULL THEN 
				...
				COLUMN 47, p_rec_tempdoc.tm_amount USING "--,---,--$.&&", 
				COLUMN 61, p_rec_tempdoc.tm_cost USING "--,---,--$.&&", 
				COLUMN 75, p_rec_tempdoc.tm_dis USING "--,---,--$.&&", 
				COLUMN 89, p_rec_tempdoc.tm_comm USING "--,---,--$.&&", 
				COLUMN 105, l_net_profit USING "--,---,--$.&&", 
				COLUMN 120, (p_rec_tempdoc.tm_amount - p_rec_tempdoc.tm_cost) / 
		...
		BEFORE GROUP OF p_rec_tempdoc.tm_sales 
			INITIALIZE l_rec_net_profit.* TO NULL 
			SELECT * 
			INTO l_rec_net_profit.* 
			FROM salesperson 
			WHERE cmpy_code = p_cmpy_code AND 
			sale_code = p_rec_tempdoc.tm_sales 
			IF status = (NOTFOUND) THEN 
				LET l_rec_net_profit.name_text = "Unknown" 
			END IF 
			SKIP 2 LINES 
			PRINT COLUMN 1, "Salesperson: ", l_rec_net_profit.name_text 
			
		AFTER GROUP OF p_rec_tempdoc.tm_sales 
			IF GROUP sum(p_rec_tempdoc.tm_amount) != 0 
			THEN 
				PRINT COLUMN 40, 
				"-----------------------------------------------------", 
				"---------------------------------------" 
				PRINT COLUMN 44, GROUP sum(p_rec_tempdoc.tm_amount) 
				USING "-----,---,--$.&&", 
				COLUMN 72, GROUP sum(p_rec_tempdoc.tm_dis) USING "-----,---,--$.&&", 
				COLUMN 102, GROUP sum(p_rec_tempdoc.tm_amount) - 
				(group sum(p_rec_tempdoc.tm_cost) 
				+ GROUP sum(p_rec_tempdoc.tm_dis) 
				+ GROUP sum(p_rec_tempdoc.tm_comm)) 
				USING "-----,---,--$.&&" 
				PRINT COLUMN 58, GROUP sum(p_rec_tempdoc.tm_cost) 
				USING "-----,---,--$.&&", 
				COLUMN 86, GROUP sum(p_rec_tempdoc.tm_comm) 
				USING "-----,---,--$.&&", 
				COLUMN 120, (group sum(p_rec_tempdoc.tm_amount) 
				- GROUP sum(p_rec_tempdoc.tm_cost)) / 
		...
		ON LAST ROW 
			IF sum(p_rec_tempdoc.tm_amount) <> 0 AND 
			sum(p_rec_tempdoc.tm_amount) IS NOT NULL 
			THEN 
				PRINT COLUMN 40, 
				"-----------------------------------------------------", 
				"---------------------------------------" 
				PRINT COLUMN 44, sum(p_rec_tempdoc.tm_amount) USING "-----,---,--$.&&", 
				COLUMN 72, sum(p_rec_tempdoc.tm_dis) USING "-----,---,--$.&&", 
				COLUMN 102, sum(p_rec_tempdoc.tm_amount) - 
				(sum(p_rec_tempdoc.tm_cost) 
				+ sum(p_rec_tempdoc.tm_dis) 
				+ sum(p_rec_tempdoc.tm_comm)) USING "-----,---,--$.&&" 
				PRINT COLUMN 58, sum(p_rec_tempdoc.tm_cost) USING "-----,---,--$.&&", 
				COLUMN 86, sum(p_rec_tempdoc.tm_comm) USING "-----,---,--$.&&", 
				COLUMN 120, (sum(p_rec_tempdoc.tm_amount) 
				- sum(p_rec_tempdoc.tm_cost)) / 

...
# FUNCTION AC8_main()
#
# AC8 - Cash Receipts by Reference
#####################################################################
FUNCTION AC8_main() 
	DEFER interrupt 
	DEFER quit 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 	
			...
			MENU " Cash Receipts by Reference" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","AC8","menu-cash-receipts-ref") 
					CALL AC8_rpt_process(AC8_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL AC8_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW A148 with FORM "A148" 
			CALL windecoration_a("A148") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(AC8_rpt_query()) #save where clause in env 
			CLOSE WINDOW A148 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL AC8_rpt_process(get_url_sel_text())
	END CASE 	

END FUNCTION 
#####################################################################
# END FUNCTION AC8_main()
#####################################################################


#####################################################################
# FUNCTION AC8_rpt_query()
#
#
#####################################################################
FUNCTION AC8_rpt_query() 
	...
	cashreceipt.entry_date 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","AC8","construct-cashreceipt") 
...
END FUNCTION 
#####################################################################
# END FUNCTION AC8_rpt_query()
#####################################################################


#####################################################################
# FUNCTION AC8_rpt_process(p_where_text)
#
#
#####################################################################
FUNCTION AC8_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"AC8_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT AC8_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	" AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("AC8_rpt_list")].sel_text clipped," ", 
	" ORDER BY cashreceipt.cust_code, order_num, cash_num" 
	...
	FOREACH c_cashreceipt INTO l_rec_cashreceipt.* 
			#---------------------------------------------------------
		OUTPUT TO REPORT AC8_rpt_list(l_rpt_idx,l_rec_cashreceipt.*)  
		IF NOT rpt_int_flag_handler2("Receipt:",l_rec_cashreceipt.cash_num, NULL,l_rpt_idx) THEN
			EXIT FOREACH 
		END IF 
		#---------------------------------------------------------	
	...
	#------------------------------------------------------------
	FINISH REPORT AC8_rpt_list
	RETURN rpt_finish("AC8_rpt_list")
	#------------------------------------------------------------
	 
END FUNCTION 
#####################################################################
# END FUNCTION AC8_rpt_process(p_where_text)
#####################################################################


#####################################################################
# REPORT AC8_rpt_list(p_rpt_idx,p_rec_cashreceipt)
#
#
#####################################################################
REPORT AC8_rpt_list(p_rpt_idx,p_rec_cashreceipt) 
	...
	DEFINE l_arr_line array[4] OF CHAR(80) 

	OUTPUT 
 
	ORDER external BY p_rec_cashreceipt.cust_code, 
	p_rec_cashreceipt.order_num, 
...
--			WHERE cust_code = p_rec_cashreceipt.cust_code 
--			AND cmpy_code = glob_rec_kandoouser.cmpy_code 
			PRINT COLUMN 01, "Customer: ", p_rec_cashreceipt.cust_code, 
			COLUMN 20, l_rec_customer.name_text, " Currency ", 
			p_rec_cashreceipt.currency_code 

		ON EVERY ROW 
			PRINT COLUMN 01, p_rec_cashreceipt.cash_num USING "########", 
			COLUMN 10, p_rec_cashreceipt.order_num USING "########", 
		...
		AFTER GROUP OF p_rec_cashreceipt.order_num 
			PRINT COLUMN 44, "--------------------------------" 
...
END REPORT
#####################################################################
# END REPORT AC8_rpt_list(p_rpt_idx,p_rec_cashreceipt)
##################################################################### 
...
# FUNCTION ICE_main()
#
# Purpose - Price Execption Report - by Product
############################################################
FUNCTION ICE_main()

	CALL setModuleId("ICE") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Price Execption Report" 
				...
				BEFORE MENU 
					CALL publish_toolbar("kandoo","ICE","menu-Price Execption-1") -- albo kd-505
					CALL rpt_rmsreps_reset(NULL)
					CALL ICE_rpt_process(ICE_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL ICE_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I214 with FORM "I214" 
			 CALL windecoration_i("I214") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(ICE_rpt_query()) #save where clause in env 
			CLOSE WINDOW I214 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL ICE_rpt_process(get_url_sel_text())
	END CASE	

END FUNCTION 
############################################################
# END FUNCTION ICE_main()
############################################################

############################################################
# FUNCTION ICE_rpt_query() 
...
FUNCTION ICE_rpt_query() 
	...
	invoicehead.period_num 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","ICE","construct-product-1") -- albo kd-505 
...
END FUNCTION 
############################################################
# END FUNCTION ICE_rpt_query() 
############################################################

############################################################
# FUNCTION ICE_rpt_process(p_where_text) 
# 
# The report driver
############################################################
FUNCTION ICE_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"ICE_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT ICE_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	"ORDER BY product.part_code,customer.name_text,customer.cust_code,invoicehead.inv_date,invoicehead.inv_num" 
	...
	FOREACH selcurs INTO l_rec_invoice.* 
		...
		CASE 
			...
			OTHERWISE 
				LET l_rec_invoice.list_amt = l_rec_prodstatus.list_amt 
		END CASE 
		IF l_rec_invoice.list_amt != l_rec_invoice.unit_sale_amt AND 
			l_rec_invoice.list_amt <> 0 THEN 
			# discounted price need TO REPORT this one
			#---------------------------------------------------------
			OUTPUT TO REPORT ICE_rpt_list(l_rpt_idx,l_rec_invoice.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT ICE_rpt_list
	RETURN rpt_finish("ICE_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION ICE_rpt_process() 
############################################################

############################################################
# REPORT ICE_rpt_list(p_rpt_idx,p_rec_invoice)
#
...
REPORT ICE_rpt_list(p_rpt_idx,p_rec_invoice) 
	...
	ORDER EXTERNAL BY p_rec_invoice.part_code,p_rec_invoice.name_text, p_rec_invoice.cust_code,p_rec_invoice.inv_date,p_rec_invoice.inv_num 
	...
	BEFORE GROUP OF p_rec_invoice.part_code 
		PRINT COLUMN 1, p_rec_invoice.part_code CLIPPED,3 SPACES,p_rec_invoice.desc_text CLIPPED,1 SPACES,p_rec_invoice.desc2_text CLIPPED 

	BEFORE GROUP OF p_rec_invoice.cust_code 
		PRINT COLUMN 20, p_rec_invoice.cust_code CLIPPED,3 SPACES,p_rec_invoice.name_text CLIPPED 

	AFTER GROUP OF p_rec_invoice.cust_code 
		SKIP 1 LINE 

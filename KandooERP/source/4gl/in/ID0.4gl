...
# FUNCTION ID0_main()
#
# Purpose - Stock Re-order Reports
############################################################
FUNCTION ID0_main() 
	...
	CALL setModuleId("ID0")

	LET modu_report_run_once_code = get_url_module_child()
	IF modu_report_run_once_code IS NOT NULL THEN 
		CALL ID0_run_module(modu_report_run_once_code)
		RETURN 
	ELSE
		LET l_list_menuchoice = "ID1"
		LET l_inp_menuchoice = l_list_menuchoice 
	END IF
	 
	OPEN WINDOW ID00 WITH FORM "ID00" ATTRIBUTE(BORDER,STYLE="full-screen")
	 CALL windecoration_i("ID00") 
	...
	DIALOG ATTRIBUTES(UNBUFFERED) 
		...
		ON ACTION ("ACCEPT","DoubleClickItem") 
			...
			DISPLAY l_list_menuchoice TO tf_menuChoice		
			CALL ID0_run_module(l_list_menuchoice)
			IF int_flag THEN
...
END FUNCTION 
############################################################
# END FUNCTION ID0_main()
############################################################

############################################################
# FUNCTION ID0_run_module(p_module_id)
#
...
FUNCTION ID0_run_module(p_module_id)
	...
	CASE p_module_id.toUpperCase()
		...
		WHEN "ID8" 
			CALL ID8_main()
		OTHERWISE # 
			CALL fgl_winmessage("Invalid Menu Option in ID","Invalid Menu Option choosen","error") 
	END CASE
	 	
	CALL setModuleId("ID0") #Reset program module id
	CALL rpt_rmsreps_reset(NULL)

END FUNCTION
############################################################
# END FUNCTION ID0_run_module(p_module_id)
############################################################
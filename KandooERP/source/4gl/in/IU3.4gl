...
# FUNCTION IU3_main()
#
# Purpose - Costs Amendment Report
############################################################
FUNCTION IU3_main()

	CALL setModuleId("IU3") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Cost Amendment" 
				...
				BEFORE MENU 
					CALL publish_toolbar("kandoo","IU3","menu-Cost Amendment-1") -- albo kd-505
				...
				ON ACTION "Report" #COMMAND "Run Report" " SELECT Criteria AND Print Report"
					CALL IU3_rpt_process(IU3_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IU3_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I254 with FORM "I254" 
			 CALL windecoration_i("I254") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IU3_rpt_query()) #save where clause in env 
			CLOSE WINDOW I254 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IU3_rpt_process(get_url_sel_text())
	END CASE	

END FUNCTION 

############################################################
# FUNCTION IU3_rpt_query() 
#
#
############################################################
FUNCTION IU3_rpt_query() 
	...
	prodstatlog.user_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IU3","construct-prodstatlog-1") -- albo kd-505 
...
############################################################
# FUNCTION IU3_rpt_process() 
#
#
############################################################
FUNCTION IU3_rpt_process(p_where_text)
	DEFINE p_where_text STRING 
	DEFINE l_query_text STRING
	DEFINE l_rpt_idx SMALLINT
	DEFINE l_rec_product RECORD LIKE product.* 
	DEFINE l_rec_prodstatlog RECORD LIKE prodstatlog.* 
	DEFINE l_s_rec_prodstatlog RECORD LIKE prodstatlog.* 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IU3_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IU3_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH c_prodstatlog INTO l_rec_prodstatlog.*,l_rec_product.* 
		IF l_s_rec_prodstatlog.part_code IS NULL 
		OR l_s_rec_prodstatlog.ware_code IS NULL 
		OR l_s_rec_prodstatlog.part_code != l_rec_prodstatlog.part_code 
		OR l_s_rec_prodstatlog.ware_code != l_rec_prodstatlog.ware_code 
		OR l_s_rec_prodstatlog.list_price_amt != l_rec_prodstatlog.list_price_amt 
		OR l_s_rec_prodstatlog.est_cost_amt != l_rec_prodstatlog.est_cost_amt 
		OR l_s_rec_prodstatlog.act_cost_amt != l_rec_prodstatlog.act_cost_amt 
		OR l_s_rec_prodstatlog.for_cost_amt != l_rec_prodstatlog.for_cost_amt THEN 
			#---------------------------------------------------------
			OUTPUT TO REPORT IU3_rpt_list (l_rpt_idx,l_rec_prodstatlog.*,l_rec_product.*) 
			IF NOT rpt_int_flag_handler2("Product: ",l_rec_prodstatlog.part_code,"",l_rpt_idx) THEN
				EXIT FOREACH 
			END IF 
			#---------------------------------------------------------
			LET l_s_rec_prodstatlog.* = l_rec_prodstatlog.* 
		END IF 
	END FOREACH 

	#------------------------------------------------------------
	FINISH REPORT IU3_rpt_list
	CALL rpt_finish("IU3_rpt_list")
	#------------------------------------------------------------
...
############################################################
# REPORT IU3_rpt_list(p_rpt_idx,p_rec_prodstatlog,p_rec_product)
#
#
############################################################
REPORT IU3_rpt_list(p_rpt_idx,p_rec_prodstatlog,p_rec_product)
	...
	ON EVERY ROW 
		...
		COLUMN 63, l_name_text[1,20], 
		COLUMN 84, p_rec_product.sell_uom_code, 
		COLUMN 89, p_rec_prodstatlog.list_price_amt USING "-------&.&&", 
		COLUMN 100,p_rec_prodstatlog.est_cost_amt USING "-------&.&&", 
		COLUMN 111,p_rec_prodstatlog.act_cost_amt USING "-------&.&&", 
		COLUMN 122,p_rec_prodstatlog.for_cost_amt USING "-------&.&&" 

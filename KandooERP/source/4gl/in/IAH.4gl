...
# RETURN VOID
#
# Purpose - Product Quantity History Report
############################################################
FUNCTION IAH_main()

	CALL setModuleId("IAH") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Product Quantity History" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","IAH","menu-Product_Report-1") -- albo kd-505 
					CALL rpt_rmsreps_reset(NULL)
					CALL IAH_rpt_process(IAH_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IAH_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I111 with FORM "I111" 
			 CALL windecoration_i("I111") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IAH_rpt_query()) #save where clause in env 
			CLOSE WINDOW I111 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IAH_rpt_process(get_url_sel_text())
	END CASE

END FUNCTION 
############################################################
# END FUNCTION IAH_main() 
############################################################

############################################################
# FUNCTION IAH_rpt_query() 
...
FUNCTION IAH_rpt_query() 
	...
	prodhist.adj_amt 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IAH","construct-prodhist-1") -- albo kd-505 
...
END FUNCTION
############################################################
# END FUNCTION IAH_rpt_query() 
############################################################

############################################################
# FUNCTION IAH_rpt_process(p_where_text) 
# 
# The report driver
############################################################
FUNCTION IAH_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IAH_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IAH_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH selcurs INTO l_rec_prodhist.*,l_rec_product.*,l_rec_warehouse.* 
		#---------------------------------------------------------
		OUTPUT TO REPORT IAH_rpt_list(l_rpt_idx,l_rec_prodhist.*,l_rec_product.*,l_rec_warehouse.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT IAH_rpt_list
	RETURN rpt_finish("IAH_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION IAH_rpt_process() 
############################################################

############################################################
# REPORT IAH_rpt_list(p_rpt_idx,p_rec_prodhist,p_rec_product,p_rec_warehouse)
#
...
REPORT IAH_rpt_list(p_rpt_idx,p_rec_prodhist,p_rec_product,p_rec_warehouse)
	...
	ON EVERY ROW 
		...
		COLUMN 29, p_rec_prodhist.sales_qty      USING "---,---,--&.&&", 
		COLUMN 44, p_rec_prodhist.credit_qty     USING "---,---,--&.&&", 
		COLUMN 59, p_rec_prodhist.reclassout_qty USING "---,---,--&.&&", 
		COLUMN 74, p_rec_prodhist.reclassin_qty  USING "---,---,--&.&&", 
		COLUMN 89, p_rec_prodhist.pur_qty        USING "---,---,--&.&&", 
		COLUMN 104,p_rec_prodhist.transin_qty    USING "---,---,--&.&&", 
		COLUMN 119,p_rec_prodhist.transout_qty   USING "---,---,--&.&&" 
	...
	AFTER GROUP OF p_rec_prodhist.year_num 
		...
		COLUMN 29, NVL(GROUP SUM(p_rec_prodhist.sales_qty),0)		 USING "---,---,--&.&&", 
		COLUMN 44, NVL(GROUP SUM(p_rec_prodhist.credit_qty),0)	 USING "---,---,--&.&&", 
		COLUMN 59, NVL(GROUP SUM(p_rec_prodhist.reclassout_qty),0)USING "---,---,--&.&&", 
		COLUMN 74, NVL(GROUP SUM(p_rec_prodhist.reclassin_qty),0) USING "---,---,--&.&&", 
		COLUMN 89, NVL(GROUP SUM(p_rec_prodhist.pur_qty),0) 		 USING "---,---,--&.&&", 
		COLUMN 104,NVL(GROUP SUM(p_rec_prodhist.transin_qty),0)	 USING "---,---,--&.&&", 
		COLUMN 119,NVL(GROUP SUM(p_rec_prodhist.transout_qty),0)	 USING "---,---,--&.&&" 
	...
	ON LAST ROW 
		...
		COLUMN 29, NVL(SUM(p_rec_prodhist.sales_qty),0) 	 USING "---,---,--&.&&", 
		COLUMN 44, NVL(SUM(p_rec_prodhist.credit_qty),0)	 USING "---,---,--&.&&", 
		COLUMN 59, NVL(SUM(p_rec_prodhist.reclassout_qty),0)USING "---,---,--&.&&", 
		COLUMN 74, NVL(SUM(p_rec_prodhist.reclassin_qty),0) USING "---,---,--&.&&", 
		COLUMN 89, NVL(SUM(p_rec_prodhist.pur_qty),0) 		 USING "---,---,--&.&&", 
		COLUMN 104,NVL(SUM(p_rec_prodhist.transin_qty),0)	 USING "---,---,--&.&&", 
		COLUMN 119,NVL(SUM(p_rec_prodhist.transout_qty),0)	 USING "---,---,--&.&&" 
		SKIP 2 LINES 
		#End Of Report

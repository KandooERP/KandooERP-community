...
# FUNCTION IC2_main()
#
# Purpose - Product Pricing by Warehouse Report
############################################################
FUNCTION IC2_main()

	CALL setModuleId("IC2") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Product Pricing" 
				...
				BEFORE MENU 
					CALL publish_toolbar("kandoo","IC2","menu-Product_Pricing-1") -- albo kd-505
					CALL rpt_rmsreps_reset(NULL)
					CALL IC2_rpt_process(IC2_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IC2_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I162a with FORM "I162a" 
			 CALL windecoration_i("I162a") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IC2_rpt_query()) #save where clause in env 
			CLOSE WINDOW I162a 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IC2_rpt_process(get_url_sel_text())
	END CASE

END FUNCTION 
############################################################
# END FUNCTION IC2_main()
############################################################

#####################################################################
# FUNCTION IC2_rpt_query()
...
FUNCTION IC2_rpt_query() 
	...
	prodstatus.last_receipt_date 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IC2","construct-prodstatus-1") -- albo kd-505 
...
END FUNCTION 
#####################################################################
# END FUNCTION IC2_rpt_query()
#####################################################################

#####################################################################
# FUNCTION IC2_rpt_process(p_where_text) 
#
# The report driver
#####################################################################
FUNCTION IC2_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IC2_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IC2_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	"ORDER BY desc_text,prodstatus.part_code,prodstatus.ware_code" 
	...
	FOREACH c_product INTO l_rec_prodstatus.*, l_rec_product.* 
		#---------------------------------------------------------
		OUTPUT TO REPORT IC2_rpt_list(l_rpt_idx,l_rec_prodstatus.*,l_rec_product.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT IC2_rpt_list
	RETURN rpt_finish("IC2_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
#####################################################################
# END FUNCTION IC2_rpt_process(p_where_text) 
#####################################################################

#####################################################################
# REPORT IC2_rpt_list(p_rpt_idx,p_rec_prodstatus,p_rec_product)
#
...
REPORT IC2_rpt_list(p_rpt_idx,p_rec_prodstatus,p_rec_product) 
	...
	ORDER EXTERNAL BY p_rec_product.desc_text,p_rec_prodstatus.part_code,p_rec_prodstatus.ware_code 
	...
	ON EVERY ROW 
		...
		COLUMN 54, p_rec_prodstatus.ware_code CLIPPED, 
		COLUMN 63, p_rec_product.price_uom_code CLIPPED, 
		COLUMN 69, (p_rec_prodstatus.list_amt * l_conv_rate) USING "-----,--&.&&" 

...
# FUNCTION ID2_main()
#
# Purpose - Stock Over-stock Report by Warehouse
############################################################
FUNCTION ID2_main()

	CALL setModuleId("ID2") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Over-stock by Warehouse" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","ID2","menu-Over-stock-1") -- albo kd-505
					CALL rpt_rmsreps_reset(NULL)
					CALL ID2_rpt_process(ID2_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL ID2_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I175 with FORM "I175" 
			 CALL windecoration_i("I175") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(ID2_rpt_query()) #save where clause in env 
			CLOSE WINDOW I175 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL ID2_rpt_process(get_url_sel_text())
	END CASE	

END FUNCTION 
############################################################
# END FUNCTION ID2_main()
############################################################

############################################################
# FUNCTION ID2_rpt_query() 
...
FUNCTION ID2_rpt_query() 
	...
	product.ref8_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","ID2","construct-product-1") -- albo kd-505 
...
END FUNCTION 
############################################################
# END FUNCTION ID2_rpt_query() 
############################################################

############################################################
# FUNCTION ID2_rpt_process(p_where_text) 
# 
# The report driver
############################################################
FUNCTION ID2_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"ID2_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT ID2_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH c_prodstatus INTO l_rec_prodstatus.*,l_rec_product.* 
		...
		IF l_rec_prodstatus.onhand_qty > l_required_qty THEN 
			#---------------------------------------------------------
			OUTPUT TO REPORT ID2_rpt_list(l_rpt_idx,l_rec_prodstatus.*,l_rec_product.*,l_required_qty) 
	...
	#------------------------------------------------------------
	FINISH REPORT ID2_rpt_list
	RETURN rpt_finish("ID2_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION ID2_rpt_process() 
############################################################

############################################################
# REPORT ID2_rpt_list(p_rpt_idx,p_rec_prodstatus,p_rec_product,p_required_qty)
#
# Report Definition/Layout
############################################################
REPORT ID2_rpt_list(p_rpt_idx,p_rec_prodstatus,p_rec_product,p_rec_required_qty) 
	DEFINE p_rpt_idx SMALLINT #array index for glob_arr_rmsreps[p_rpt_idx]
	DEFINE p_rec_prodstatus RECORD LIKE prodstatus.*
	DEFINE p_rec_product RECORD LIKE product.* 
 	DEFINE p_rec_required_qty LIKE prodstatus.onhand_qty
	DEFINE l_rec_warehouse RECORD LIKE warehouse.* 
	DEFINE l_over_stock_qty LIKE prodstatus.onhand_qty 
	DEFINE l_over_days_num INTEGER 

	ORDER BY EXTERNAL p_rec_prodstatus.ware_code, p_rec_prodstatus.part_code 
	...
	ON EVERY ROW 
		...
		END IF 
		LET l_over_stock_qty = (p_rec_prodstatus.onhand_qty - p_rec_required_qty) 
		IF p_rec_required_qty != 0 THEN 
			LET l_over_days_num = (l_over_stock_qty / p_rec_required_qty) / p_rec_product.days_lead_num 
		ELSE 
		...
		COLUMN 54, p_rec_prodstatus.onhand_qty USING "----,--&.&&", 
		COLUMN 66, p_rec_product.sell_uom_code CLIPPED, 
		COLUMN 75, p_rec_required_qty   USING "----,--&.&&", 
		COLUMN 87, p_rec_product.sell_uom_code CLIPPED, 
		COLUMN 96, l_over_stock_qty USING "----,--&.&&", 
		COLUMN 108,p_rec_product.sell_uom_code CLIPPED, 
		COLUMN 113,l_over_days_num USING "-------&"

...
# FUNCTION ID1_main()
#
# Purpose - Stock Replenishment Report by Warehouse
############################################################
FUNCTION ID1_main()

	CALL setModuleId("ID1") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Replenishment by Warehouse" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","ID1","menu-Replenishment-1") -- albo kd-505
					CALL rpt_rmsreps_reset(NULL)
					CALL ID1_rpt_process(ID1_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL ID1_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I175 with FORM "I175" 
			 CALL windecoration_i("I175") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(ID1_rpt_query()) #save where clause in env 
			CLOSE WINDOW I175 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL ID1_rpt_process(get_url_sel_text())
	END CASE	

END FUNCTION 
############################################################
# END FUNCTION ID1_main()
############################################################

############################################################
# FUNCTION ID1_rpt_query() 
...
FUNCTION ID1_rpt_query() 
	...
	product.ref8_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","ID1","construct-product-1") -- albo kd-505 
...
END FUNCTION 
############################################################
# END FUNCTION ID1_rpt_query() 
############################################################

############################################################
# FUNCTION ID1_rpt_process(p_where_text) 
# 
# The report driver
############################################################
FUNCTION ID1_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"ID1_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT ID1_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH c_prodstatus INTO l_rec_prodstatus.*, l_rec_product.* 
		...
		IF l_required_qty > l_rec_prodstatus.onhand_qty THEN 
			#---------------------------------------------------------
			OUTPUT TO REPORT ID1_rpt_list(l_rpt_idx,l_rec_prodstatus.*,l_rec_product.*,l_required_qty) 
	...
	#------------------------------------------------------------
	FINISH REPORT ID1_rpt_list
	RETURN rpt_finish("ID1_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION ID1_rpt_process() 
############################################################

############################################################
# REPORT ID1_rpt_list(p_rpt_idx,p_rec_prodstatus,p_rec_product,p_required_qty)
#
...
REPORT ID1_rpt_list(p_rpt_idx,p_rec_prodstatus,p_rec_product,p_required_qty) 
	...
	DEFINE l_reorder_qty LIKE prodstatus.onhand_qty 
	DEFINE l_stk_pur_con_qty LIKE product.pur_stk_con_qty 
	...
	ON EVERY ROW 
		...
		END IF 
		LET l_stk_pur_con_qty = 1 / p_rec_product.stk_sel_con_qty / p_rec_product.pur_stk_con_qty 
		LET l_reorder_qty = (p_required_qty - p_rec_prodstatus.onhand_qty)	* l_stk_pur_con_qty 
		...
		COLUMN 75, p_required_qty    USING "----,--&.&&", 
		COLUMN 87, p_rec_product.sell_uom_code CLIPPED, 
		COLUMN 96, l_stk_pur_con_qty USING "---.---&", 
		COLUMN 109,l_reorder_qty     USING "----,--&.&&", 
		COLUMN 121,p_rec_product.pur_uom_code CLIPPED 

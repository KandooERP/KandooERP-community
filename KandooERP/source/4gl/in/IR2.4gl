...
# FUNCTION IR2_main()
#
# Purpose - Period Activity by Reference Report
############################################################
FUNCTION IR2_main()

	CALL setModuleId("IR2") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Period Activity by Reference" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","IR2","menu-Period Activity-1") -- albo kd-505
					CALL rpt_rmsreps_reset(NULL)
					CALL IR2_rpt_process(IR2_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IR2_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I112 with FORM "I112" 
			 CALL windecoration_i("I112") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IR2_rpt_query()) #save where clause in env 
			CLOSE WINDOW I112 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IR2_rpt_process(get_url_sel_text())
	END CASE

END FUNCTION 
############################################################
# END FUNCTION IR2_main()
############################################################

############################################################
# FUNCTION IR2_rpt_query() 
...
FUNCTION IR2_rpt_query() 
	...
	prodledg.sales_amt 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IR2","construct-prodledg-1") -- albo kd-505 
...
END FUNCTION 
############################################################
# END FUNCTION IR2_rpt_query() 
############################################################

############################################################
# FUNCTION IR2_rpt_process(p_where_text) 
# 
# The report driver
############################################################
FUNCTION IR2_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IR2_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IR2_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	"ORDER BY prodledg.source_text,prodledg.trantype_ind,prodledg.tran_date" 
	...
	FOREACH selcurs INTO l_rec_prodledg.* 
		...
		#---------------------------------------------------------
		OUTPUT TO REPORT IR2_rpt_list(l_rpt_idx,l_rec_prodledg.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT IR2_rpt_list
	RETURN rpt_finish("IR2_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION IR2_rpt_process() 
############################################################

############################################################
# REPORT IR2_rpt_list(p_rpt_idx,p_rec_prodledg)
#
...
REPORT IR2_rpt_list(p_rpt_idx,p_rec_prodledg) 
	...
	DEFINE p_rec_prodledg RECORD 
		...
		eprice MONEY(12,2) 
	END RECORD 
	DEFINE whatitis CHAR(30) 

	ORDER EXTERNAL BY p_rec_prodledg.source_text,p_rec_prodledg.trantype_ind,p_rec_prodledg.tran_date 
	...
	BEFORE GROUP OF p_rec_prodledg.source_text 
		SKIP 1 LINE 
		PRINT COLUMN 1, "Reference: ", p_rec_prodledg.source_text  CLIPPED 

	BEFORE GROUP OF p_rec_prodledg.trantype_ind 
		CASE (p_rec_prodledg.trantype_ind) 
			WHEN "A" LET whatitis = "Adjustments" 
			WHEN "B" LET whatitis = "Backorders" 
			WHEN "S" LET whatitis = "Sales" 
			WHEN "C" LET whatitis = "Credits" 
			WHEN "R" LET whatitis = "Returns" 
			WHEN "T" LET whatitis = "Transfers" 
			WHEN "P" LET whatitis = "Purchases" 
		END CASE 
		PRINT COLUMN 1, "Activity: ", whatitis CLIPPED
	...
	ON EVERY ROW 
		...
		COLUMN 111,p_rec_prodledg.part_code CLIPPED, 
		COLUMN 127,p_rec_prodledg.ware_code CLIPPED 

	AFTER GROUP OF p_rec_prodledg.source_text 
		PRINT COLUMN 35, "---------------------------------------------", 
		"------------------------------" 
		PRINT COLUMN 5, "Total Reference: ", 

	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Detailed Stock Valuation" 
				...
				BEFORE MENU 
					CALL publish_toolbar("kandoo","IF5","menu-Summary Stock-1") -- albo kd-505
					CALL rpt_rmsreps_reset(NULL)
					CALL IF5_rpt_process(IF5_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IF5_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I206 with FORM "I206" 
			 CALL windecoration_i("I206") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IF5_rpt_query()) #save where clause in env 
			CLOSE WINDOW I206 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IF5_rpt_process(get_url_sel_text())
	END CASE

END FUNCTION 
############################################################
# END FUNCTION IF5_main()
############################################################ 

############################################################
# FUNCTION IF5_rpt_query() 
...
FUNCTION IF5_rpt_query() 
	...
	DIALOG ATTRIBUTES(UNBUFFERED)
		...
		FROM book_tax,start_date,end_date 
			BEFORE INPUT 
				CALL publish_toolbar("kandoo","IF5","input-pr_book_tax-1") -- albo kd-505
		...
		ware_code 
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","IF5","construct-product-1") -- albo kd-505
				MESSAGE " Enter criteria FOR selection - ESC TO begin search"
		END CONSTRUCT 
...
END FUNCTION
############################################################
# END FUNCTION IF5_rpt_query() 
############################################################

############################################################
# FUNCTION IF5_rpt_process(p_where_text) 
# 
# The report driver
############################################################
FUNCTION IF5_rpt_process(p_where_text)
	...
	IF modu_book_tax = "B" THEN 
		LET l_rpt_idx = rpt_start(getmoduleid(),"IF5_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	ELSE 
		LET l_rpt_idx = rpt_start(getmoduleid()||".","IF5_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	END IF

	START REPORT IF5_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH selcurs INTO l_rec_product.*,l_rec_prodstatus.*,l_rec_costledg.* 
		#---------------------------------------------------------
		OUTPUT TO REPORT IF5_rpt_list(l_rpt_idx,l_rec_product.*,l_rec_prodstatus.*,l_rec_costledg.*,modu_book_tax) 
	...
	#------------------------------------------------------------
	FINISH REPORT IF5_rpt_list
	RETURN rpt_finish("IF5_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION IF5_rpt_process() 
############################################################

############################################################
# REPORT IF5_rpt_list(p_rpt_idx,p_rec_product,p_rec_prodstatus,p_rec_costledg,p_book_tax)
#
...
REPORT IF5_rpt_list(p_rpt_idx,p_rec_product,p_rec_prodstatus,p_rec_costledg,p_book_tax) 
	...
	DEFINE l_tot_val   MONEY(16,4)
	DEFINE l_oh        MONEY(16,4)
	DEFINE l_curr      MONEY(16,4)
	DEFINE l_wo        MONEY(16,4)
	DEFINE l_pwo       MONEY(16,4)
	DEFINE l_val       MONEY(16,4)
	...
	AFTER GROUP OF p_rec_product.cat_code 
		IF p_book_tax = "B" THEN 
			...
			LET l_wo = NVL(GROUP SUM(p_rec_costledg.onhand_qty * p_rec_costledg.curr_wo_amt),0) 
			LET l_pwo = NVL(GROUP SUM(p_rec_costledg.onhand_qty * p_rec_costledg.prev_wo_amt),0) 
			LET l_val = l_curr + l_wo + l_pwo 
			PRINT COLUMN 01, "Category: ", 
			COLUMN 12, pr_category.cat_code CLIPPED, 
			COLUMN 17, pr_category.desc_text CLIPPED, 
			COLUMN 57, l_oh    USING "-------&", 
			COLUMN 66, l_curr  USING "----,---,--&.&&", 
			COLUMN 83, l_wo    USING "----,---,--&.&&", 
			COLUMN 100,l_pwo   USING "----,---,--&.&&", 
			COLUMN 117,l_val   USING "----,---,--&.&&" 
		...
		ELSE 
			...
			LET l_wo = NVL(GROUP SUM(p_rec_costledg.onhand_qty * p_rec_costledg.tax_wo_amt),0) 
			LET l_pwo = NVL(GROUP SUM(p_rec_costledg.onhand_qty * p_rec_costledg.prev_tax_wo_amt),0) 
			LET l_val = l_curr + l_wo + l_pwo 
			PRINT COLUMN 01, "Category: ", 
			COLUMN 12, pr_category.cat_code CLIPPED, 
			COLUMN 17, pr_category.desc_text CLIPPED, 
	...
	ON LAST ROW 
		...
		COLUMN 57, l_tot_oh   USING "-------&", 
		COLUMN 66, l_tot_curr USING "----,---,--&.&&", 
		COLUMN 83, l_tot_wo   USING "----,---,--&.&&", 
		COLUMN 100,l_tot_pwo  USING "----,---,--&.&&", 
		COLUMN 117,l_tot_val  USING "----,---,--&.&&" 
		SKIP 2 LINES 
		#End Of Report

...
FUNCTION IF0_main() 
	...
	CALL setModuleId("IF0")

	LET modu_report_run_once_code = get_url_module_child()
	IF modu_report_run_once_code IS NOT NULL THEN 
		CALL IF0_run_module(modu_report_run_once_code)
		RETURN 
	ELSE
		LET l_list_menuchoice = "IF4"
		LET l_inp_menuchoice = l_list_menuchoice 
	END IF
	 
	OPEN WINDOW IF00 WITH FORM "IF00" ATTRIBUTE(BORDER,STYLE="full-screen")
	 CALL windecoration_i("IF00") 
	...
	DIALOG ATTRIBUTES(UNBUFFERED) 
		...
		ON ACTION ("ACCEPT","DoubleClickItem") 
			...
			DISPLAY l_list_menuchoice TO tf_menuChoice		
			CALL IF0_run_module(l_list_menuchoice)
			IF int_flag THEN
...
END FUNCTION 
############################################################
# END FUNCTION IF0_main()
############################################################

############################################################
# FUNCTION IF0_run_module(p_module_id)
#
...
FUNCTION IF0_run_module(p_module_id)
	...
	CASE p_module_id.toUpperCase()
		...
		WHEN "IF8" 
			CALL IF8_main()
		OTHERWISE # 
			CALL fgl_winmessage("Invalid Menu Option in IF","Invalid Menu Option choosen","error") 
	END CASE
	 	
	CALL setModuleId("IF0") #Reset program module id
	CALL rpt_rmsreps_reset(NULL)

END FUNCTION
############################################################
# END FUNCTION IF0_run_module(p_module_id)
############################################################
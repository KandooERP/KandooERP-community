...
# FUNCTION IB9_main()
#
# Purpose - Bottom 100 Products Report
############################################################
FUNCTION IB9_main()

 	CALL setModuleId("IB9") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Bottom 100 Products" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","IB9","menu-Product_Status-1") -- albo kd-505
					CALL rpt_rmsreps_reset(NULL)
					CALL IB9_rpt_process(IB9_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IB9_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I163 with FORM "I163" 
			 CALL windecoration_i("I163") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IB9_rpt_query()) #save where clause in env 
			CLOSE WINDOW I163 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IB9_rpt_process(get_url_sel_text())
	END CASE	

END FUNCTION
############################################################
# END FUNCTION IB9_main()
############################################################

############################################################
# FUNCTION IB9_rpt_query() 
...
FUNCTION IB9_rpt_query() 
	...
	prodledg.period_num 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IB9","construct-product-1") -- albo kd-505 
...
END FUNCTION
############################################################
# END FUNCTION IB9_rpt_query() 
############################################################
 
############################################################
# FUNCTION IB9_rpt_process(p_where_text) 
#
# The report driver
############################################################
FUNCTION IB9_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IB9_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IB9_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOR i=1 TO 100 
		...
		IF l_arr_prodarray[i].gross_num > 0 THEN 
			#---------------------------------------------------------
			OUTPUT TO REPORT IB9_rpt_list (l_rpt_idx,l_arr_prodarray[i].*) 
	...
	#------------------------------------------------------------
	FINISH REPORT IB9_rpt_list
	RETURN rpt_finish("IB9_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION IB9_rpt_process(p_where_text) 
############################################################

############################################################
# REPORT IB9_rpt_list(p_rpt_idx,p_rec_prodledg)
#

...
# RETURN VOID
#
# Purpose - Product List by Inventory Class Report
############################################################
FUNCTION IA3_main()

	CALL setModuleId("IA3") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Product List by Inventory Class" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","IA3","menu-Product_Report-1") -- albo kd-505 
					CALL rpt_rmsreps_reset(NULL)
					CALL IA3_rpt_process(IA3_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IA3_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I160 with FORM "I160" 
			 CALL windecoration_i("I160") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IA3_rpt_query()) #save where clause in env 
			CLOSE WINDOW I160 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IA3_rpt_process(get_url_sel_text())
	END CASE	

END FUNCTION 
############################################################
# END FUNCTION IA3_main() 
############################################################

############################################################
# FUNCTION IA3_rpt_query() 
...
FUNCTION IA3_rpt_query() 
	...
	product.sell_uom_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IA3","construct-product-1") -- albo kd-505 
...
END FUNCTION 
############################################################
# END FUNCTION IA3_rpt_query() 
############################################################

############################################################
# FUNCTION IA3_rpt_process(p_where_text) 
# 
# The report driver
############################################################
FUNCTION IA3_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IA3_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IA3_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH selcurs INTO l_rec_product.* 
		#---------------------------------------------------------
		OUTPUT TO REPORT IA3_rpt_list(l_rpt_idx,l_rec_product.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT IA3_rpt_list
	RETURN rpt_finish("IA3_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION IA3_rpt_process() 
############################################################

############################################################
# REPORT IA3_rpt_list(p_rpt_idx,p_rec_product)
#
...
REPORT IA3_rpt_list(p_rpt_idx,p_rec_product)
	...
	ORDER EXTERNAL BY p_rec_product.class_code,p_rec_product.part_code 
	...
	BEFORE GROUP OF p_rec_product.class_code 
		...
		COLUMN 8, p_rec_product.class_code CLIPPED, 
		COLUMN 17, l_class_text  CLIPPED
		SKIP 1 LINE 
	...
	ON EVERY ROW 
		...
		COLUMN 57, p_rec_product.cat_code CLIPPED, 
		COLUMN 61, l_category_text CLIPPED, 
		COLUMN 95, p_rec_product.pur_uom_code CLIPPED, 
		COLUMN 103,p_rec_product.pur_stk_con_qty USING "####&.&&", 
		COLUMN 112,p_rec_product.stock_uom_code CLIPPED, 
		COLUMN 120,p_rec_product.stk_sel_con_qty USING "####&.&&", 
		COLUMN 129,p_rec_product.sell_uom_code CLIPPED 

...
# FUNCTION IB4_main()
#
# Purpose - Minimum Reorder Report
############################################################
FUNCTION IB4_main()

	CALL setModuleId("IB4") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Minimum Reorder" 
				...
				BEFORE MENU 
					CALL publish_toolbar("kandoo","IB4","menu-Minimum_Reorder-1") -- albo kd-505
					CALL rpt_rmsreps_reset(NULL)
					CALL IB4_rpt_process(IB4_rpt_query())
		...
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IB4_rpt_process(get_url_sel_text())
	END CASE	

END FUNCTION 
############################################################
# END FUNCTION IB4_main()
############################################################

############################################################
# FUNCTION IB4_rpt_query() 
...
FUNCTION IB4_rpt_query() 
	...
	prodstatus.abc_ind 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IB4","construct-product-1") -- albo kd-505 
...
############################################################
# FUNCTION IB4_rpt_process(p_where_text)
#
# The report driver
############################################################
FUNCTION IB4_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IB4_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IB4_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH selcurs INTO l_rec_report_line.* 
		...
		- l_rec_report_line.forward_qty) > 
		l_rec_report_line.reorder_point_qty THEN 
			LET l_rec_report_line.reorder_qty = 0 
		END IF 
		#---------------------------------------------------------
		OUTPUT TO REPORT IB4_rpt_list (l_rpt_idx,l_rec_report_line.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT IB4_rpt_list
	RETURN rpt_finish("IB4_rpt_list")
	#------------------------------------------------------------
...
############################################################
# REPORT IB4_rpt_list(p_rpt_idx,p_rec_report_line)
#
...
REPORT IB4_rpt_list(p_rpt_idx,p_rec_report_line) 
	...
	DEFINE l_tot_for_avail MONEY(15,2)
	DEFINE l_tot_avail MONEY(15,2)
	...
	ON EVERY ROW 
		LET p_rec_report_line.onhand_qty = p_rec_report_line.onhand_qty 
		...
		- p_rec_report_line.forward_qty 
		PRINT COLUMN 1, p_rec_report_line.ware_code, 
		COLUMN 12, p_rec_report_line.onhand_qty USING "----,--&.&&", 
		COLUMN 29, p_rec_report_line.back_qty USING "----,--&.&&", 
		COLUMN 46, p_rec_report_line.onord_qty	USING "----,--&.&&", 
		COLUMN 63, p_rec_report_line.forward_qty USING "----,--&.&&", 
		COLUMN 81, l_for_avail USING "----,--&.&&", 
		COLUMN 100, p_rec_report_line.reorder_point_qty	USING "----,--&.&&", 
		COLUMN 118, p_rec_report_line.reorder_qty USING "----,--&.&&" 
	...
	AFTER GROUP OF p_rec_report_line.part_code 
		...
		COLUMN 12, GROUP sum(p_rec_report_line.onhand_qty - p_rec_report_line.reserved_qty)	USING "----,--&.&&", 
		COLUMN 29, GROUP sum(p_rec_report_line.back_qty) USING "----,--&.&&", 
		COLUMN 46, GROUP sum(p_rec_report_line.onord_qty) USING "----,--&.&&", 
		COLUMN 63, GROUP sum(p_rec_report_line.forward_qty) USING "----,--&.&&", 
		COLUMN 81, l_tot_for_avail USING "----,--&.&&", 
		COLUMN 100, GROUP sum(p_rec_report_line.reorder_point_qty) USING "----,--&.&&", 
		COLUMN 118, GROUP sum(p_rec_report_line.reorder_qty) USING "----,--&.&&" 
		SKIP 1 line 

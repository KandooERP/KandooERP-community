...
# RETURN VOID
#
# Purpose - Product Movement Ledger Report
############################################################
FUNCTION IAD_main()

	CALL setModuleId("IAD") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Product Movement Ledger" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","IAD","menu-Product_Ledger-1") -- albo kd-505 
					CALL rpt_rmsreps_reset(NULL)
					CALL IAD_rpt_process(IAD_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IAD_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I161 with FORM "I161" 
			 CALL windecoration_i("I161") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IAD_rpt_query()) #save where clause in env 
			CLOSE WINDOW I161 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IAD_rpt_process(get_url_sel_text())
	END CASE	

END FUNCTION 
############################################################
# END FUNCTION IAD_main() 
############################################################

############################################################
# FUNCTION IAD_rpt_query() 
...
FUNCTION IAD_rpt_query() 
	...
	prodledg.sales_amt 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IAD","construct-prodledg-1") -- albo kd-505 
...
END FUNCTION 
############################################################
# END FUNCTION IAD_rpt_query() 
############################################################

############################################################
# FUNCTION IAD_rpt_process(p_where_text) 
# 
# The report driver
############################################################
FUNCTION IAD_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IAD_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IAD_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	"ORDER BY prodledg.part_code,prodledg.ware_code,prodledg.seq_num" 
	...
	FOREACH selcurs INTO l_rec_prodledg.*,l_rec_product.*,l_rec_warehouse.* 
		...
		END IF 
		#---------------------------------------------------------
		OUTPUT TO REPORT IAD_rpt_list(l_rpt_idx,l_rec_prodledg.*,l_rec_product.*,l_rec_warehouse.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT IAD_rpt_list
	RETURN rpt_finish("IAD_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION IAD_rpt_process() 
############################################################

############################################################
# REPORT IAD_rpt_list(p_rpt_idx,p_rec_prodledg,p_rec_product,p_rec_warehouse)
#
...
REPORT IAD_rpt_list(p_rpt_idx,p_rec_prodledg,p_rec_product,p_rec_warehouse)
	...
	ORDER EXTERNAL BY p_rec_prodledg.part_code,p_rec_prodledg.ware_code,p_rec_prodledg.seq_num 
	...
	BEFORE GROUP OF p_rec_prodledg.ware_code 
		NEED 4 LINES 
		SKIP 1 LINE 
		PRINT COLUMN 1, "Warehouse: ", p_rec_prodledg.ware_code CLIPPED, 
	...
	ON EVERY ROW 
		...
		COLUMN 34, p_rec_prodledg.source_num USING "#######&", 
		COLUMN 43, p_rec_prodledg.tran_qty   USING "--,---,---,--&.&&&", 
		COLUMN 62, p_rec_prodledg.bal_amt    USING "--,---,---,--&.&&&" 

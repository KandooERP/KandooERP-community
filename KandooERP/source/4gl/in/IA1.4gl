...
# RETURN VOID
#
# Purpose - Product Detail List Report
############################################################
FUNCTION IA1_main()

	CALL setModuleId("IA1") 
	...
	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			...
			MENU "Product Report" 
				...
				BEFORE MENU 
					CALL publish_toolbar("kandoo","IA1","menu-Product_Report") -- albo kd-505 
					CALL rpt_rmsreps_reset(NULL)
					CALL IA1_rpt_process(IA1_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IA1_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I160 with FORM "I160" 
			 CALL windecoration_i("I160") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IA1_rpt_query()) #save where clause in env 
			CLOSE WINDOW I160 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IA1_rpt_process(get_url_sel_text())
	END CASE	

END FUNCTION 
############################################################
# END FUNCTION IA1_main() 
############################################################

############################################################
# FUNCTION IA1_rpt_query() 
...
FUNCTION IA1_rpt_query() 
	...
	product.sell_uom_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IA1","construct-product-1") -- albo kd-505
...
END FUNCTION 
############################################################
# END FUNCTION IA1_rpt_query() 
############################################################

############################################################
# FUNCTION IA1_rpt_process(p_where_text) 
# 
# The report driver
############################################################
FUNCTION IA1_rpt_process(p_where_text)
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IA1_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IA1_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	FOREACH selcurs INTO l_rec_product.* 
		#---------------------------------------------------------
		OUTPUT TO REPORT IA1_rpt_list(l_rpt_idx,l_rec_product.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT IA1_rpt_list
	RETURN rpt_finish("IA1_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
############################################################
# END FUNCTION IA1_rpt_process() 
############################################################

############################################################
# REPORT IA1_rpt_list(p_rpt_idx,p_rec_product)
#
...
REPORT IA1_rpt_list(p_rpt_idx,p_rec_product)
	...
	DEFINE l_class_text CHAR(16) 
	DEFINE l_category_text CHAR(16) 
	...
	ON EVERY ROW 
		...
		COLUMN 54, p_rec_product.cat_code CLIPPED, 
		COLUMN 58, l_category_text CLIPPED, 
		COLUMN 75, p_rec_product.class_code CLIPPED, 
		COLUMN 84, l_class_text CLIPPED, 
		COLUMN 101,p_rec_product.pur_uom_code CLIPPED, 
		COLUMN 106,p_rec_product.pur_stk_con_qty USING "####&.&&", 
		COLUMN 115,p_rec_product.stock_uom_code CLIPPED, 

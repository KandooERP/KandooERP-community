...
# FUNCTION IC3_main()
#
# Purpose - Product Pricing Report - FIFO
############################################################
FUNCTION IC3_main() 

	CALL setModuleId("IC3") 

	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN "1" #UI/MENU Mode
			OPEN WINDOW I162a WITH FORM "I162a" 
			 CALL windecoration_i("I162a")
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title
			
			MENU "Product Pricing" 

				BEFORE MENU 
					CALL publish_toolbar("kandoo","IC3","menu-Product_Pricing-1") -- albo kd-505
					CALL rpt_rmsreps_reset(NULL)
					CALL IC3_rpt_process(IC3_rpt_query())
		...
		WHEN "2" #Background Process with rmsreps.report_code
			CALL IC3_rpt_process(NULL)  

		WHEN "3" #Only create query-where-part
			OPEN WINDOW I162a with FORM "I162a" 
			 CALL windecoration_i("I162a") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(IC3_rpt_query()) #save where clause in env 
			CLOSE WINDOW I162a 
			
		WHEN "4" #Background Process with SQL WHERE ARGUMENT
			CALL IC3_rpt_process(get_url_sel_text())
	END CASE
...
FUNCTION IC3_rpt_query() 
	...
	prodstatus.last_receipt_date 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","IC3","construct-prodstatus-1") -- albo kd-505 
...
END FUNCTION 
#####################################################################
# END FUNCTION IC3_rpt_query()
#####################################################################

#####################################################################
# FUNCTION IC3_rpt_process(p_where_text) 
#
# The report driver
#####################################################################
FUNCTION IC3_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"IC3_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	

	START REPORT IC3_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
	...
	LET l_query_text = 
	"SELECT prodstatus.*,product.*,costledg.* ", 
	"FROM prodstatus,product,costledg ", 
	"WHERE prodstatus.cmpy_code = '",glob_rec_kandoouser.cmpy_code,"' ", 
	"AND product.cmpy_code = prodstatus.cmpy_code ", 
	"AND costledg.cmpy_code =product.cmpy_code ", 
	...
	"ORDER BY product.cat_code,prodstatus.part_code,prodstatus.ware_code,costledg.seq_num" 

	PREPARE s_product FROM l_query_text 
	DECLARE c_product CURSOR FOR s_product 

	FOREACH c_product INTO l_rec_prodstatus.*,l_rec_product.*,l_rec_costledg.* 
		#---------------------------------------------------------
		OUTPUT TO REPORT IC3_rpt_list(l_rpt_idx,l_rec_prodstatus.*,l_rec_product.*,l_rec_costledg.*) 
	...
	#------------------------------------------------------------
	FINISH REPORT IC3_rpt_list
	RETURN rpt_finish("IC3_rpt_list")
	#------------------------------------------------------------

END FUNCTION 
#####################################################################
# END FUNCTION IC3_rpt_process(p_where_text) 
#####################################################################

#####################################################################
# REPORT IC3_rpt_list(p_rpt_idx,p_rec_prodstatus,p_rec_product,p_rec_costledg)
#
...
REPORT IC3_rpt_list(p_rpt_idx,p_rec_prodstatus,p_rec_product,p_rec_costledg) 
	...
	DEFINE l_rec_conv_rate LIKE uomconv.conversion_qty 

	ORDER EXTERNAL BY p_rec_product.cat_code,p_rec_prodstatus.part_code,p_rec_prodstatus.ware_code,p_rec_costledg.seq_num 
	...
	ON EVERY ROW 
		IF p_rec_product.sell_uom_code = p_rec_product.price_uom_code THEN 
			LET l_rec_conv_rate = 1 
		ELSE 
			LET l_rec_conv_rate = get_uom_conversion_factor(glob_rec_kandoouser.cmpy_code,p_rec_product.part_code,p_rec_product.sell_uom_code,p_rec_product.price_uom_code,2) 
			IF l_rec_conv_rate < 0 THEN 
				LET l_rec_conv_rate = 0 
			END IF 
		END IF 
		PRINT COLUMN 09, p_rec_prodstatus.part_code CLIPPED, 
		COLUMN 30, p_rec_prodstatus.ware_code CLIPPED, 
		COLUMN 36, (p_rec_prodstatus.list_amt * l_rec_conv_rate)	USING "-,---,--&.&&", 
		COLUMN 52, p_rec_product.price_uom_code CLIPPED, 
		COLUMN 58, p_rec_costledg.onhand_qty     USING "-------&", 

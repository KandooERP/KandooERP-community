...
###########################################################################
# FUNCTION sper_wtargets(p_cmpy_code,p_sale_code,p_year_num) 
#
#
###########################################################################
FUNCTION sper_wtargets(p_cmpy_code,p_sale_code,p_year_num) 
	...
	IF l_rec_salesperson.sale_code IS NULL THEN  
		OPEN WINDOW E249 with FORM "E249" 
		 CALL windecoration_e("E249")
		...
		WHILE TRUE 
			...
			FOREACH c_statint INTO l_rec_statint.* 
				...
				WHERE cmpy_code = p_cmpy_code 
				AND bdgt_type_ind = "4" 
				AND bdgt_type_code = p_sale_code 
				AND bdgt_ind = "1" 
				AND year_num = l_rec_statint.year_num 
				AND type_code = l_rec_statint.type_code 
				AND int_num = l_rec_statint.int_num 
				...
				IF l_rec_statint.int_num <= glob_rec_statparms.week_num THEN 
			...
			ELSE 
				...
				MESSAGE kandoomsg2("E",1104,"")	#1104 RETURN TO View Daily Sales - F9 Previous Year - F10 Next Year
 				DISPLAY ARRAY l_arr_rec_statsper TO sr_statsper.* ATTRIBUTE(UNBUFFERED) 
					BEFORE DISPLAY 
						CALL publish_toolbar("kandoo","EC5a","input-arr-l_arr_rec_statsper-1") -- albo kd-502 
...
END FUNCTION 
###########################################################################
# END FUNCTION sper_wtargets(p_cmpy_code,p_sale_code,p_year_num) 
###########################################################################
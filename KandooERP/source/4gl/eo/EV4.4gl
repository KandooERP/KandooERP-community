...
# FUNCTION EV4_main() 
#
# EV4 Customer Yearly Turnover by Salesperson
###########################################################################
FUNCTION EV4_main() 
	DEFER QUIT 
	DEFER INTERRUPT 

	CALL setModuleId("EV4") 

	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 
			OPEN WINDOW E281 with FORM "E281" 
			 CALL windecoration_e("E281") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title
			DISPLAY getmenuitemlabel(NULL) TO header_text 
		
			MENU " Customer Turnover report" 
				BEFORE MENU 
					CALL publish_toolbar("kandoo","EV4","menu-Customer-1") -- albo kd-502
					CALL rpt_rmsreps_reset(NULL)					 
					CALL EV4_rpt_process(EV4_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL EV4_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW E281 with FORM "E281" 
			 CALL windecoration_e("E281") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(EV4_rpt_query()) #save where clause in env 
			CLOSE WINDOW E281 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL EV4_rpt_process(get_url_sel_text())
	END CASE
		 
END FUNCTION 
###########################################################################
# END FUNCTION EV4_main() 
###########################################################################


###########################################################################
# FUNCTION EV4_rpt_query()  
#
# 
###########################################################################
FUNCTION EV4_rpt_query() 
	...
	IF NOT EV4_enter_year() OR int_flag = TRUE THEN
		LET int_flag = FALSE
		RETURN NULL
	END IF
	...
	CONSTRUCT l_where_text ON cust_code, 
	customer.name_text, 
	type_code, 
	customer.sale_code, 
	...
	customer.country_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","EV4","construct-cust_code-1") -- albo kd-502 
...
END FUNCTION
###########################################################################
# END FUNCTION EV4_rpt_query()  
###########################################################################
  

############################################################
# FUNCTION EV4_rpt_process(p_where_text) 
#
#
############################################################
FUNCTION EV4_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"EV4_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT EV4_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
...
--	LET modu_order_ind = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("EV4_rpt_list")].ref5_ind
	LET modu_zero_stats_flag = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("EV4_rpt_list")].ref6_ind
	LET glob_rec_statparms.year_num = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("EV4_rpt_list")].ref5_num 
	#------------------------------------------------------------	


	LET l_query_text = "SELECT customer.*,salesperson.name_text ", 
	...
	"AND customer.sale_code = salesperson.sale_code ", 
	"AND delete_flag = 'N' ", 
	"AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("EV4_rpt_list")].sel_text clipped," ", 
	"ORDER BY customer.cmpy_code,customer.sale_code,cust_code" 
...
--	MESSAGE kandoomsg2("E",1160,"")	#1160 Reporting on Customer...
	FOREACH c_customer INTO l_rec_customer.*,l_rec_salesperson.name_text
	
		#---------------------------------------------------------
		OUTPUT TO REPORT EV4_rpt_list(l_rpt_idx,
		l_rec_customer.*,l_rec_salesperson.name_text) 
	...
	#------------------------------------------------------------
	FINISH REPORT EV4_rpt_list
	CALL rpt_finish("EV4_rpt_list")
	#------------------------------------------------------------
...
END FUNCTION 
############################################################
# END FUNCTION EV4_rpt_process(p_where_text) 
############################################################


###########################################################################
# FUNCTION EV4_enter_year()  
#
# 
###########################################################################
FUNCTION EV4_enter_year() 
	...
	ATTRIBUTE(UNBUFFERED)
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","EV4","input-year_num-1") -- albo kd-502 
...
END FUNCTION 
###########################################################################
# END FUNCTION EV4_enter_year() 
###########################################################################


###########################################################################
# REPORT EV4_rpt_list(p_rec_customer,p_rec_sper_name) 
#
# 
###########################################################################
REPORT EV4_rpt_list(p_rpt_idx,p_rec_customer,p_rec_sper_name) 
	DEFINE p_rpt_idx SMALLINT #array index for glob_arr_rmsreps[p_rpt_idx]	 
	DEFINE p_rec_customer RECORD LIKE customer.*
	DEFINE p_rec_sper_name LIKE salesperson.name_text
	...
	DEFINE l_no_cust_printed SMALLINT 
	DEFINE i SMALLINT 

	OUTPUT 

	ORDER external BY p_rec_customer.cmpy_code, 
	p_rec_customer.sale_code, 
	...
	FORMAT 
		PAGE HEADER 
			...
			COLUMN 20,p_rec_sper_name 
			SKIP 1 line 
			
		BEFORE GROUP OF p_rec_customer.sale_code 
			SKIP TO top OF PAGE 
			LET l_no_cust_printed = TRUE 
			
		AFTER GROUP OF p_rec_customer.sale_code 
			IF l_no_cust_printed THEN 
				PRINT COLUMN 10, "No customer statistics FOR selected salesperson" 
			END IF
...
END REPORT
###########################################################################
# END REPORT EV4_rpt_list(p_rec_customer,p_rec_sper_name) 
###########################################################################
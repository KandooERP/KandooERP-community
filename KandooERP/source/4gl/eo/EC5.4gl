...
GLOBALS "../eo/EC5_GLOBALS.4gl"
###########################################################################
# FUNCTION EC5_main()
#
# EC5 - allows users TO SELECT a salesperson TO which peruse
#       Weekly Turnover vs Target information FROM statistics tables.
###########################################################################
FUNCTION EC5_main() 
	...
	CALL setModuleId("EC5") 
	 
	LET l_arg_sale_code = get_url_sale_code()
	IF l_arg_sale_code IS NOT NULL THEN
		CALL sper_wtargets(glob_rec_kandoouser.cmpy_code,l_arg_sale_code,"") 
	ELSE 
...
FUNCTION db_salesperson_get_datasource(p_filter)
	...
	IF p_filter THEN
		...
		CONSTRUCT BY NAME l_where_text ON 
			...
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","EC5","construct-sale_code-1") 
...
FUNCTION scan_sale() 
	...
	MESSAGE kandoomsg2("E",1103,"")	#1103 Salesperson Targets - RETURN TO View
	DISPLAY ARRAY l_arr_rec_salesperson TO sr_salesperson.* ATTRIBUTE(UNBUFFERED) 
		BEFORE DISPLAY 
			CALL publish_toolbar("kandoo","EC5","input-arr-l_arr_rec_salesperson-1")  
		...
		ON ACTION ("ACCEPT","DOUBLECLCIK") 
			IF (l_idx > 0) AND (l_idx <= l_arr_rec_salesperson.getSize()) THEN
				CALL sper_wtargets(glob_rec_kandoouser.cmpy_code,l_arr_rec_salesperson[l_idx].sale_code,"")
			END IF 

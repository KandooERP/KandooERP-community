...
GLOBALS "../eo/E74_GLOBALS.4gl"
###########################################################################
# FUNCTION E74_main()
#
# E74 - allows users TO SELECT a sales condition TO peruse
#       turnover vs target information FROM statistics tables.
###########################################################################
FUNCTION E74_main() 
	DEFER QUIT 
	DEFER INTERRUPT 
	
	CALL setModuleId("E74") 
...
--	DEFINE j SMALLINT
	...
	IF p_filter THEN
		...
		CONSTRUCT BY NAME l_where_text ON 
			...
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","E74","construct-cond_code-1") -- albo kd-502 
...
FUNCTION scan_condsale() 
	...
	MESSAGE kandoomsg2("E",1153,"") #1153 " Sales Condition Monthly Sales vs Targets - RETURN TO View "
	DISPLAY ARRAY l_arr_rec_condsale TO sr_condsale.*
		BEFORE DISPLAY
			CALL publish_toolbar("kandoo","E74","input-arr-l_arr_rec_condsale-1") 
		...
		ON ACTION ("ACCEPT","DOUBLECLICK") 
			IF (l_idx > 0) AND (l_idx <= l_arr_rec_condsale.getSize()) THEN
				CALL cond_targets(glob_rec_kandoouser.cmpy_code,l_arr_rec_condsale[l_idx].cond_code) 
			END IF 
...
###########################################################################
# FUNCTION cond_targets(p_cmpy_code,p_cond_code)  
#
#
###########################################################################
FUNCTION cond_targets(p_cmpy_code,p_cond_code) 
	...
	IF status = 0 THEN 
		OPEN WINDOW E259 with FORM "E259" 
		 CALL windecoration_e("E259") -- albo kd-755 
		...
		WHILE TRUE 
			...
			FOREACH c_statint INTO l_rec_statint.* 
				...
				ELSE 
					LET l_arr_rec_statcond[l_idx].disc_per = 100 * (1-(l_arr_rec_statcond[l_idx].net_amt/l_arr_rec_statcond[l_idx].gross_amt)) 
				END IF 

				SELECT bdgt_amt 
				INTO l_arr_rec_statcond[l_idx].bdgt_amt 
				FROM stattarget 
				WHERE cmpy_code = p_cmpy_code 
				AND bdgt_type_ind = "5" 
				AND bdgt_type_code = p_cond_code 
				AND bdgt_ind = "1" 
				AND year_num = l_rec_statint.year_num 
				AND type_code = l_rec_statint.type_code 
				AND int_num = l_rec_statint.int_num 
				IF l_arr_rec_statcond[l_idx].bdgt_amt IS NULL THEN 
					LET l_arr_rec_statcond[l_idx].bdgt_amt = 0 
				END IF 
				IF l_arr_rec_statcond[l_idx].bdgt_amt = 0 THEN 
					LET l_arr_rec_statcond[l_idx].achieve_per = 0 
				ELSE 
					LET l_arr_rec_statcond[l_idx].achieve_per = 100 * (l_arr_rec_statcond[l_idx].net_amt/l_arr_rec_statcond[l_idx].bdgt_amt) 
				END IF 
				...
				LET l_arr_rec_stattotal[1].tot_bdgt_amt = l_arr_rec_stattotal[1].tot_bdgt_amt	+ l_arr_rec_statcond[l_idx].bdgt_amt 
				IF l_rec_statint.int_num <= l_rec_statparms.mth_num THEN 
					...
					LET l_arr_rec_stattotal[2].tot_bdgt_amt = l_arr_rec_stattotal[2].tot_bdgt_amt + l_arr_rec_statcond[l_idx].bdgt_amt 
				END IF 
			END FOREACH 

			IF l_idx = 0 THEN 
				CALL fgl_winmessage("#7086 No statistical information exists",kandoomsg2("E",7086,""),"ERROR")	#7086 No statistical information exists FOR this selection "
				EXIT WHILE 
			ELSE 
				FOR i = 1 TO 2 
				...
				MESSAGE kandoomsg2("E",1154,"")		#1154 Sale Condition Monthly Sales Vs Targets - F9 - F10
				DISPLAY ARRAY l_arr_rec_statcond TO sr_statcond.*
					BEFORE DISPLAY 
						CALL publish_toolbar("kandoo","E74","input-arr-l_arr_rec_statcond-1") -- albo kd-502 
...
END FUNCTION
###########################################################################
# END FUNCTION cond_targets(p_cmpy_code,p_cond_code)  
###########################################################################
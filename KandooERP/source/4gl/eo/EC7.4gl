...
# FUNCTION EC7_main()
#
# EC7 - allows users TO SELECT a salesperson TO which peruse
...
FUNCTION EC7_main() 
	...
	CALL setModuleId("EC7") -- albo 

	LET l_arg_sale_code = get_url_sale_code()
	IF l_arg_sale_code IS NOT NULL THEN
		CALL sper_yr_offer(glob_rec_kandoouser.cmpy_code,l_arg_sale_code) 
	ELSE 
...
FUNCTION db_salesperson_get_datasource(p_filter) 
	...
	IF p_filter THEN
		...
		CONSTRUCT BY NAME l_where_text ON 
			...
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","EC7","construct-sale_code-1") -- albo kd-502 
...
FUNCTION scan_sale() 
	...
	MESSAGE kandoomsg2("E",1126,"") 	#1126 Salesperson Spec Offer Results - RETURN TO View
	DISPLAY ARRAY l_arr_rec_salesperson TO sr_salesperson.* 
		BEFORE DISPLAY 
			CALL publish_toolbar("kandoo","EC7","input-arr-l_arr_rec_salesperson-1")
		...
		ON ACTION ("ACCEPT","DOUBLECLICK") --BEFORE FIELD sale_code
			IF (l_idx > 0) AND (l_idx <= l_arr_rec_salesperson.getSize()) THEN
				CALL sper_yr_offer(glob_rec_kandoouser.cmpy_code,l_arr_rec_salesperson[l_idx].sale_code)
			END IF 

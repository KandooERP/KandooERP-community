...
# FUNCTION EC2_main()
#
# EC2 - allows users TO SELECT a salesperson TO which peruse
...
FUNCTION EC2_main() 
	...
	ELSE 
		...
		CALL salesperson_daily_sales() 
		 
		CLOSE WINDOW E184 
	END IF 
END FUNCTION
###########################################################################
# END FUNCTION EC2_main()
###########################################################################
...
FUNCTION db_salesperson_get_datasource(p_filter)
	...
	IF p_filter THEN
		...
		CONSTRUCT BY NAME l_where_text ON 
			...
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","EC2","construct-sale_code-1") -- albo kd-502 
...
###########################################################################
# FUNCTION salesperson_daily_sales() 
#
#
###########################################################################
FUNCTION salesperson_daily_sales() 
	...
	MESSAGE kandoomsg2("E",1122,"") 
	DISPLAY ARRAY l_arr_rec_salesperson TO sr_salesperson.* 
		BEFORE DISPLAY 
			CALL publish_toolbar("kandoo","EC2","input-arr-l_arr_rec_salesperson-1") -- albo kd-502 
		...
		ON ACTION ("ACCEPT","DOUBLECLICK") --BEFORE FIELD sale_code 
			IF (l_idx > 0) AND (l_idx <= l_arr_rec_salesperson.getSize()) THEN
				CALL sper_daysales(glob_rec_kandoouser.cmpy_code,l_arr_rec_salesperson[l_idx].sale_code,"","") 
			END IF
...
END FUNCTION 
###########################################################################
# END FUNCTION salesperson_daily_sales() 
###########################################################################
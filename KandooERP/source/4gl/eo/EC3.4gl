...
# FUNCTION EC3_main()
#
# EC3 - allows users TO SELECT a salesperson TO which peruse
...
FUNCTION EC3_main() 
	...
	CALL setModuleId("EC3") -- albo 

	LET l_arg_sale_code = get_url_sale_code()
	IF l_arg_sale_code IS NOT NULL THEN
		CALL sper_daysales(glob_rec_kandoouser.cmpy_code,l_arg_sale_code,"","") 
	ELSE 
...
FUNCTION db_salesperson_get_datasource(p_filter)
	...
	DEFINE l_salesperson RECORD LIKE salesperson.* 
	...
	IF p_filter THEN	 
		...
		CONSTRUCT BY NAME l_where_text ON 
			...
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","EC3","construct-sale_code-1") -- albo kd-502 
	...
	LET l_idx = 0 
	FOREACH c_salesperson INTO l_salesperson.* 
		LET l_idx = l_idx + 1 
		LET l_arr_rec_salesperson[l_idx].sale_code = l_salesperson.sale_code 
		LET l_arr_rec_salesperson[l_idx].name_text = l_salesperson.name_text 

		IF l_salesperson.city_text IS NOT NULL THEN 
			LET l_arr_rec_salesperson[l_idx].city_text = l_salesperson.city_text 
		ELSE 
			LET l_arr_rec_salesperson[l_idx].city_text = l_salesperson.addr2_text 
		END IF 

		LET l_arr_rec_salesperson[l_idx].state_code = l_salesperson.state_code 
		LET l_arr_rec_salesperson[l_idx].mgr_code = l_salesperson.mgr_code 
		LET l_arr_rec_salesperson[l_idx].sale_type_ind = l_salesperson.sale_type_ind 
		
		SELECT unique 1 FROM statsper 
		WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
		AND sale_code = l_salesperson.sale_code 
...
FUNCTION scan_sale() 
	...
	MESSAGE kandoomsg2("E",1123,"") #1123 Salesperson Monthly Sales Info - RETRUN TO View
	DISPLAY ARRAY l_arr_rec_salesperson TO sr_salesperson.* ATTRIBUTE(UNBUFFERED) 
		BEFORE DISPLAY 
			CALL publish_toolbar("kandoo","EC3","input-arr-l_arr_rec_salesperson-1")
		...
		ON ACTION ("ACCEPT","DOUBLECLICK") 
			IF (l_idx > 0) AND (l_idx <= l_arr_rec_salesperson.getSize()) THEN
				CALL sper_mthsales(glob_rec_kandoouser.cmpy_code,l_arr_rec_salesperson[l_idx].sale_code,"")
			END IF 

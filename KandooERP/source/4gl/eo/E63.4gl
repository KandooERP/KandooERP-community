...
# FUNCTION E63_main()
#
# E63 - allows users TO SELECT a special offer TO peruse
...
FUNCTION db_offersale_get_datasource(p_filter)
	...
	IF p_filter THEN
		...
		CONSTRUCT BY NAME l_where_text ON 
			...
			prodline_disc_flag 
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","E63","construct-offer_code-1")
...
FUNCTION scan_offersale() 
	...
	#Populate dataArray with conditional construct, based on the size 
	...
	MESSAGE kandoomsg2("E",1144,"") #1144 Special Offer Weekly Turnover - RETURN TO View
	DISPLAY ARRAY l_arr_rec_offersale TO sr_offersale.* ATTRIBUTE(UNBUFFERED)
		BEFORE DISPLAY  
			CALL publish_toolbar("kandoo","E63","input-l_arr_rec_offersale-1") 
...
###########################################################################
# FUNCTION off_turnover(p_cmpy,p_offer_code)
#
#
###########################################################################
FUNCTION off_turnover(p_cmpy,p_offer_code) 
	...
	IF status = 0 THEN 
		...
		OPEN WINDOW E254 with FORM "E254" 
		 CALL windecoration_e("E254")  
		...
		WHILE TRUE #------------------------------------------------------------ 
			...
			LET l_arr_rec_stattotal[1].tot_sales_qty = 0 
			...
			FOREACH c_statint INTO l_rec_statint.* 
				...
				IF l_rec_cur_statoffer.sales_qty IS NULL THEN 
					LET l_rec_cur_statoffer.sales_qty = 0 
				END IF 
				...
				LET l_arr_rec_statoffer[l_idx].sales_qty = l_rec_cur_statoffer.sales_qty 
				...
				LET l_arr_rec_stattotal[1].tot_sales_qty = l_arr_rec_stattotal[1].tot_sales_qty	+ l_rec_cur_statoffer.sales_qty 
			END FOREACH 
			...
			ELSE 
				...
				MESSAGE kandoomsg2("E",1145,"")		#1145 " Special Offer Weekly Turnover - F3/F4

				DISPLAY ARRAY l_arr_rec_statoffer TO sr_statoffer.* ATTRIBUTE(UNBUFFERED)				
					BEFORE DISPLAY 
						CALL publish_toolbar("kandoo","E63","input-arr-l_arr_rec_statoffer-1") 

					BEFORE ROW 
						LET l_idx = arr_curr() 

					AFTER ROW 
						LET l_arr_rec_statoffer[l_idx].scroll_flag = NULL 
...
END FUNCTION
###########################################################################
# END FUNCTION off_turnover(p_cmpy,p_offer_code)
###########################################################################
...
GLOBALS "../eo/EE3_GLOBALS.4gl"
###########################################################################
# FUNCTION EE3_main()
#
# EE3 - allows users TO SELECT a sales territory TO peruse
...
FUNCTION EE3_main() 
	...
	CALL setModuleId("EE3") -- albo 

	LET l_terr_code = get_url_terr_code()
	IF l_terr_code IS NOT NULL THEN  
		CALL terr_dist(glob_rec_kandoouser.cmpy_code,l_terr_code,"") 
	ELSE 
		OPEN WINDOW E251 with FORM "E251" 
		 CALL windecoration_e("E251") -- albo kd-755
...
FUNCTION select_territory(p_filter) 
	...
	IF p_filter THEN
		...
		CONSTRUCT BY NAME l_where_text ON 
			...
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","EE3","construct-terr_code-1") -- albo kd-502 
	...
	FOREACH c_territory INTO l_arr_rec_territory[l_idx].* 
		...
		AND maingrp_code = l_arr_rec_territory[l_idx].maingrp_code 
		AND prodgrp_code = l_arr_rec_territory[l_idx].prodgrp_code 
		AND part_code = l_arr_rec_territory[l_idx].part_code
	...
	CALL l_arr_rec_territory.delete(l_idx)
	LET l_idx = l_idx -1

	IF l_arr_rec_territory.getSize() = 0 THEN
		ERROR "No records found/selected"
	END IF
...
FUNCTION scan_territory() 
	...
	MESSAGE kandoomsg2("E",1119,"")	#1119 Sales Territory Monthly Distribution - RETURN TO View
	DISPLAY ARRAY l_arr_rec_territory TO sr_territory.* ATTRIBUTE(UNBUFFERED) 
		BEFORE DISPLAY 
			CALL publish_toolbar("kandoo","EE3","input-arr-l_arr_rec_territory-1") -- albo kd-502 
		...
		ON ACTION ("ACCEPT","DOUBLECLICK") --BEFORE FIELD terr_code
			IF (l_idx > 0) AND (l_idx <= l_arr_rec_territory.getSize()) THEN 
				IF l_arr_rec_territory[l_idx].terr_code IS NOT NULL THEN 
					CALL terr_dist(glob_rec_kandoouser.cmpy_code,l_arr_rec_territory[l_idx].terr_code,	l_arr_rec_territory[l_idx].part_code) 
				END IF 
			END IF 
...
###########################################################################
# FUNCTION terr_dist(p_cmpy_code,p_terr_code,p_part_code)
#
#
###########################################################################
FUNCTION terr_dist(p_cmpy_code,p_terr_code,p_part_code) 
	...
	DEFINE l_terr_desc_text LIKE territory.desc_text 
	DEFINE l_rec_territory RECORD LIKE territory.* 
	...
	DEFINE l_arr_rec_distterr DYNAMIC ARRAY OF RECORD --array[100] OF RECORD 
		scroll_flag char(1), 
	...
	END RECORD 
	DEFINE l_arr_rec_stattotal_1 array[1] OF RECORD 
	...
	DEFINE l_terr_text char(40)
	DEFINE l_idx SMALLINT 
	DEFINE i SMALLINT
	
	OPEN WINDOW E252 with FORM "E252" 
	 CALL windecoration_e("E252") -- albo kd-755 
	...
	IF p_part_code IS NOT NULL THEN 
		...
		IF status = NOTFOUND THEN 
			CALL fgl_winmessage("Logic ERROR",kandoomsg2("I",5010,p_part_code),"ERROR") #5010" Logic Error: product NOT found"
			RETURN 
		END IF 
	END IF 
	...
	LET l_terr_desc_text = l_rec_territory.desc_text 
	IF status = 0 THEN 
		MENU " Inquiry level" 
			BEFORE MENU 
				CALL publish_toolbar("kandoo","EE3","menu-Inquiry_Level-1") -- albo kd-502 

			ON ACTION "WEB-HELP" -- albo kd-370 
				CALL onlinehelp(getmoduleid(),null) 

			ON ACTION "PRODUCT" --COMMAND "Product" " Inquiry on product sales" 
				LET l_type_ind = "1" 
				IF p_part_code IS NOT NULL THEN 
		...
		IF NOT (int_flag OR quit_flag) THEN 
			...
			LET l_terr_text = kandooword("Sales Territory","1") 
			LET l_terr_text = l_terr_text clipped,"........" 
			...
			WHILE TRUE 
				...
				DISPLAY l_terr_text TO terr_text
				DISPLAY l_prompt_text TO prompt_text
				DISPLAY l_rec_territory.terr_code TO terr_code 
				DISPLAY l_terr_desc_text TO territory.desc_text 
				...
				ELSE 
					...
					MESSAGE kandoomsg2("E",1120,"") #1120 Sales Terr Monthly Distribution - F9 Previous - F10 Next
	 
					DISPLAY ARRAY l_arr_rec_distterr TO sr_distterr.* ATTRIBUTE(UNBUFFERED) 
						BEFORE DISPLAY 
							CALL publish_toolbar("kandoo","EE3","input-arr-l_arr_rec_distterr-1") -- albo kd-502 
...
END FUNCTION 
###########################################################################
# END FUNCTION terr_dist(p_cmpy_code,p_terr_code,p_part_code)
###########################################################################
...
# FUNCTION EV3_main()
#
# EV3 Customer Yearly Turnover by Sales Area
###########################################################################
FUNCTION EV3_main() 
	DEFER QUIT 
	DEFER INTERRUPT 

	CALL setModuleId("EV3") 

	CASE rpt_init_url_get_operation_method_exec_ind() #url-arg 1=menu 2=batch 3=construct 4=query
		WHEN RPT_OP_MENU #UI/MENU Mode 	

			OPEN WINDOW E280 with FORM "E280" 
			 CALL windecoration_e("E280") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title
			DISPLAY getmenuitemlabel(NULL) TO header_text 
		
			MENU " Customer Turnover report" 
				BEFORE MENU 
					CALL publish_toolbar("kandoo","EV3","menu-Customer-1") -- albo kd-502 
					CALL rpt_rmsreps_reset(NULL)					 
					CALL EV3_rpt_process(EV3_rpt_query())
		...
		WHEN RPT_OP_BATCH #Background Process with rmsreps.report_code
			CALL EV3_rpt_process(NULL)  

		WHEN RPT_OP_CONSTRUCT #Only create query-where-part
			OPEN WINDOW E280 with FORM "E280" 
			 CALL windecoration_e("E280") 
			CALL displaymoduletitle(NULL) --first FORM OF the module get's the title 
			CALL set_url_sel_text(EV3_rpt_query()) #save where clause in env 
			CLOSE WINDOW E280 
			
		WHEN RPT_OP_QUERY #Background Process with SQL WHERE ARGUMENT
			CALL EV3_rpt_process(get_url_sel_text())
	END CASE 		

END FUNCTION 
###########################################################################
# END FUNCTION EV3_main()
###########################################################################


###########################################################################
# FUNCTION EV3_rpt_query()
#
# EV3 Customer Yearly Turnover by Sales Area
###########################################################################
FUNCTION EV3_rpt_query() 
	...
	country_code 
		...
		BEFORE CONSTRUCT 
			CALL publish_toolbar("kandoo","EV3","construct-cust_codecust_codecust_codes-1") -- albo kd-502 
...
END FUNCTION
###########################################################################
# END FUNCTION EV3_rpt_query()
###########################################################################


############################################################
# FUNCTION EV3_rpt_process(p_where_text) 
#
#
############################################################
FUNCTION EV3_rpt_process(p_where_text) 
	...
	LET l_rpt_idx = rpt_start(getmoduleid(),"EV3_rpt_list",p_where_text, RPT_SHOW_RMS_DIALOG)
	IF l_rpt_idx = 0 THEN #User pressed CANCEL
		RETURN FALSE
	END IF	
	START REPORT EV3_rpt_list TO rpt_get_report_file_with_path2(l_rpt_idx)
...
--	LET modu_order_ind = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("EV3_rpt_list")].ref5_ind
	LET modu_zero_stats_flag = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("EV3_rpt_list")].ref6_ind
	LET glob_rec_statparms.year_num = glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("EV3_rpt_list")].ref5_num 
	#------------------------------------------------------------	


	LET l_query_text = "SELECT customer.*,territory.area_code", 
	...
	"AND customer.territory_code = territory.terr_code ", 
	"AND delete_flag = 'N' ", 
	"AND ",glob_arr_rec_rpt_rmsreps[rpt_rmsreps_idx_get_idx("EV3_rpt_list")].sel_text clipped," ",
	"ORDER BY customer.cmpy_code,area_code,cust_code" 
	...
	FOREACH c_customer INTO l_rec_customer.*,l_rec_territory.area_code 

		#---------------------------------------------------------
		OUTPUT TO REPORT EV3_rpt_list(l_rpt_idx,
		l_rec_customer.*,l_rec_territory.area_code) 
	...
	#------------------------------------------------------------
	FINISH REPORT EV3_rpt_list
	CALL rpt_finish("EV3_rpt_list")
	#------------------------------------------------------------
...
END FUNCTION 
############################################################
# END FUNCTION EV3_rpt_process(p_where_text) 
############################################################


###########################################################################
# FUNCTION EV3_enter_year()
#
# 
###########################################################################
FUNCTION EV3_enter_year() 
	...
	ATTRIBUTE(UNBUFFERED)
		...
		BEFORE INPUT 
			CALL publish_toolbar("kandoo","EV3","input-year_num-1") -- albo kd-502 
...
END FUNCTION 
###########################################################################
# END FUNCTION EV3_enter_year()
###########################################################################


###########################################################################
# REPORT EV3_rpt_list(p_rec_customer,p_area_code)
#
# 
###########################################################################
REPORT EV3_rpt_list(p_rpt_idx,p_rec_customer,p_area_code) 
	...
	FORMAT 
		PAGE HEADER 
			...
			INITIALIZE l_rec_salearea.* TO NULL 
			SELECT * INTO l_rec_salearea.* FROM salearea 
			WHERE cmpy_code = glob_rec_kandoouser.cmpy_code 
			AND area_code = p_area_code 
			PRINT COLUMN 01,"Sales Area: ", 
			COLUMN 13,p_area_code, 
			COLUMN 19,l_rec_salearea.desc_text 
			SKIP 1 line 
			
		BEFORE GROUP OF p_area_code 
			SKIP TO top OF PAGE 
			LET l_no_cust_printed = TRUE
			 
		AFTER GROUP OF p_area_code 
			IF l_no_cust_printed THEN 
				PRINT COLUMN 10, "No customer statistics FOR selected sales area" 
			END IF
...
END REPORT
###########################################################################
# END REPORT EV3_rpt_list(p_rec_customer,p_area_code)
###########################################################################
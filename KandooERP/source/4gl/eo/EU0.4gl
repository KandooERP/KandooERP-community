...
# WITHOUT ANY WARRANTY; without EUen the implied warranty of
...
FUNCTION EU0_main() 
	...
	CALL setModuleId("EU0")

	LET modu_report_run_once_code = get_url_module_child() 
	IF modu_report_run_once_code IS NOT NULL THEN 
		CALL EU0_run_module(modu_report_run_once_code)
		RETURN 
	ELSE
		LET l_list_menuchoice = "EU1"
		LET l_inp_menuchoice = l_list_menuchoice 
	END IF
	 
	OPEN WINDOW EU00 with FORM "EU00" ATTRIBUTE(BORDER,STYLE="full-SCREEN")
	 CALL windecoration_e("EU00") 
	...
	DIALOG ATTRIBUTES(UNBUFFERED) 
		...
		ON ACTION ("ACCEPT", "DoubleClickItem") 
			...
			DISPLAY l_list_menuchoice TO tf_menuChoice		
			CALL EU0_run_module(l_list_menuchoice)
			IF int_flag THEN
...
############################################################
# FUNCTION EU0_run_module(p_module_id)
#
...
FUNCTION EU0_run_module(p_module_id)
	...
	CASE p_module_id.toUpperCase()
		...
		OTHERWISE # 
			CALL fgl_winmessage("Invalid Menu Option in EU","Invalid Menu Option choosen","error") 

	END CASE
	 	
	CALL setModuleId("EU00") #Reset program module id

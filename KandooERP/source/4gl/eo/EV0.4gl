...
FUNCTION EV0_main() 
	...
	CALL setModuleId("EV0")

	LET modu_report_run_once_code = get_url_module_child()
	IF modu_report_run_once_code IS NOT NULL THEN 
		CALL EV0_run_module(modu_report_run_once_code)
		RETURN 
	ELSE
		LET l_list_menuchoice = "EV1"
		LET l_inp_menuchoice = l_list_menuchoice 
	END IF
	 
	OPEN WINDOW EV00 with FORM "EV00" ATTRIBUTE(BORDER,STYLE="full-SCREEN")
	 CALL windecoration_e("EV00") 
	...
	DIALOG ATTRIBUTES(UNBUFFERED) 
		...
		ON ACTION ("ACCEPT", "DoubleClickItem") 
			...
			DISPLAY l_list_menuchoice TO tf_menuChoice		
			CALL EV0_run_module(l_list_menuchoice)
			IF int_flag THEN
...
############################################################
# FUNCTION EV0_run_module(p_module_id)
#
...
FUNCTION EV0_run_module(p_module_id)
	...
	CASE p_module_id.toUpperCase()
		...
		OTHERWISE # 
			CALL fgl_winmessage("Invalid Menu Option in EV","Invalid Menu Option choosen","error") 

	END CASE
	 	
	CALL setModuleId("EV00") #Reset program module id

...
GLOBALS "../eo/EC9_GLOBALS.4gl"
###########################################################################
# FUNCTION EC9_main()
#
# EC9 - allows users TO SELECT a salesperson TO which peruse
...
FUNCTION EC9_main() 
	...
	CALL setModuleId("EC9") -- albo 

	LET l_arg_sale_code = get_url_sale_code()
	IF l_arg_sale_code IS NOT NULL THEN
		CALL sper_profit(glob_rec_kandoouser.cmpy_code,l_arg_sale_code) 
	ELSE 
...
FUNCTION db_salesperson_get_datasource(p_filter) 
	...
	IF p_filter THEN 
		...
		sale_type_ind 
			...
			BEFORE CONSTRUCT 
				CALL publish_toolbar("kandoo","EC9","construct-sale_code-1") -- albo kd-502 
...
FUNCTION scan_sale() 
	...
	MESSAGE kandoomsg2("E",1092,"") #1092 Salesperson Profit Figures - RETURN TO View
	DISPLAY ARRAY l_arr_rec_salesperson TO sr_salesperson.* 
		BEFORE DISPLAY 
			CALL publish_toolbar("kandoo","EC9","input-arr-l_arr_rec_salesperson-1") 
		...
		ON ACTION ("ACCEPT","DOUBLECLICK") 
			IF (l_idx > 0) AND (l_idx <= l_arr_rec_salesperson.getSize()) THEN
				CALL sper_profit(glob_rec_kandoouser.cmpy_code,l_arr_rec_salesperson[l_idx].sale_code)
			END IF 
